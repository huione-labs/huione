...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "huione", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The HC CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a HC validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// HC documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://docs.huione.com/cluster/rpc-endpoints
    ...
    /// documentation for `huione_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `huione_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from HC addresses to human-readable names.
    ///
    ...
    /// `huione_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "huione", "id.json"]);
