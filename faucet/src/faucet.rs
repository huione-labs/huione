//! The `faucet` module provides an object for launching a HC Faucet,
//! which is the custodian of any remaining lamports in a mint.
//! The HC Faucet builds and sends airdrop transactions,
...
pub struct Faucet {
    ...
    ip_cache: HashMap<IpAddr, u128>,
    address_cache: HashMap<Pubkey, u128>,
    pub time_slice: Duration,
    per_time_cap: Option<u128>,
...
impl Faucet {
    ...
    pub fn new_with_allowed_ips(
        ...
        if let Some((per_request_cap, per_time_cap)) = per_request_cap.zip(per_time_cap) {
            if per_time_cap < per_request_cap {
                warn!(
                    "per_time_cap {} HC < per_request_cap {} HC; \
                    maximum single requests will fail",
                    lamports_to_hc(per_time_cap),
                    lamports_to_hc(per_request_cap),
                );
            }
        }
        Self {
        ...
        if let Some(cap) = self.per_time_cap {
            if new_total > cap {
                return Err(FaucetError::PerTimeCapExceeded(
                    lamports_to_hc(request_amount),
                    to.to_string(),
                    lamports_to_hc(new_total),
                    lamports_to_hc(cap),
                ));
            }
        }
    ...
    /// the request exceeds this per-request limit, this method returns a signed HPL Memo
    /// transaction with the memo: "request too large; req: <REQUEST> HC cap: <CAP> HC"
    pub fn build_airdrop_transaction(
        ...
        match req {
            FaucetRequest::GetAirdrop {
                ...
                info!(
                    "Requesting airdrop of {} HC to {:?}",
                    lamports_to_hc(lamports),
                    to
                );
                ...
                if let Some(cap) = self.per_request_cap {
                    if lamports > cap {
                        ...
                        let memo_instruction = Instruction {
                            program_id: Pubkey::from(hpl_memo::id().to_bytes()),
                            accounts: vec![],
...
pub fn run_local_faucet(faucet_keypair: Keypair, per_time_cap: Option<u128>) -> SocketAddr {
...
pub trait LimitByTime {
    fn check_cache(&self, faucet: &mut Faucet, request_amount: u128) -> u128;
    fn datapoint_info(&self, request_amount: u128, new_total: u128);
}
...
impl LimitByTime for IpAddr {
    ...
    fn datapoint_info(&self, request_amount: u128, new_total: u128) {
        datapoint_info!(
            "faucet-airdrop",
            ("request_amount", request_amount, i128),
            ("ip", self.to_string(), String),
            ("new_total", new_total, i128)
        );
    }
}
...
#[cfg(test)]
mod tests {
    use {super::*, huione_sdk::system_instruction::SystemInstruction, std::time::Duration};
    ...
    fn test_faucet_default_init() {
        ...
        let per_time_cap: Option<u128> = Some(200);
        let per_request_cap: Option<u128> = Some(100);
    ...
    fn test_faucet_build_airdrop_transaction() {
            ...
            faucet.build_airdrop_transaction(request, ip).unwrap()
        {
            ...
            assert_eq!(
                message.account_keys,
                vec![mint_pubkey, Pubkey::from(hpl_memo::id().to_bytes())]
            );
            assert_eq!(message.recent_blockhash, blockhash);
