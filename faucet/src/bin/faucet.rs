use {
    ...
    log::*,
    huione_clap_utils::input_parsers::{lamports_of_hc, value_of},
    huione_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    huione_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = huione_cli_config::Config::default().keypair_path;

    huione_logger::setup_with_default("huione=info");
    huione_metrics::set_panic_hook("faucet", /*version:*/ None);
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in HC"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in HC"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_hc(&matches, "per_time_cap");
    let per_request_cap = lamports_of_hc(&matches, "per_request_cap");
