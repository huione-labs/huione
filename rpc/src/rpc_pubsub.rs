...
use {
    ...
    huione_account_decoder::{UiAccount, UiAccountEncoding},
    ...
    huione_transaction_status::UiTransactionEncoding,
    std::{str::FromStr, sync::Arc},
};

// We have to keep both of the following traits to not break backwards compatibility.
// `RpcHuionePubSubInternal` is actually used by the current PubSub API implementation.
// `RpcHuionePubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcHuionePubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcHuionePubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcHuionePubSubInternal for RpcHuionePubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        huione_account_decoder::{parse_account_data::parse_account_data, UiAccountEncoding},
        ...
        huione_vote_program::vote_state::Vote,
        std::{
            sync::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = huione_sdk::pubkey::new_rand();
    ...
    fn test_get_version() {
        ...
        let expected_version = huione_version::Version::default();
