...
use {
    crate::{
        rpc_pubsub::{RpcHuionePubSubImpl, RpcHuionePubSubInternal},
...
impl PubSubService {
    pub fn new(
        ...
        let thread_hdl = Builder::new()
            .name("huioneRpcPubSub".to_string())
            .spawn(move || {
...
pub fn test_connection(
    subscriptions: &Arc<RpcSubscriptions>,
) -> (RpcHuionePubSubImpl, TestBroadcastReceiver) {
    let current_subscriptions = Arc::new(DashMap::new());

    let rpc_impl = RpcHuionePubSubImpl::new(
        PubSubConfig {
...
mod tests {
    ...
    fn test_pubsub_new() {
        ...
        assert_eq!(thread.name().unwrap(), "huioneRpcPubSub");
    }
}
