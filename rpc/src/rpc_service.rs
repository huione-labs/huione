//! The `rpc_service` module implements the HC JSON RPC service.
...
use {
    ...
    regex::Regex,
    huione_client::connection_cache::ConnectionCache,
    ...
    huione_perf::thread::renice_this_thread,
    huione_poh::poh_recorder::PohRecorder,
    huione_runtime::{
    ...
    huione_sdk::{
        ...
        native_token::lamports_to_hc, pubkey::Pubkey,
    },
    huione_send_transaction_service::send_transaction_service::{self, SendTransactionService},
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                huione_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
                    .expect("Scan should not error on root banks")
                    .lamports;
            Some(format!(
                "{}",
                lamports_to_hc(total_supply - non_circulating_supply)
            ))
        }
        "/v0/total-supply" => {
            let bank = bank_forks.read().unwrap().root_bank();
            let total_supply = bank.capitalization();
            // feature comment
            // non_circulating_supply include stakers lamports. 
            Some(format!("{}", lamports_to_hc(total_supply - 14703040)))
        }
        _ => None,
    }
}
...
impl JsonRpcService {
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            ...
            {
                let bigtable_config = huione_storage_bigtable::LedgerStorageConfig {
                ...
                runtime
                    .block_on(huione_storage_bigtable::LedgerStorage::new_with_config(
                        bigtable_config,
                    ))
        ...
        if config.rpc_hbase_config.is_some() {
            huione_storage_hbase::LedgerStorage::new(
                    config.rpc_hbase_config.clone().unwrap().hbase_rpc_url,
                    None,
                )
                .map(|hbase_ledger_storage| {
                    info!("Hbase edger storage initialized");
                    Some(hbase_ledger_storage)
                })
                .unwrap_or_else(|err| {
        ...
        let thread_hdl = Builder::new()
            .name("huioneJsonRpcSvc".to_string())
            .spawn(move || {
...
mod tests {
    use {
        ...
        huione_rpc_client_api::config::RpcContextConfig,
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            huione_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
        ...
        assert_eq!(thread.name().unwrap(), "huioneJsonRpcSvc");
