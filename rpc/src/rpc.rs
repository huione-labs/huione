//! The `rpc` module implements the HC RPC interface.

use hpl_name::instruction::get_domain;
use hpl_name::state::{get_seeds_and_key, AccountType};
use huione_account_decoder::parse_name::{is_known_name_id, NameAccountType};
use huione_account_decoder::parse_nft::is_known_nft_token_id;
use huione_account_decoder::UiAccountData;
use huione_runtime::inline_name;
use huione_runtime::inline_name::{
    DOMAIN_ACCOUNT_OWNER_OFFSET, DOMAIN_ACCOUNT_TYPE_OFFSET, DOMAIN_RESOLVE_ACCOUNT_PARENT_OFFSET,
    DOMAIN_RESOLVE_ACCOUNT_TYPE_OFFSET,
};
use huione_runtime::inline_nft::{NFT_ACCOUNT_LENGTH, NFT_ACCOUNT_OWNER_OFFSET};
...
use {
    ...
    huione_account_decoder::{
        parse_token::{is_known_hpl_token_id, token_amount_to_ui_amount, UiTokenAmount},
        UiAccount, UiAccountEncoding, UiDataSliceConfig, MAX_BASE58_BYTES,
    },
    huione_client::connection_cache::{ConnectionCache, Protocol},
    huione_entry::entry::Entry,
    huione_faucet::faucet::request_airdrop_transaction,
    huione_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo},
    huione_ledger::{
    ...
    huione_metrics::inc_new_counter_info,
    huione_perf::packet::PACKET_DATA_SIZE,
    huione_rpc_client_api::{
        config::*,
    ...
    huione_runtime::{
        ...
        inline_hpl_token::{HPL_TOKEN_ACCOUNT_MINT_OFFSET, HPL_TOKEN_ACCOUNT_OWNER_OFFSET},
    ...
    huione_send_transaction_service::{
        send_transaction_service::{SendTransactionService, TransactionInfo},
        tpu_info::NullTpuInfo,
    },
    huione_stake_program,
    huione_storage_bigtable::Error as StorageError,
    huione_storage_hbase::Error as HBaseStorageError,
    huione_streamer::socket::SocketAddrSpace,
    huione_transaction_status::{
    ...
    huione_vote_program::vote_state::{VoteState, MAX_LOCKOUT_HISTORY},
    std::{
        any::type_name,
        cmp::{max, min},
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = huione_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = huione_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
...
pub struct RpcHBaseConfig {
    ...
    pub enable_rpc_hbase_ledger_storage: bool,
    pub hbase_rpc_url: String,
}
...
impl Default for RpcHBaseConfig {
    fn default() -> Self {
        Self {
            ...
            enable_rpc_hbase_ledger_storage: false,
            hbase_rpc_url: String::default(),
        }
    }
}
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<huione_storage_bigtable::LedgerStorage>,
    hbase_ledger_storage: Option<huione_storage_hbase::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://github.com/huione-labs/huione/issues/11078
            warn!(
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                huione_sdk::timing::timestamp(), // wallclock
            );
    ...
    pub fn get_program_accounts(
        ...
        let keyed_accounts = {
            if let Some(owner) = get_hpl_token_owner_filter(program_id, &filters) {
                self.get_filtered_hpl_token_accounts_by_owner(&bank, program_id, &owner, filters)?
            } else if let Some(mint) = get_hpl_token_mint_filter(program_id, &filters) {
                self.get_filtered_hpl_token_accounts_by_mint(&bank, program_id, &mint, filters)?
            } else {
                self.get_filtered_program_accounts(&bank, program_id, filters)?
            }
        };
        let accounts = if is_known_hpl_token_id(program_id)
    ...
    pub async fn get_inflation_reward(
        ...
        if first_slot_in_epoch < first_available_block {
            if self.bigtable_ledger_storage.is_some() || self.hbase_ledger_storage.is_some() {
    ...
    fn get_total_supply(&self, commitment: Option<CommitmentConfig>) -> Result<u128> {
        let bank = self.bank(commitment);
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, huione_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(huione_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    fn check_hbase_result<T>(
        &self,
        result: &std::result::Result<T, huione_storage_hbase::Error>,
    ) -> Result<()> {
        if let Err(huione_storage_hbase::Error::BlockNotFound(slot)) = result {
            return Err(RpcCustomError::LongTermStorageSlotSkipped { slot: *slot }.into());
        }
        Ok(())
    }
                    ...
                    .highest_super_majority_root()
            {
                ...
                if result.is_err() {
                    ...
                    if let Some(hbase_ledger_storage) = &self.hbase_ledger_storage {
                        let hbase_result =
                            hbase_ledger_storage.get_confirmed_block(slot);
            ...
            if let Some(hbase_ledger_storage) = &self.hbase_ledger_storage {
                return hbase_ledger_storage
                    ...
                    .map(|mut hbase_blocks| {
                        hbase_blocks.retain(|&slot| slot <= end_slot);
                        hbase_blocks
                    })
                    .map_err(|_| {
                        Error::invalid_params(
    ...
    pub async fn get_block_time(&self, slot: Slot) -> Result<Option<UnixTimestamp>> {
                ...
                .highest_super_majority_root()
        {
            ...
            if result.is_err() || matches!(result, Ok(None)) {
                ...
                if let Some(hbase_ledger_storage) = &self.hbase_ledger_storage {
                    let hbase_result = hbase_ledger_storage.get_confirmed_block(slot);
                    self.check_hbase_result(&hbase_result)?;
            ...
            let status = if let Some(status) = self.get_transaction_status(signature, &bank) {
                    ...
                    })
                {
                    ...
                    bigtable_ledger_storage
                        ...
                        .map(Some)
                        .unwrap_or(None)
                } else if let Some(hbase_ledger_storage) = &self.hbase_ledger_storage {
                    hbase_ledger_storage
                        .get_signature_status(&signature)
                        .map(Some)
                        .unwrap_or(None)
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            match confirmed_transaction.unwrap_or(None) {
                Some(mut confirmed_transaction) => {
                    ...
                    {
                        // // oa debug
                        // let return_data = encode_transaction(confirmed_transaction).expect("encode_transaction : error!");
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://github.com/huione-labs/huione/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub async fn get_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            if results.len() < limit {
                ...
                if let Some(hbase_ledger_storage) = &self.hbase_ledger_storage {
                    let mut hbase_before = before;
                    if !results.is_empty() {
                        limit -= results.len();
                        hbase_before = results.last().map(|x| x.signature);
                    }

                    // If the oldest address-signature found in Blockstore has not yet been
                    // uploaded to long-term storage, modify the storage query to return all latest
                    // signatures to prevent erroring on RowNotFound. This can race with upload.
                    if found_before && hbase_before.is_some() {
                        match hbase_ledger_storage
                            .get_signature_status(&hbase_before.unwrap())
                        {
                            Err(HBaseStorageError::SignatureNotFound) => {
                                hbase_before = None;
                            }
                            Err(err) => {
                                warn!("{:?}", err);
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > huione_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            huione_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
    ...
    pub fn get_token_account_balance(
        ...
        if !is_known_hpl_token_id(account.owner()) {
        ...
        let (_, decimals, _, _, _) = get_mint_owner_and_decimals(&bank, mint)?;
    ...
    pub fn get_token_supply(
        ...
        if !is_known_hpl_token_id(mint_account.owner()) {
            return Err(Error::invalid_params(
    ...
    pub fn get_token_largest_accounts(
        ...
        let bank = self.bank(commitment);
        let (mint_owner, decimals, _, _, _) = get_mint_owner_and_decimals(&bank, mint)?;
        if !is_known_hpl_token_id(&mint_owner) {
            return Err(Error::invalid_params(
                "Invalid param: not a Token mint".to_string(),
            ));
        }
        let mut token_balances: Vec<RpcTokenAccountBalance> = self
            .get_filtered_hpl_token_accounts_by_mint(&bank, &mint_owner, mint, vec![])?
            .into_iter()
            .map(|(address, account)| {
        ...
        token_balances.sort_by(|a, b| {
            a.amount
                ...
                .parse::<u128>()
                .unwrap()
                .cmp(&b.amount.amount.parse::<u128>().unwrap())
                .reverse()
        });
    ...
    pub fn get_token_accounts_by_owner(
        ...
        let keyed_accounts = self.get_filtered_hpl_token_accounts_by_owner(
            &bank,
            &token_program_id,
            owner,
            filters,
    ...
    pub fn get_nft_accounts_by_owner(
        ...
        let (nft_program_id, mint) = get_nft_program_id_and_mint(&bank, token_account_filter)?;
        ...
        let keyed_accounts =
            self.get_filtered_nft_accounts_by_owner(&bank, &nft_program_id, owner, filters)?;

        let accounts = if encoding == UiAccountEncoding::JsonParsed {
            get_parsed_ui_accounts(keyed_accounts.into_iter()).collect()
        } else {
            keyed_accounts
                .into_iter()
                .map(|(pubkey, account)| {
    ...
    pub fn get_domain_accounts_by_owner(
        &self,
        owner: &Pubkey,
        config: Option<RpcAccountInfoConfig>,
    ) -> Result<RpcResponse<Vec<RpcKeyedAccount>>> {
        ...
        let data_slice_config = config.data_slice;
        check_slice_and_encoding(&encoding, data_slice_config.is_some())?;
        let name_program_id = inline_name::id();
        let filters = vec![];

        // Get domain accounts by owner
        let own_domain_accounts =
            self.get_filtered_domain_accounts_by_owner(&bank, &name_program_id, owner, filters)?;
        // Filter expired domain
        let domaint_accounts: Vec<RpcKeyedAccount> =
            get_parsed_ui_accounts(own_domain_accounts.into_iter()).collect();
        let mut ret = domaint_accounts.clone();
        for account in domaint_accounts {
            let domain_puk = account.pubkey.parse::<Pubkey>().unwrap();
            let filters = vec![];
            let own_domain_resolve_accounts = self.get_filtered_domain_resolve_accounts_by_parent(
                &bank,
                &name_program_id,
                &domain_puk,
                filters,
            );
            if own_domain_resolve_accounts.is_err() {
                info!("get_filtered_domain_resolve_accounts_by_parent is err");
                continue;
            }
            let own_domain_resolve_accounts = own_domain_resolve_accounts.unwrap();
            let mut domain_resolve_accounts: Vec<RpcKeyedAccount> =
                get_parsed_ui_accounts(own_domain_resolve_accounts.into_iter()).collect();
            ret.append(&mut domain_resolve_accounts);
        }

        Ok(new_response(&bank, ret))
    }

    pub fn get_domain_resolve_account(
        &self,
        ...
        let response = get_resolve_domain_encoded_account(
            &bank,
    ...
    pub fn get_domain_account(
        ...
        let response =
            get_domain_encoded_account(&bank, domain_account, encoding, config.data_slice)?;
        Ok(new_response(&bank, response))
    }

    pub fn get_domain_address_account(
        &self,
        domain_address_account: &Pubkey,
        config: Option<RpcAccountInfoConfig>,
    ) -> Result<RpcResponse<Option<UiAccount>>> {
        let config = config.unwrap_or_default();
        let bank = self.bank(config.commitment);
        ...
        let response =
            get_domain_encoded_account(&bank, domain_address_account, encoding, config.data_slice)?;
        Ok(new_response(&bank, response))
    }
    ...
    pub fn get_token_accounts_by_delegate(
        ...
        let keyed_accounts = if let Some(mint) = mint {
            self.get_filtered_hpl_token_accounts_by_mint(&bank, &token_program_id, &mint, filters)?
        } else {
    ...
    /// Get an iterator of hpl-token accounts by owner address
    fn get_filtered_hpl_token_accounts_by_owner(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        owner_key: &Pubkey,
        ...
        filters.push(RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            HPL_TOKEN_ACCOUNT_OWNER_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::HplTokenOwner)
        {
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::HplTokenOwner(*owner_key),
                    |account| {
    ...
    /// Get an iterator of nft accounts by owner address
    fn get_filtered_nft_accounts_by_owner(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        owner_key: &Pubkey,
        ...
        //
        // Filter on Token Account state
        filters.push(RpcFilterType::DataSize(NFT_ACCOUNT_LENGTH as u64));
        // Filter on Owner address
        filters.push(RpcFilterType::Memcmp(Memcmp::new_raw_bytes(
            NFT_ACCOUNT_OWNER_OFFSET,
            owner_key.to_bytes().into(),
        )));
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::HplNftOwner)
        {
            debug!("nft account index in effect!");
            if !self.config.account_indexes.include_key(owner_key) {
                return Err(RpcCustomError::KeyExcludedFromSecondaryIndex {
                    index_key: owner_key.to_string(),
                });
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::HplNftOwner(*owner_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    /// Get an iterator of domain accounts by owner address
    fn get_filtered_domain_accounts_by_owner(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        owner_key: &Pubkey,
        ...
        // later updates. We include the redundant filters here to avoid returning these accounts.
        //
        // Filter on Domain Type
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: DOMAIN_ACCOUNT_TYPE_OFFSET,
            bytes: MemcmpEncodedBytes::Bytes(vec![1]),
            encoding: None,
        }));
        // Filter on Owner address
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: DOMAIN_ACCOUNT_OWNER_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::HplNameOwner)
        {
            if !self.config.account_indexes.include_key(owner_key) {
                return Err(RpcCustomError::KeyExcludedFromSecondaryIndex {
                    index_key: owner_key.to_string(),
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::HplNameOwner(*owner_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    /// Get an iterator of domain resolve accounts by parent address
    fn get_filtered_domain_resolve_accounts_by_parent(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        parent_key: &Pubkey,
        ...
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: DOMAIN_RESOLVE_ACCOUNT_TYPE_OFFSET,
            bytes: MemcmpEncodedBytes::Bytes(vec![2]),
            encoding: None,
        }));
        // Filter on Owner address
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: DOMAIN_RESOLVE_ACCOUNT_PARENT_OFFSET,
            bytes: MemcmpEncodedBytes::Bytes(parent_key.to_bytes().into()),
            encoding: None,
        }));
        ...
        if self
            ...
            .contains(&AccountIndex::HplNameOwner)
        {
            if !self.config.account_indexes.include_key(parent_key) {
                return Err(RpcCustomError::KeyExcludedFromSecondaryIndex {
                    index_key: parent_key.to_string(),
                });
            }
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::HplNameOwner(*parent_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    /// Get an iterator of hpl-token accounts by mint address
    fn get_filtered_hpl_token_accounts_by_mint(
        &self,
        bank: &Arc<Bank>,
        program_id: &Pubkey,
        mint_key: &Pubkey,
        ...
        #[allow(deprecated)]
        filters.push(RpcFilterType::Memcmp(Memcmp {
            offset: HPL_TOKEN_ACCOUNT_MINT_OFFSET,
        ...
        if self
            .config
            .account_indexes
            .contains(&AccountIndex::HplTokenMint)
        {
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    &IndexKey::HplTokenMint(*mint_key),
                    |account| {
                        account.owner() == program_id
                            && filters.iter().all(|filter_type| match filter_type {
                                RpcFilterType::DataSize(size) => {
    ...
    fn get_stake_minimum_delegation(&self, config: RpcContextConfig) -> Result<RpcResponse<u128>> {
        let bank = self.get_bank_with_config(config)?;
        let stake_minimum_delegation =
            huione_stake_program::get_minimum_delegation(&bank.feature_set);
...
fn get_encoded_account(
    ...
    match bank.get_account(pubkey) {
        Some(account) => {
            let response = if is_known_hpl_token_id(account.owner())
                && encoding == UiAccountEncoding::JsonParsed
            {
...
fn get_resolve_domain_encoded_account(
    bank: &Arc<Bank>,
    domain_resolve_account: &Pubkey,
    domain_account: &Pubkey,
    encoding: UiAccountEncoding,
    ...
    // Check whether domain account is expired
    match bank.get_account(domain_account) {
        Some(account) => {
            if !is_known_name_id(account.owner()) {
                info!("invalid domain account");
                return Ok(None);
            }
            let domain_ui_account = encode_account(&account, domain_account, encoding, data_slice)?;
            if !check_domain_expired(&domain_ui_account.data) {
                return Ok(None);
            }
        }

        None => {
            info!("domain account not exist");
            return Ok(None);
        }
    };
    // Parse domain parse account
    match bank.get_account(domain_resolve_account) {
        Some(account_data) => {
            if is_known_name_id(account_data.owner()) {
                let response =
                    encode_account(&account_data, domain_resolve_account, encoding, data_slice)?;
                Ok(Some(response))
            } else {
...
#[allow(dead_code)]
fn get_domain_address_encoded_account(
    bank: &Arc<Bank>,
    domain_address_account: &Pubkey,
    encoding: UiAccountEncoding,
    data_slice: Option<UiDataSliceConfig>,
) -> Result<Option<UiAccount>> {
    match bank.get_account(domain_address_account) {
        Some(account) => {
            if is_known_name_id(account.owner()) {
                let response =
                    encode_account(&account, domain_address_account, encoding, data_slice)?;
                let ui_account = response.data.clone();
                if !check_address_domain_expired(bank, ui_account, encoding, data_slice) {
                    return Ok(None);
                }
                Ok(Some(response))
            } else {
                info!("domain address account not exist");
                return Ok(None);
            }
        }
        None => {
...
#[allow(dead_code)]
fn check_address_domain_expired(
    bank: &Arc<Bank>,
    address_ui_account: UiAccountData,
    encoding: UiAccountEncoding,
    data_slice: Option<UiDataSliceConfig>,
) -> bool {
    if let UiAccountData::Json(parsed_account) = address_ui_account {
        let name_account_type: NameAccountType =
            serde_json::from_value(parsed_account.parsed).unwrap();

        if let NameAccountType::AddressResolve(address_ui_account) = name_account_type {
            let resolve_domain_name = address_ui_account.domain;
            let domain_name = get_domain(&resolve_domain_name);
            let domain_name_hash = hashv(&[domain_name.as_bytes()]);
            let (domain_account_pk, _) = get_seeds_and_key(
                &hpl_name::id(),
                Some(domain_name_hash.to_bytes().to_vec()),
                AccountType::Domain,
                None,
            )
            .unwrap();
            let domain_account = bank.get_account(&domain_account_pk);

            if domain_account.is_none()
                || !is_known_name_id(domain_account.clone().unwrap().owner())
            {
                return false;
            }
            // Check whether domain account expired
            let domain_ui_account = encode_account(
                &domain_account.unwrap(),
                &domain_account_pk,
                encoding,
                data_slice,
            );
            if domain_ui_account.is_err() {
                return false;
            }
            return check_domain_expired(&domain_ui_account.unwrap().data);
        }
    }
    false
}

fn check_domain_expired(domain_ui_account: &UiAccountData) -> bool {
    if let UiAccountData::Json(_) = domain_ui_account {
        true
    } else {
        false
    }
...
/// Analyze custom filters to determine if the result will be a subset of hpl-token accounts by
/// owner.
...
fn get_hpl_token_owner_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_hpl_token_id(program_id) {
        return None;
    }
    ...
    for filter in filters {
        match filter {
            ...
            RpcFilterType::Memcmp(Memcmp {
                offset: HPL_TOKEN_ACCOUNT_OWNER_OFFSET,
                bytes: MemcmpEncodedBytes::Bytes(bytes),
                ..
            }) => {
                if bytes.len() == PUBKEY_BYTES {
                    owner_key = Some(Pubkey::try_from(bytes.clone()).unwrap());
                } else {
    ...
    if data_size_filter == Some(TokenAccount::get_packed_len() as u64) {
        if let Some(incorrect_owner_len) = incorrect_owner_len {
            info!(
                "Incorrect num bytes ({:?}) provided for hpl_token_owner_filter",
                incorrect_owner_len
            );
        }
        owner_key
    } else {
        debug!("hpl_token program filters do not match by-owner index requisites");
        None
    }
}
...
/// NOTE: `optimize_filters()` should almost always be called before using this method because of
/// the strict match on `MemcmpEncodedBytes::Bytes`.
fn get_hpl_token_mint_filter(program_id: &Pubkey, filters: &[RpcFilterType]) -> Option<Pubkey> {
    if !is_known_hpl_token_id(program_id) {
        return None;
    }
    let mut data_size_filter: Option<u64> = None;
    ...
    for filter in filters {
        match filter {
            ...
            RpcFilterType::Memcmp(Memcmp {
                ...
                if bytes.len() == PUBKEY_BYTES {
                    mint = Some(Pubkey::try_from(bytes.clone()).unwrap());
                } else {
    ...
    if data_size_filter == Some(TokenAccount::get_packed_len() as u64) {
        if let Some(incorrect_mint_len) = incorrect_mint_len {
            info!(
                "Incorrect num bytes ({:?}) provided for hpl_token_mint_filter",
                incorrect_mint_len
            );
        }
        mint
    } else {
        debug!("hpl_token program filters do not match by-mint index requisites");
        None
    }
}
...
fn get_token_program_id_and_mint(
    ...
    match token_account_filter {
        TokenAccountsFilter::Mint(mint) => {
            let (mint_owner, _, _, _, _) = get_mint_owner_and_decimals(bank, &mint)?;
            if !is_known_hpl_token_id(&mint_owner) {
                return Err(Error::invalid_params(
                    "Invalid param: not a Token mint".to_string(),
                ));
...
/// Analyze a passed Pubkey that may be a Token program id or Mint address to determine the program
/// id and optional Mint
fn get_nft_program_id_and_mint(
    bank: &Arc<Bank>,
    token_account_filter: TokenAccountsFilter,
) -> Result<(Pubkey, Option<Pubkey>)> {
    match token_account_filter {
        TokenAccountsFilter::Mint(mint) => {
            let mint_program_owner = get_nft_mint_owner(bank, &mint)?;
            if !is_known_nft_token_id(&mint_program_owner) {
                return Err(Error::invalid_params(
                    "Invalid param: not a Token mint".to_string(),
                ));
            }
            Ok((mint_program_owner, Some(mint)))
        }
        TokenAccountsFilter::ProgramId(program_id) => {
            if is_known_nft_token_id(&program_id) {
                Ok((program_id, None))
            } else {
                Err(Error::invalid_params(
                    "Invalid param: unrecognized Token program id".to_string(),
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `huione-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = huione_version::Version::default();
            Ok(RpcVersionInfo {
                huione_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        huione_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == huione_sdk::slot_history::Check::Found {
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // HPL Token-specific RPC endpoints
        // See https://github.com/huione-labs/huione-program-library/releases/tag/token-v2.0.0 for
        // program details
        ...
        #[rpc(meta, name = "getNftAccountsByOwner")]
        fn get_nft_accounts_by_owner(
            &self,
            meta: Self::Metadata,
            owner_str: String,
            token_account_filter: RpcTokenAccountsFilter,
        ...
        #[rpc(meta, name = "getDomainAccountsByOwner")]
        fn get_domain_accounts_by_owner(
            &self,
            meta: Self::Metadata,
            owner_str: String,
            config: Option<RpcAccountInfoConfig>,
        ...
        #[rpc(meta, name = "getDomainResolveAccount")]
        fn get_domain_resolve_account(
            &self,
            meta: Self::Metadata,
            domain_pubkey_str: String,
            domain_resolve_pubkey_str: String,
            config: Option<RpcAccountInfoConfig>,
        ) -> Result<RpcResponse<Option<UiAccount>>>;

        #[rpc(meta, name = "getDomainAccount")]
        fn get_domain_account(
            &self,
            meta: Self::Metadata,
            domain_pubkey_str: String,
            config: Option<RpcAccountInfoConfig>,
        ...
        #[rpc(meta, name = "getDomainAddressAccount")]
        fn get_domain_address_account(
            &self,
            meta: Self::Metadata,
            address_pubkey_str: String,
            config: Option<RpcAccountInfoConfig>,
        ) -> Result<RpcResponse<Option<UiAccount>>>;
    }
    ...
    impl AccountsData for AccountsDataImpl {
        ...
        fn get_domain_resolve_account(
            ...
            debug!(
                "GetDomainResolveAccount rpc request received: {:?}",
                domain_pubkey_str
            );
            let domain_pubkey = verify_pubkey(&domain_pubkey_str)?;
            let domain_resolve_pubkey = verify_pubkey(&domain_resolve_pubkey_str)?;
            meta.get_domain_resolve_account(&domain_resolve_pubkey, &domain_pubkey, config)
        }
        ...
        fn get_domain_account(
            ...
            debug!(
                "getDomainAccount rpc request received: {:?}",
                domain_pubkey_str
            );
            let domain_pubkey = verify_pubkey(&domain_pubkey_str)?;
            meta.get_domain_account(&domain_pubkey, config)
        }
        ...
        fn get_domain_address_account(
            ...
            debug!(
                "getDomainAddressAccount rpc request received: {:?}",
                address_pubkey_str
            );
            let domain_address_pubkey = verify_pubkey(&address_pubkey_str)?;
            meta.get_domain_address_account(&domain_address_pubkey, config)
        }
        ...
        fn get_nft_accounts_by_owner(
            ...
            meta.get_nft_accounts_by_owner(&owner, token_account_filter, config)
        }
        ...
        fn get_domain_accounts_by_owner(
            ...
            debug!(
                "get_domain_accounts_by_owner rpc request received: {:?}",
                owner_str
            );
            let owner = verify_pubkey(&owner_str)?;
            meta.get_domain_accounts_by_owner(&owner, config)
        }
    }
}
...
pub mod rpc_full {
    use {
        super::*,
        huione_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
    };
    #[rpc]
    pub trait Full {
        type Metadata;
        ...
        fn get_fee_for_message(
            ...
            data: String,
            config: Option<RpcContextConfig>,
        ) -> Result<RpcResponse<Option<u128>>>;
    ...
    impl Full for FullImpl {
        ...
        fn get_cluster_nodes(&self, meta: Self::Metadata) -> Result<Vec<RpcContactInfo>> {
            ...
            let url_address_or_none = |url: &String| -> Option<String> {
                if url != "" {
                    Some(url.clone())
                } else {
                    None
            ...
            Ok(cluster_info
                ...
                .filter_map(|(contact_info, _)| {
                            ...
                            .unwrap_or_default()
                    {
                        ...
                        let mut rpc_url = url_address_or_none(&contact_info.rpc_url);
                        if rpc_url == None {
                            rpc_url = contact_info.rpc().ok()
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = huione_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = huione_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = huione_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = huione_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = huione_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        huione_ledger::blockstore_processor::process_entries_for_tests(
            &bank,
            entries,
            true,
            Some(
                &huione_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        hpl_token::{
            huione_program::{program_option::COption, pubkey::Pubkey as HplTokenPubkey},
            state::{AccountState as TokenAccountState, Mint},
        },
        serde::de::DeserializeOwned,
        huione_address_lookup_table_program::state::{AddressLookupTable, LookupTableMeta},
        huione_entry::entry::next_versioned_entry,
        huione_gossip::socketaddr,
        huione_ledger::{
    ...
    fn hpl_token_id() -> Pubkey {
        huione_account_decoder::parse_token::hpl_token_ids()[0]
    }

    const TEST_MINT_LAMPORTS: u128 = 1_000_000;
    const TEST_SIGNATURE_FEE: u128 = 5_000;
    ...
    impl RpcHandler {
        ...
        fn store_address_lookup_table(&self) -> Pubkey {
            ...
            let address_table_account = {
                ...
                AccountSharedData::create(
                    ...
                    address_table_data,
                    huione_address_lookup_table_program::id(),
                    false,
                    0,
                )
            };
        ...
        fn store_vote_account(&self, vote_pubkey: &Pubkey, vote_state: VoteState) {
            ...
            let mut vote_account =
                AccountSharedData::new(balance, space, &huione_vote_program::id());
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = huione_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_balance_via_client() {
        ...
        async fn use_client(client: rpc_minimal::gen_client::Client, mint_pubkey: Pubkey) -> u128 {
            client
    ...
    fn test_get_supply() {
        let rpc = RpcHandler::start();
        // feature comment
        // NonCirculating include stakers
        let request = create_test_request("getSupply",Some(json!([{"excludeNonCirculatingAccountsList": true}])));
        let result = {
        ...
        let expected = {
            ...
            RpcSupply {
                non_circulating: 14703040,
                circulating: total_capitalization - 14703040,
    ...
    fn test_get_largest_accounts() {
        ...
        let non_circulating_key = &Pubkey::from_str("A6pE1ZnErnh21YSoQTqF7chVmsyJTtdYiY5Wfz4vygwW").unwrap(); 
        ...
        let mint_balance_result: RpcResponse<u128> =
            parse_success_result(rpc.handle_request_sync(request));
        ...
        let non_circulating_balance_result: RpcResponse<u128> =
            parse_success_result(rpc.handle_request_sync(request));
        assert!(largest_accounts_result.value.contains(&RpcAccountBalance {
        ...
        let largest_accounts_result: RpcResponse<Vec<RpcAccountBalance>> =
            parse_success_result(rpc.handle_request_sync(request));
        assert_eq!(largest_accounts_result.value.len(), 2);
        assert!(largest_accounts_result.value.contains(&RpcAccountBalance {
            address: non_circulating_key.to_string(),
            lamports: non_circulating_balance_result.value,
        }));
    ...
    fn test_rpc_get_minimum_balance_for_rent_exemption() {
        ...
        let result: u128 = parse_success_result(rpc.handle_request_sync(request));
        let expected = rpc
            .working_bank()
            .get_minimum_balance_for_rent_exemption(data_len);
        assert_eq!(result, expected);
    ...
    fn test_rpc_verify_pubkey() {
        let pubkey = huione_sdk::pubkey::new_rand();
    ...
    fn test_get_vote_accounts() {
        ...
        let expected_credits = TEST_SLOTS_PER_EPOCH as u128 - MAX_LOCKOUT_HISTORY as u128 - 1;
        assert_eq!(
    ...
    fn test_token_rpcs() {
        ...
        let mint = HplTokenPubkey::from([2; 32]);
        let owner = HplTokenPubkey::from([3; 32]);
        let delegate = HplTokenPubkey::from([4; 32]);
        ...
        let token_account_pubkey = huione_sdk::pubkey::new_rand();
        ...
        let other_token_account_pubkey = huione_sdk::pubkey::new_rand();
        bank.store_account(&other_token_account_pubkey, &token_account);

        // Add another token account with the same owner and delegate but different mint
        let mut account_data = vec![0; TokenAccount::get_packed_len()];
        let new_mint = HplTokenPubkey::from([5; 32]);
        let token_account = TokenAccount {
            mint: new_mint,
            owner,
            delegate: COption::Some(delegate),
        ...
        let token_with_different_mint_pubkey = huione_sdk::pubkey::new_rand();
        ...
        let token_with_smaller_balance = huione_sdk::pubkey::new_rand();
    ...
    #[test]
    fn test_get_hpl_token_owner_filter() {
        let owner = Pubkey::new_unique();
        assert_eq!(
            get_hpl_token_owner_filter(
                &Pubkey::from_str("HuiToken11111111111111111111111111111111111").unwrap(),
                &[
                    RpcFilterType::Memcmp(Memcmp::new_raw_bytes(32, owner.to_bytes().to_vec())),
                    RpcFilterType::Memcmp(Memcmp::new_raw_bytes(165, vec![2])),
                ],
            )
            .unwrap(),
            owner
        ...
        assert!(get_hpl_token_owner_filter(
            &Pubkey::from_str("HuiToken11111111111111111111111111111111111").unwrap(),
            &[
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded huione_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded huione_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded huione_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize huione_sdk::transaction::Transaction: invalid value: \
    ...
    fn test_rpc_get_stake_minimum_delegation() {
        ...
        let response: RpcResponse<u128> = parse_success_result(rpc.handle_request_sync(request));
