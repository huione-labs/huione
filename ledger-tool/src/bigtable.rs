...
use {
    ...
    huione_storage_bigtable::CredentialType,
...
async fn upload(
    ...
    force_reupload: bool,
    config: huione_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = huione_storage_bigtable::LedgerStorage::new_with_config(config)
        .await
    ...
    while starting_slot <= ending_slot {
        ...
        let last_slot_checked = huione_ledger::bigtable_upload::upload_confirmed_blocks(
            blockstore.clone(),
            bigtable.clone(),
            starting_slot,
            current_ending_slot,
...
async fn delete_slots(
    ...
    huione_ledger::bigtable_delete::delete_confirmed_blocks(bigtable, slots, dry_run).await
}

async fn first_available_block(
    config: huione_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = huione_storage_bigtable::LedgerStorage::new_with_config(config).await?;
...
async fn block(
    ...
    println!("{}", outhuione_format.formatted_string(&cli_block));
    Ok(())
}
...
async fn compare_blocks(
    ...
    limit: usize,
    config: huione_storage_bigtable::LedgerStorageConfig,
    ref_config: huione_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let owned_bigtable = huione_storage_bigtable::LedgerStorage::new_with_config(config)
        .await
    ...
    let reference_bigtable = huione_storage_bigtable::LedgerStorage::new_with_config(ref_config)
        .await
...
async fn confirm(
    ...
    println!("{}", outhuione_format.formatted_string(&cli_transaction));
    Ok(())
}
                ...
                while let Ok(slot) = r.try_recv() {
                    ...
                    if args.dry_run {
                        ...
                        match source_bigtable_clone.get_confirmed_block(slot).await {
                            ...
                            Err(huione_storage_bigtable::Error::BlockNotFound(slot)) => {
                                debug!("block not found, slot: {}", slot);
                                block_not_found_slots_clone.lock().unwrap().push(slot);
                                continue;
                            }
...
) -> huione_storage_bigtable::Result<huione_storage_bigtable::LedgerStorage> {
    if let Some(endpoint) = args.emulated_source {
        huione_storage_bigtable::LedgerStorage::new_for_emulator(
            &args.instance_name,
            &args.app_profile_id,
            &endpoint,
            args.timeout,
        ...
        huione_storage_bigtable::LedgerStorage::new_with_config(
            huione_storage_bigtable::LedgerStorageConfig {
            ...
            SubCommand::with_name("bigtable")
                ...
                .arg(
                    Arg::with_name("rpc_bigtable_instance_name")
                        ...
                        .default_value(huione_storage_bigtable::DEFAULT_INSTANCE_NAME)
...
pub fn bigtable_process_command(ledger_path: &Path, matches: &ArgMatches<'_>) {
    ...
    let outhuione_format = OutputFormat::from_matches(matches, "outhuione_format", verbose);
    ...
    let instance_name = get_global_subcommand_arg(
        ...
        huione_storage_bigtable::DEFAULT_INSTANCE_NAME,
    );
    let app_profile_id = get_global_subcommand_arg(
        matches,
        sub_matches,
        "rpc_bigtable_app_profile_id",
        huione_storage_bigtable::DEFAULT_APP_PROFILE_ID,
    );
    ...
    let future = match (subcommand, sub_matches) {
        ("upload", Some(arg_matches)) => {
            ...
            let config = huione_storage_bigtable::LedgerStorageConfig {
                read_only: false,
                instance_name,
                app_profile_id,
                ..huione_storage_bigtable::LedgerStorageConfig::default()
            };
        ...
        ("block", Some(arg_matches)) => {
            ...
            runtime.block_on(block(slot, outhuione_format, config))
        }
        ...
        ("compare-blocks", Some(arg_matches)) => {
            ...
            let ref_config = huione_storage_bigtable::LedgerStorageConfig {
                read_only: false,
        ...
        ("confirm", Some(arg_matches)) => {
            ...
            runtime.block_on(confirm(&signature, verbose, outhuione_format, config))
        }
