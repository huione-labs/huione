...
use {
    crate::{args::*, bigtable::*, ledger_path::*, ledger_utils::*, output::*, program::*, hbase::*},
    chrono::{DateTime, Utc},
    clap::{
    ...
    serde_json::json,
    huione_account_decoder::{UiAccount, UiAccountData, UiAccountEncoding},
    huione_clap_utils::{
    ...
    huione_cli_output::{CliAccount, CliAccountNewConfig, OutputFormat},
    huione_core::{
    ...
    huione_measure::{measure, measure::Measure},
    huione_runtime::{
        accounts::Accounts,
    ...
    huione_sdk::{
        ...
        hash::Hash,
        inflation::Inflation,
        native_token::{lamports_to_hc, hc_to_lamports, HC},
        pubkey::Pubkey,
        rent::Rent,
    ...
    huione_stake_program::stake_state::{self, PointValue},
    huione_vote_program::{
        self,
...
fn outhuione_slot_rewards(blockstore: &Blockstore, slot: Slot, method: &LedgerOutputMethod) {
            ...
            if !rewards.is_empty() {
                ...
                for reward in rewards {
                    ...
                    println!(
                        ...
                        sign,
                        lamports_to_hc(reward.lamports.unsigned_abs() as u128),
                        lamports_to_hc(reward.post_balance),
                        reward
                            .commission
    ...
    match method {
        LedgerOutputMethod::Print => {
            ...
            for (transactions_index, transaction) in entry.transactions.into_iter().enumerate() {
                ...
                huione_cli_output::display::println_transaction(
                    &transaction,
...
fn outhuione_slot(
    ...
    if verbose_level >= 2 {
        for (entry_index, entry) in entries.into_iter().enumerate() {
            outhuione_entry(blockstore, method, slot, entry_index, entry);
        }

        outhuione_slot_rewards(blockstore, slot, method);
        ...
        println!("  Programs:");
        outhuione_sorted_program_ids(program_ids);
    }
    Ok(())
}
...
fn outhuione_ledger(
    ...
    if method == LedgerOutputMethod::Json {
        ...
        outhuione_sorted_program_ids(all_program_ids);
    }
}

fn outhuione_sorted_program_ids(program_ids: HashMap<Pubkey, u64>) {
...
fn outhuione_account(
    ...
    println!("{pubkey}:");
    println!("  balance: {} SOL", lamports_to_hc(account.lamports()));
...
fn render_dot(dot: String, outhuione_file: &str, outhuione_format: &str) -> io::Result<()> {
    let mut child = Command::new("dot")
        .arg(format!("-T{outhuione_format}"))
        .arg(format!("-o{outhuione_file}"))
        .stdin(Stdio::piped())
        .spawn()
        .map_err(|err| {
...
fn analyze_column<
    C: huione_ledger::blockstore_db::Column + huione_ledger::blockstore_db::ColumnName,
>(
    db: &Database,
    name: &str,
) {
...
fn slot_contains_nonvote_tx(blockstore: &Blockstore, slot: Slot) -> bool {
    ...
    let contains_nonvote = entries
        ...
        .any(|program_id| *program_id != huione_vote_program::id());
    contains_nonvote
}
...
fn main() {
    ...
    let default_bootstrap_validator_lamports = &hc_to_lamports("500.0")
        .max(VoteState::get_rent_exempt_reserve(&rent))
        .to_string();
    let default_bootstrap_validator_stake_lamports = &hc_to_lamports("0.5")
    ...
    let matches = App::new(crate_name!())
        ...
        .version(huione_version::version!())
        ...
        .subcommand(
            ...
            .arg(
                Arg::with_name("outhuione_directory")
                    .index(1)
                    .value_name("DIR")
                    .takes_value(true)
    ...
    info!("{} {}", crate_name!(), huione_version::version!());
    ...
    if let ("bigtable", Some(arg_matches)) = matches.subcommand() {
        ...
        if let ("hbase", Some(arg_matches)) = matches.subcommand() {
            hbase_process_command(&ledger_path, arg_matches)
        } else {
            ...
            match matches.subcommand() {
                ...
                ("modify-genesis", Some(arg_matches)) => {
                    ...
                    let outhuione_directory =
                        PathBuf::from(arg_matches.value_of("outhuione_directory").unwrap());
                    ...
                    if let Some(hashes_per_tick) = arg_matches.value_of("hashes_per_tick") {
                        genesis_config.poh_config.hashes_per_tick = match hashes_per_tick {
                            // Note: Unlike `huione-genesis`, "auto" is not supported here.
                            "sleep" => None,
                    ...
                    create_new_ledger(
                        &outhuione_directory,
                        &genesis_config,
                        huione_runtime::hardened_unpack::MAX_GENESIS_ARCHIVE_UNPACKED_SIZE,
                    ...
                    println!("{}", open_genesis_config_by(&outhuione_directory, arg_matches));
                }
			...
			incremental_snapshot_archive_path,
                ...
                ("graph", Some(arg_matches)) => {
                    let outhuione_file = value_t_or_exit!(arg_matches, "graph_filename", String);
                    ...
                    match load_and_process_ledger(
                        ...
                        Ok((bank_forks, ..)) => {
                            ...
                            let extension = Path::new(&outhuione_file).extension();
                            let result = if extension == Some(OsStr::new("pdf")) {
                                render_dot(dot, &outhuione_file, "pdf")
                            } else if extension == Some(OsStr::new("png")) {
                                render_dot(dot, &outhuione_file, "png")
                            } else {
                                File::create(&outhuione_file)
                                    .and_then(|mut file| file.write_all(&dot.into_bytes()))
                            };
    
                            match result {
                                Ok(_) => println!("Wrote {outhuione_file}"),
                                Err(err) => eprintln!("Unable to write {outhuione_file}: {err}"),
                            }
                        }
                        Err(err) => {
                            eprintln!("Failed to load ledger: {err:?}");
                ...
                ("create-snapshot", Some(arg_matches)) => {
                    ...
                    let outhuione_directory = value_t!(arg_matches, "outhuione_directory", PathBuf)
                        .unwrap_or_else(|_| {
                            match (
                                is_incremental,
                                &snapshot_archive_path,
                    ...
                    let faucet_lamports = value_t!(arg_matches, "faucet_lamports", u128).unwrap_or(0);
                    ...
                    let bootstrap_validator_lamports =
                        value_t_or_exit!(arg_matches, "bootstrap_validator_lamports", u128);
                    let bootstrap_validator_stake_lamports =
                        value_t_or_exit!(arg_matches, "bootstrap_validator_stake_lamports", u128);
                ...
                ("accounts", Some(arg_matches)) => {
                    ...
                    let (summarize, mut json_serializer) =
                        match OutputFormat::from_matches(arg_matches, "outhuione_format", false) {
                    ...
                    let scan_func = |some_account_tuple: Option<(&Pubkey, AccountSharedData, Slot)>| {
                        ...
                        {
                            if !include_sysvars && huione_sdk::sysvar::is_sysvar_id(pubkey) {
                                return;
                            }
                        ...
                        Ok((bank_forks, ..)) => {
                            ...
                            if arg_matches.is_present("warp_epoch") {
                                ...
                                struct CalculationDetail {
                                    ...
                                    voter: Pubkey,
                                    voter_owner: Pubkey,
                                    current_effective_stake: u128,
                                    total_stake: u128,
                                    rent_exempt_reserve: u128,
                                    ...
                                    old_credits_observed: Option<u128>,
                                    new_credits_observed: Option<u128>,
                                    skipped_reasons: String,
                                }
                                use huione_stake_program::stake_state::InflationPointCalculationEvent;
                                ...
                                println!(
                                    ...
                                    HC(base_bank.capitalization()),
                                    HC(warped_bank.capitalization()),
                                    HC(warped_bank.capitalization() - base_bank.capitalization()),
                                    interest_per_epoch,
                                    interest_per_year,
                                );
                                ...
                                for (pubkey, warped_account) in all_accounts {
                                    ...
                                    // inflation.
                                    if huione_sdk::sysvar::is_sysvar_id(&pubkey) {
                                        continue;
                                    }
                                    ...
                                    if let Some(base_account) = base_bank.get_account(&pubkey) {
                                        ...
                                        println!(
                                            ...
                                            base_account.owner(),
                                            HC(base_account.lamports()),
                                            HC(warped_account.lamports()),
                                            HC(delta),
                                        ...
                                        if let Some(ref mut csv_writer) = csv_writer {
                                            ...
                                            struct InflationRecord {
                                                ...
                                                old_capitalization: u128,
                                                new_capitalization: u128,
                                            }
                                ...
                                if overall_delta > 0 {
                                    println!("Sum of lamports changes: {}", HC(overall_delta));
                                }
                            } else {
                                if arg_matches.is_present("recalculate_capitalization") {
                                    eprintln!(
                                ...
                                println!("Capitalization: {}", HC(bank.capitalization()));
                            }
                        }
                        Err(err) => {
                            eprintln!("Failed to load ledger: {err:?}");
                ...
                ("bounds", Some(arg_matches)) => {
                    ...
                    match blockstore.slot_meta_iterator(0) {
                        Ok(metas) => {
                            let outhuione_format =
                                OutputFormat::from_matches(arg_matches, "outhuione_format", false);
                            ...
                            println!("{}", outhuione_format.formatted_string(&slot_bounds));
                        }
                        Err(err) => {
                            eprintln!("Unable to read the Ledger: {err:?}");
                            exit(1);
...
pub mod tests {
    use {
        super::*,
        huione_ledger::{blockstore::make_many_slot_entries, get_tmp_ledger_path_auto_delete},
    };
