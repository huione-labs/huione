...
use {
    ...
    huione_cli_config::Config,
    huione_rpc_client::rpc_client::RpcClient,
    huione_rpc_client_api::client_error::Error as ClientError,
    huione_sdk::{
        message::Message,
...
fn get_balance_at(client: &RpcClient, pubkey: &Pubkey, i: usize) -> Result<u128, ClientError> {
...
) -> Result<Vec<(Pubkey, u128)>, ClientError> {
    addresses
        .into_iter()
...
fn process_new_stake_account(
    ...
    let lockup = Lockup{
        unix_timestamp:args.lockup_date.unwrap_or_default(),
        epoch:args.lockup_epoch.unwrap_or_default(),
        custodian:args.lockup_custodian.unwrap_or(Pubkey::default()),
    };
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match resolve_command(&command_args.command)? {
        ...
        Command::Balance(args) => {
            ...
            let balances = get_balances(&client, addresses)?;
            let lamports: u128 = balances.into_iter().map(|(_, bal)| bal).sum();
            let hc = lamports_to_hc(lamports);
            println!("{hc} HC");
        }
