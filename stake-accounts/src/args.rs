use {
    clap::ArgMatches,
    huione_clap_utils::keypair::{pubkey_from_path, signer_from_path},
...
fn resolve_lock_custodian_pubkey(
    wallet_manager: &mut Option<Arc<RemoteWalletManager>>,
    key_url: &Option<String>,
) -> Result<Option<Pubkey>, Box<dyn Error>> {
    ...
    let pubkey = match key_url {
        None => None,
        Some(key_url) => {
            let pubkey = pubkey_from_path(&matches, key_url, "lock custodian pubkey", wallet_manager)?;
            Some(pubkey)
        }
    };
    Ok(pubkey)
    ...
    match command {
        Command::New(args) => {
            let resolved_args = NewArgs {
                ...
                lockup_date:args.lockup_date,
                lockup_epoch:args.lockup_epoch,
                lockup_custodian: resolve_lock_custodian_pubkey(&mut wallet_manager,&args.lockup_custodian)?,
                index: args.index,
            };
