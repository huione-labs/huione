...
fn full_battery_tests(
    ...
    request_and_confirm_airdrop(
        ...
        hc_to_lamports(&2000.0.to_string()),
    )
    .unwrap();
    check_balance!(
        hc_to_lamports(&2000.0.to_string()),
    ...
    config_payer.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(hc_to_lamports("1000.0")),
    ...
    check_balance!(hc_to_lamports("1000.0"), &rpc_client, &nonce_account);
    ...
    let payee_pubkey = huione_sdk::pubkey::new_rand();
    ...
    check_balance!(hc_to_lamports("900.0"), &rpc_client, &nonce_account);
    check_balance!(hc_to_lamports("100.0"), &rpc_client, &payee_pubkey);
    ...
    check_balance!(hc_to_lamports("800.0"), &rpc_client, &nonce_account);
    check_balance!(hc_to_lamports("200.0"), &rpc_client, &payee_pubkey);
}
...
fn test_create_account_with_seed() {
    ...
    // const ONE_SIG_FEE: f64 = 0.000000001;
    // const ONE_SIG_FEE: f64 =  0.00005;
    huione_logger::setup();
    let mint_keypair = Keypair::new();
    let mint_pubkey = mint_keypair.pubkey();
    let faucet_addr = run_local_faucet(mint_keypair, None);
    ...
    creator_config.command = CliCommand::CreateNonceAccount {
        ...
        memo: None,
        amount: SpendAmount::Some(hc_to_lamports("241.0")),
        compute_unit_price: None,
    };
    process_command(&creator_config).unwrap();
    check_balance!(hc_to_lamports("241.0"), &rpc_client, &nonce_address);
    check_balance!(
        hc_to_lamports("42.0"),
        &rpc_client,
        &offline_nonce_authority_signer.pubkey(),
    ...
    check_balance!(
        hc_to_lamports(&(4001.0 - ONE_SIG_FEE).to_string()),
        &rpc_client,
        &online_nonce_creator_signer.pubkey(),
    );
    check_balance!(0, &rpc_client, &to_address);
    ...
    authority_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(hc_to_lamports("10.0")),
        to: to_address,
        from: 0,
        sign_only: true,
    ...
    authority_config.outhuione_format = OutputFormat::JsonCompact;
    ...
    // 2
    check_balance!(
        hc_to_lamports(&(32.0 - ONE_SIG_FEE).to_string()),
        &rpc_client,
        &offline_nonce_authority_signer.pubkey(),
    );
    // 3
    ...
    check_balance!(hc_to_lamports("10.0"), &rpc_client, &to_address);
}
