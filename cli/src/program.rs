use {
    ...
    log::*,
    huione_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    ...
    huione_program_runtime::{compute_budget::ComputeBudget, invoke_context::InvokeContext},
    huione_rbpf::{
        elf::Executable,
            ...
            SubCommand::with_name("program")
                    ...
                    SubCommand::with_name("deploy")
                        ...
                        .arg(
                            Arg::with_name("allow_excessive_balance")
                                ...
                                .takes_value(false)
                                .help("Use the designated program id even if the account already holds a large balance of HC")
                        ),
                )
                .subcommand(
                ...
                .subcommand(
                    SubCommand::with_name("dump")
                        ...
                        .arg(
                            Arg::with_name("outhuione_location")
                                .index(2)
    ...
    let response = match (subcommand, sub_matches) {
        ...
        ("dump", Some(matches)) => CliCommandInfo {
            command: CliCommand::Program(ProgramCliCommand::Dump {
                account_pubkey: pubkey_of(matches, "account"),
                outhuione_location: matches.value_of("outhuione_location").unwrap().to_string(),
            }),
            signers: vec![],
        },
        ("close", Some(matches)) => {
...
pub fn process_program_subcommand(
    ...
    match program_subcommand {
        ...
        ProgramCliCommand::Dump {
            account_pubkey,
            outhuione_location,
        } => process_dump(&rpc_client, config, *account_pubkey, outhuione_location),
...
fn process_set_authority(
    ...
    Ok(config.outhuione_format.formatted_string(&authority))
}
    ...
    if let Some(account_pubkey) = account_pubkey {
            ...
            .value
        {
            if account.owner == bpf_loader::id() || account.owner == bpf_loader_deprecated::id() {
                Ok(config.outhuione_format.formatted_string(&CliProgram {
        ...
        Ok(config.outhuione_format.formatted_string(&programs))
    } else if buffers {
        let authority_pubkey = if all { None } else { Some(authority_pubkey) };
        let buffers = get_buffers(rpc_client, authority_pubkey, use_lamports_unit)?;
        Ok(config.outhuione_format.formatted_string(&buffers))
    } else {
...
fn process_dump(
    ...
    if let Some(account_pubkey) = account_pubkey {
        ...
        {
            if account.owner == bpf_loader::id() || account.owner == bpf_loader_deprecated::id() {
                let mut f = File::create(outhuione_location)?;
                f.write_all(&account.data)?;
                Ok(format!("Wrote program to {outhuione_location}"))
            } else if account.owner == bpf_loader_upgradeable::id() {
                if let Ok(UpgradeableLoaderState::Program {
                    programdata_address,
                }) = account.state()
                        ...
                        .value
                    {
                        ...
                        }) = account.state()
                        {
                            if authority_pubkey != Some(authority_signer.pubkey()) {
                                ...
                                Ok(config.outhuione_format.formatted_string(
...
fn do_process_program_write_and_deploy(
    ...
    if let Some(program_signers) = program_signers {
        ...
        Ok(config.outhuione_format.formatted_string(&program_id))
    } else {
        let buffer = CliProgramBuffer {
            buffer: buffer_pubkey.to_string(),
        };
        Ok(config.outhuione_format.formatted_string(&buffer))
    }
}
...
fn complete_partial_program_init(
    ...
    minimum_balance: u128,
    allow_excessive_balance: bool,
) -> Result<(Vec<Instruction>, u128), Box<dyn std::error::Error>> {
...
fn check_payer(
    ...
    if !write_messages.is_empty() {
        ...
        if let Some(message) = write_messages.get(0) {
            fee += rpc_client.get_fee_for_message(message)? * (write_messages.len() as u128);
        }
    }
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("`huione-keygen recover` and the following {words}-word seed phrase:");
    eprintln!("{divider}\n{phrase}\n{divider}");
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `huione program deploy` or `huione write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!("[BUFFER_ACCOUNT_ADDRESS] argument to `huione program close`.\n{divider}");
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        huione_cli_output::OutputFormat,
        huione_sdk::signature::write_keypair_file,
    };
    ...
    fn test_cli_keypair_file() {
        ...
        let config = CliConfig {
            ...
            outhuione_format: OutputFormat::JsonCompact,
            ..CliConfig::default()
        };
