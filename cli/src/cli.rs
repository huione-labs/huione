use {
    ...
    huione_clap_utils::{self, input_parsers::*, keypair::*},
    huione_cli_config::ConfigInput,
    huione_cli_output::{
        display::println_name_value, CliSignature, CliValidatorsSortOrder, OutputFormat,
    },
    huione_remote_wallet::remote_wallet::RemoteWalletManager,
    ...
    huione_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    huione_sdk::{
        clock::{Epoch, Slot},
    ...
    huione_vote_program::vote_state::VoteAuthorize,
...
pub enum CliCommand {
    ...
    Ping {
        ...
        compute_unit_price: Option<u128>,
    },
    Rent {
        data_length: usize,
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} HC)")]
    InsufficientFundsForFee(String, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} HC)")]
    InsufficientFundsForSpend(String, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} HC) + fee ({1} HC)")]
    InsufficientFundsForSpendAndFee(String, String, Pubkey),
    #[error(transparent)]
    InvalidNonce(huione_rpc_client_nonce_utils::Error),
...
impl From<huione_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: huione_rpc_client_nonce_utils::Error) -> Self {
        match error {
            huione_rpc_client_nonce_utils::Error::Client(client_error) => {
...
pub struct CliConfig<'a> {
    ...
    pub verbose: bool,
    pub outhuione_format: OutputFormat,
...
impl Default for CliConfig<'_> {
    fn default() -> CliConfig<'static> {
        CliConfig {
            ...
            verbose: false,
            outhuione_format: OutputFormat::Display,
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                huione_version::version!(),
            )
            .gen_completions_to("huione", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`huione deploy` has been replaced with `huione program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    if config.verbose && config.outhuione_format == OutputFormat::DisplayVerbose {
    ...
    match &config.command {
        ...
        // Return software version of huione-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Request an airdrop from HC Faucet;
        ...
        CliCommand::ShowAccount {
            ...
            outhuione_file,
            use_lamports_unit,
        } => process_show_account(&rpc_client, config, pubkey, outhuione_file, *use_lamports_unit),
        CliCommand::Transfer {
            amount,
            to,
            from,
    ...
    F: Fn(&InstructionError) -> Option<E>,
{
    match result {
        ...
        Ok(sig) => {
            ...
            Ok(config.outhuione_format.formatted_string(&signature))
        }
    }
}
...
mod tests {
    use {
        super::*,
        serde_json::json,
        huione_rpc_client::mock_sender_for_cli::SIGNATURE,
        huione_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        huione_rpc_client_nonce_utils::blockhash_query,
        huione_sdk::{
            pubkey::Pubkey,
            signature::{
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(huione_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(huione_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", huione_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 HC");
        ...
        let new_authorized_pubkey = huione_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = huione_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = huione_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = huione_sdk::pubkey::new_rand();
