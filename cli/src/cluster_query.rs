use {
    ...
    huione_pubsub_client::pubsub_client::PubsubClient,
    huione_remote_wallet::remote_wallet::RemoteWalletManager,
    huione_rpc_client::rpc_client::{GetConfirmedSignaturesForAddress2Config, RpcClient},
    ...
    huione_vote_program::vote_state::VoteState,
    std::{
...
impl ClusterQuerySubCommands for App<'_, '_> {
    fn cluster_query_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("supply").about("Get information about the cluster supply of HC")
            .arg(
        ...
        .subcommand(
            SubCommand::with_name("total-supply").about("Get total number of HC")
            ...
            SubCommand::with_name("validators")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display balance in lamports instead of HC"),
                )
                .arg(
            ...
            SubCommand::with_name("rent")
                ...
                .arg(
                    Arg::with_name("lamports")
                        .long("lamports")
                        .takes_value(false)
                        .help("Display rent in lamports instead of HC"),
                ),
        )
    }
}
...
pub fn process_catchup(
    ...
    let (node_client, node_pubkey) = if our_localhost_port.is_some() {
        ...
        if let Some(node_json_rpc_url) = node_json_rpc_url {
            ...
            let rpc_addr = loop {
                ...
                {
                    if let Some(rpc_addr) = contact_info.rpc.clone() {
                        break rpc_addr;
                    }
            ...
            match is_url(rpc_addr.clone()) {
                Ok(_) => (RpcClient::new(rpc_addr), node_pubkey),
                Err(_) => (
                    RpcClient::new_socket(rpc_addr.parse().unwrap()),
                    node_pubkey,
                ),
            }
        }
    ...
    loop {
        ...
        // especially if pointing to api.meinnet-beta.huione.com at times
...
pub fn process_cluster_date(rpc_client: &RpcClient, config: &CliConfig) -> ProcessResult {
    ...
    if let Some(clock_account) = result.value {
        ...
        Ok(config.outhuione_format.formatted_string(&block_time))
    } else {
...
pub fn process_fees(
    ...
    Ok(config.outhuione_format.formatted_string(&fees))
}
...
pub fn process_leader_schedule(
    ...
    Ok(config.outhuione_format.formatted_string(&CliLeaderSchedule {
        epoch,
...
pub fn process_get_block(
    ...
    Ok(config.outhuione_format.formatted_string(&cli_block))
}
...
pub fn process_get_epoch_info(rpc_client: &RpcClient, config: &CliConfig) -> ProcessResult {
    ...
    Ok(config.outhuione_format.formatted_string(&cli_epoch_info))
}
...
pub fn process_show_block_production(
    ...
    Ok(config.outhuione_format.formatted_string(&block_production))
}
...
pub fn process_largest_accounts(
    ...
    Ok(config.outhuione_format.formatted_string(&largest_accounts))
}
...
pub fn process_ping(
    ...
    Ok(config.outhuione_format.formatted_string(&cli_ping))
}
...
pub fn process_show_stakes(
    ...
    let mut program_accounts_config = RpcProgramAccountsConfig {
        account_config: RpcAccountInfoConfig {
            encoding: Some(huione_account_decoder::UiAccountEncoding::Base64),
...
pub fn process_show_validators(
    ...
    Ok(config.outhuione_format.formatted_string(&cli_validators))
}
...
struct CliRentCalculation {
    ...
    pub lamports_per_byte_year: u128,
    pub lamports_per_epoch: u128,
    pub rent_exempt_minimum_lamports: u128,
    #[serde(skip)]
    pub use_lamports_unit: bool,
}

impl CliRentCalculation {
    fn build_balance_message(&self, lamports: u128) -> String {
...
pub fn process_calculate_rent(
    ...
    Ok(config.outhuione_format.formatted_string(&cli_rent_calculation))
}
...
mod tests {
    use {
        ...
        huione_sdk::signature::{write_keypair, Keypair},
        std::str::FromStr,
