use {
    ...
    huione_rpc_client_api::config::RpcTransactionConfig,
    ...
    fn wallet_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("airdrop")
                .about("Request HC from a faucet")
                .arg(
                    Arg::with_name("amount")
                        ...
                        .validator(is_amount)
                        .required(true)
                        .help("The airdrop amount to request, in HC"),
                )
                .arg(
            ...
            SubCommand::with_name("transfer")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .required(true)
                        .help("The amount to send, in HC; accepts keyword ALL"),
                )
                .arg(
                    pubkey!(Arg::with_name("from")
                        .long("from")
...
fn resolve_derived_address_program_id(matches: &ArgMatches<'_>, arg_name: &str) -> Option<Pubkey> {
    matches.value_of(arg_name).and_then(|v| {
        ...
        match upper.as_str() {
            ...
            "VOTE" => Some(huione_vote_program::id()),
...
pub fn parse_account(
    ...
    let outhuione_file = matches.value_of("outhuione_file");
    let use_lamports_unit = matches.is_present("lamports");
    Ok(CliCommandInfo {
        command: CliCommand::ShowAccount {
            pubkey: account_pubkey,
            outhuione_file: outhuione_file.map(ToString::to_string),
            use_lamports_unit,
        },
        signers: vec![],
    })
...
pub fn process_show_account(
    ...
    let mut account_string = config.outhuione_format.formatted_string(&cli_account);

    match config.outhuione_format {
        OutputFormat::Json | OutputFormat::JsonCompact => {
            if let Some(outhuione_file) = outhuione_file {
                let mut f = File::create(outhuione_file)?;
                f.write_all(account_string.as_bytes())?;
                writeln!(&mut account_string)?;
                writeln!(&mut account_string, "Wrote account to {outhuione_file}")?;
            }
        }
        OutputFormat::Display | OutputFormat::DisplayVerbose => {
            if let Some(outhuione_file) = outhuione_file {
                ...
                f.write_all(data)?;
                writeln!(&mut account_string)?;
                writeln!(&mut account_string, "Wrote account data to {outhuione_file}")?;
...
pub fn process_airdrop(
    ...
    if let Ok(signature) = result {
        ...
        if current_balance < pre_balance.saturating_add(lamports) {
            ...
            println!("Run `huione confirm -v {signature:?}` for more info");
            Ok("".to_string())
        } else {
...
pub fn process_balance(
    ...
    Ok(config.outhuione_format.formatted_string(&balance_output))
}
...
pub fn process_confirm(
    ...
    match rpc_client.get_signature_statuses_with_history(&[*signature]) {
        Ok(status) => {
            ...
            Ok(config.outhuione_format.formatted_string(&cli_transaction))
        }
...
pub fn process_decode_transaction(
    ...
    Ok(config.outhuione_format.formatted_string(&decode_transaction))
}
...
pub fn process_find_program_derived_address(
    ...
    Ok(config.outhuione_format.formatted_string(&result))
}
