use {
    ...
    huione_cli_output::{cli_version::CliVersion, QuietDisplay, VerboseDisplay},
    huione_remote_wallet::remote_wallet::RemoteWalletManager,
    huione_rpc_client::rpc_client::RpcClient,
    huione_rpc_client_api::{client_error::Error as ClientError, request::MAX_MULTIPLE_ACCOUNTS},
    huione_sdk::{
        account::Account,
        clock::Slot,
...
fn cluster_info_stats(rpc_client: &RpcClient) -> Result<ClusterInfoStats, ClientError> {
    ...
    let mut total_active_stake: u128 = vote_accounts
        .delinquent
        .iter()
...
> {
    ...
    let tool_version = huione_version::Version::default();
...
fn process_status(
    ...
    Ok(config.outhuione_format.formatted_string(&feature_set))
}
