use {
    ...
    huione_clap_utils::{self, input_parsers::*, input_validators::*, keypair::*},
    huione_cli_output::{CliAddressLookupTable, CliAddressLookupTableCreated, CliSignature},
    huione_remote_wallet::remote_wallet::RemoteWalletManager,
    huione_rpc_client::rpc_client::RpcClient,
    huione_rpc_client_api::config::RpcSendTransactionConfig,
    huione_sdk::{
...
fn process_freeze_lookup_table(
    ...
    if !huione_address_lookup_table_program::check_id(&lookup_table_account.owner) {
        return Err(format!(
    ...
    match result {
        ...
        Ok(signature) => Ok(config.outhuione_format.formatted_string(&CliSignature {
            signature: signature.to_string(),
        })),
    }
}
