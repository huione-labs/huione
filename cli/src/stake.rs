    ...
    fn stake_subcommands(self) -> Self {
            ...
            SubCommand::with_name("create-stake-account")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .required(true)
                        .help("The amount to send to the stake account, in HC; accepts keyword ALL")
                )
                .arg(
            ...
            SubCommand::with_name("split-stake")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .validator(is_amount)
                        .required(true)
                        .help("The amount to move into the new stake account, in HC")
                )
                .arg(
                    Arg::with_name("seed")
                        .long("seed")
        ...
        .subcommand(
            SubCommand::with_name("withdraw-stake")
                .about("Withdraw the unstaked HC from the stake account")
                .arg(
                    pubkey!(Arg::with_name("stake_account_pubkey")
                        .index(1)
                        .value_name("STAKE_ACCOUNT_ADDRESS")
                ...
                .arg(
                    Arg::with_name("amount")
                        ...
                        .validator(is_amount_or_all)
                        .required(true)
                        .help("The amount to withdraw from the stake account, in HC; accepts keyword ALL")
                )
                .arg(
                    Arg::with_name("seed")
                        .long("seed")
...
pub fn parse_split_stake(
    ...
    let lamports = lamports_of_hc(matches, "amount").unwrap();
    let seed = matches.value_of("seed").map(|s| s.to_string());
...
pub fn process_create_stake_account(
    ...
    if !sign_only {
        ...
        if let Some(nonce_account) = &nonce_account {
            let nonce_account = huione_rpc_client_nonce_utils::get_account_with_commitment(
                rpc_client,
                nonce_account,
                config.commitment,
            )?;
...
pub fn process_stake_set_lockup(
    ...
    if !sign_only {
        ...
        let meta = match state {
            StakeState::Stake(meta, ..) => Some(meta),
            StakeState::Initialized(meta,..) =>  Some(meta),
            _ => None,
        };

        if let Some(meta) = meta{
            if meta.lockup.custodian != Pubkey::default() {
                let slot = rpc_client.get_slot()?;
                let block_time = rpc_client.get_block_time(slot)?;
                let e = rpc_client.get_epoch_info()?.epoch;
                if  block_time  < meta.lockup.unix_timestamp || e < meta.lockup.epoch {
                    check_current_authority(&[meta.lockup.custodian], &custodian.pubkey())?;
                }else {
                    check_current_authority(&[meta.authorized.withdrawer], &custodian.pubkey())?;
                }
            }
        }else {
            return Err(CliError::RpcRequestError(format!(
...
fn u128_some_if_not_zero(n: u128) -> Option<u128> {
    if n > 0 {
        Some(n)
    } else {
        None
    ...
    match stake_state {
        StakeState::Stake(
            ...
            CliStakeState {
                ...
                active_stake: u128_some_if_not_zero(effective),
                activating_stake: u128_some_if_not_zero(activating),
                deactivating_stake: u128_some_if_not_zero(deactivating),
...
pub fn process_show_stake_account(
    ...
    match stake_account.state() {
        Ok(stake_state) => {
            ...
            Ok(config.outhuione_format.formatted_string(&state))
        }
...
pub fn process_show_stake_history(
    ...
    let limit_results = match config.outhuione_format {
    ...
    Ok(config.outhuione_format.formatted_string(&stake_history_output))
}
...
mod tests {
    ...
    fn test_parse_command() {
        ...
        let authorized = huione_sdk::pubkey::new_rand();
        ...
        let vote_account_pubkey = huione_sdk::pubkey::new_rand();
