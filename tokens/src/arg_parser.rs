use {
    crate::args::{
        Args, BalancesArgs, Command, DistributeTokensArgs, SenderStakeArgs, HplTokenArgs,
    ...
    huione_clap_utils::{
        input_parsers::{pubkey_of_signer, value_of, lamports_of_hc},
        input_validators::{is_amount, is_url_or_moniker, is_valid_pubkey, is_valid_signer},
        keypair::{pubkey_from_path, signer_from_path},
    },
    huione_cli_config::CONFIG_FILE,
    huione_remote_wallet::remote_wallet::maybe_wallet_manager,
    ...
    App::new(crate_name!())
            ...
            SubCommand::with_name("distribute-tokens")
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount to send to each recipient, in HC"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("create-stake")
                ...
                .arg(
                    Arg::with_name("unlocked_hc")
                        .default_value("1.0")
                        .long("unlocked-hc")
                        .takes_value(true)
                        .value_name("HC_AMOUNT")
                        .help("Amount of HC to huione in system account to pay for fees"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("distribute-hpl-tokens")
                .about("Distribute HPL tokens")
                .arg(
                    Arg::with_name("db_path")
                        .long("db-path")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("transfer_amount")
                        ...
                        .value_name("AMOUNT")
                        .validator(is_amount)
                        .help("The amount of HPL tokens to send to each recipient"),
                )
                .arg(
                    Arg::with_name("outhuione_path")
                        .long("output-path")
                ...
                .arg(
                    Arg::with_name("token_account_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("HPL token account to send from"),
                )
                .arg(
        ...
        .subcommand(
            SubCommand::with_name("hpl-token-balances")
                .about("Balance of HPL token associated accounts")
                .arg(
                    Arg::with_name("inhuione_csv")
                        .long("input-csv")
                        .required(true)
                ...
                .arg(
                    Arg::with_name("mint_address")
                        ...
                        .validator(is_valid_pubkey)
                        .help("HPL token mint of distribution"),
                ),
        )
        .subcommand(
...
fn parse_distribute_tokens_args(
    ...
    Ok(DistributeTokensArgs {
        inhuione_csv: value_t_or_exit!(matches, "inhuione_csv", String),
        transaction_db: value_t_or_exit!(matches, "db_path", String),
        outhuione_path: matches.value_of("outhuione_path").map(|path| path.to_string()),
        dry_run: matches.is_present("dry_run"),
        sender_keypair,
        fee_payer,
        stake_args: None,
        hpl_token_args: None,
        transfer_amount: lamports_of_hc(matches, "transfer_amount"),
    })
}
...
fn parse_create_stake_args(
    ...
    let stake_args = StakeArgs {
        unlocked_hc: lamports_of_hc(matches, "unlocked_hc").unwrap(),
        lockup_authority,
...
fn parse_distribute_hpl_tokens_args(
    matches: &ArgMatches<'_>,
) -> Result<DistributeTokensArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let signer_matches = ArgMatches::default(); // No default signer
    ...
    Ok(DistributeTokensArgs {
        ...
        fee_payer,
        stake_args: None,
        hpl_token_args: Some(HplTokenArgs {
            token_account_address,
            ..HplTokenArgs::default()
        }),
...
fn parse_balances_args(matches: &ArgMatches<'_>) -> Result<BalancesArgs, Box<dyn Error>> {
    let mut wallet_manager = maybe_wallet_manager()?;
    let hpl_token_args =
        pubkey_of_signer(matches, "mint_address", &mut wallet_manager)?.map(|mint| HplTokenArgs {
            mint,
            ..HplTokenArgs::default()
        });
    Ok(BalancesArgs {
...
fn parse_transaction_log_args(matches: &ArgMatches<'_>) -> TransactionLogArgs {
    TransactionLogArgs {
        transaction_db: value_t_or_exit!(matches, "db_path", String),
        outhuione_path: value_t_or_exit!(matches, "outhuione_path", String),
    }
}
    ...
    T: Into<OsString> + Clone,
{
    ...
    let command = match matches.subcommand() {
        ...
        ("distribute-hpl-tokens", Some(matches)) => {
            Command::DistributeTokens(parse_distribute_hpl_tokens_args(matches)?)
        }
        ("balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
        ("hpl-token-balances", Some(matches)) => Command::Balances(parse_balances_args(matches)?),
