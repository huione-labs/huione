use {
    ...
    huione_account_decoder::parse_token::real_number_string,
    ...
    huione_sdk::{
        ...
        message::Message,
        native_token::{lamports_to_hc, hc_to_lamports},
    ...
    huione_transaction_status::TransactionStatus,
    hpl_associated_token_account::get_associated_token_address,
    hpl_token::huione_program::program_error::ProgramError,
    std::{
        cmp::{self},
        io,
        sync::{
...
fn distribution_instructions(
    ...
    if args.hpl_token_args.is_some() {
        return build_hpl_token_instructions(allocation, args, do_create_associated_token_account);
    }
    ...
    match &args.stake_args {
        ...
        Some(stake_args) => {
            let unlocked_hc = stake_args.unlocked_hc;
...
fn build_messages(
    ...
    if let Some(spl_token_args) = &args.hpl_token_args {
    ...
    for (i, allocation) in allocations.iter().enumerate() {
        ...
        let do_create_associated_token_account = if let Some(hpl_token_args) = &args.hpl_token_args
        {
            ...
            println!(
                ...
                real_number_string(allocation.amount as u128, hpl_token_args.decimals)
            );
            ...
            println!(
                "{:<44}  {:>24.9}",
                allocation.recipient,
                lamports_to_hc(allocation.amount)
            );
            false
        };
...
fn distribute_allocations(
    ...
    if args.hpl_token_args.is_some() {
        check_hpl_token_balances(&messages, allocations, client, args, created_accounts)?;
    } else {
...
fn read_allocations(
    inhuione_csv: &str,
    transfer_amount: Option<u128>,
    require_lockup_heading: bool,
    raw_amount: bool,
) -> io::Result<Vec<Allocation>> {
    let mut rdr = ReaderBuilder::new().trim(Trim::All).from_path(inhuione_csv)?;
    let allocations = if let Some(amount) = transfer_amount {
        ...
        recipients
            ...
            .map(|(recipient, amount, lockup_date)| Allocation {
                recipient,
                amount: hc_to_lamports(amount.to_string().as_ref()),
                lockup_date,
            })
            .collect()
    } else if raw_amount {
        let recipients: Vec<(String, u128)> = rdr
            .deserialize()
            .map(|recipient| recipient.unwrap())
            .collect();
        recipients
        ...
        recipients
            ...
            .map(|(recipient, amount)| Allocation {
                recipient,
                amount: hc_to_lamports(&amount.to_string()),
                lockup_date: "".to_string(),
            })
            .collect()
    };
...
pub fn process_allocations(
    ...
    let mut allocations: Vec<Allocation> = read_allocations(
        ...
        args.hpl_token_args.is_some(),
    )?;

    let starting_total_tokens = allocations.iter().map(|x| x.amount).sum();
    let starting_total_tokens = if let Some(hpl_token_args) = &args.hpl_token_args {
        Token::hpl_token(starting_total_tokens, hpl_token_args.decimals)
    } else {
        Token::hc(starting_total_tokens)
    };
    println!(
        "{} {}",
        style("Total in inhuione_csv:").bold(),
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(hpl_token_args) = &args.hpl_token_args {
            (
                Token::hpl_token(distributed_tokens, hpl_token_args.decimals),
                Token::hpl_token(undistributed_tokens, hpl_token_args.decimals),
            )
        } else {
            (
                Token::hc(distributed_tokens),
                Token::hc(undistributed_tokens),
            )
        };
    ...
    if !args.dry_run {
        if let Some(outhuione_path) = &args.outhuione_path {
            db::write_transaction_log(&db, &outhuione_path)?;
        }
    }
...
pub fn get_fee_estimate_for_messages(
    ...
    let fee_estimate = fee
        .checked_mul(messages.len() as u128)
...
fn check_payer_balances(
    ...
    let mut undistributed_tokens: u128 = allocations.iter().map(|x| x.amount).sum();
    let fees = get_fee_estimate_for_messages(messages, client)?;

    let (distribution_source, unlocked_huione_source) = if let Some(stake_args) = &args.stake_args {
        let total_unlocked_hc = allocations.len() as u128 * stake_args.unlocked_hc;
        undistributed_tokens -= total_unlocked_hc;
        ...
        (
            from_pubkey,
            Some((args.sender_keypair.pubkey(), total_unlocked_hc)),
        )
    } else {
    ...
    if let Some((unlocked_huione_source, total_unlocked_hc)) = unlocked_huione_source {
        ...
        if staker_balance < undistributed_tokens {
            return Err(Error::InsufficientFunds(
                ...
                lamports_to_hc(undistributed_tokens).to_string(),
            ));
        }
        if args.fee_payer.pubkey() == unlocked_huione_source {
            if fee_payer_balance < fees + total_unlocked_hc {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                    lamports_to_hc(fees + total_unlocked_hc).to_string(),
                ));
            }
        } else {
            if fee_payer_balance < fees {
                return Err(Error::InsufficientFunds(
                    ...
                    lamports_to_hc(fees).to_string(),
                ));
            }
            let unlocked_huione_balance = client.get_balance(&unlocked_huione_source)?;
            if unlocked_huione_balance < total_unlocked_hc {
                return Err(Error::InsufficientFunds(
                    vec![FundingSource::SystemAccount].into(),
                    lamports_to_hc(total_unlocked_hc).to_string(),
                ));
            }
        }
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                lamports_to_hc(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(
    ...
    let allocations: Vec<Allocation> =
        read_allocations(&args.inhuione_csv, None, false, args.hpl_token_args.is_some())?;
    let allocations = merge_allocations(&allocations);

    let token = if let Some(hpl_token_args) = &args.hpl_token_args {
        hpl_token_args.mint.to_string()
    } else {
        "◎".to_string()
    };
    ...
    for allocation in &allocations {
        ...
        if let Some(hpl_token_args) = &args.hpl_token_args {
            print_token_balances(client, allocation, hpl_token_args)?;
        } else {
            let address: Pubkey = allocation.recipient.parse().unwrap();
            let expected = lamports_to_hc(allocation.amount);
            let actual = lamports_to_hc(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
pub fn process_transaction_log(args: &TransactionLogArgs) -> Result<(), Error> {
    ...
    db::write_transaction_log(&db, &args.outhuione_path)?;
    Ok(())
}

use {
    crate::db::check_outhuione_file,
    huione_sdk::{pubkey::Pubkey, signature::Keypair},
...
pub fn test_process_distribute_tokens_with_client(
    ...
    let expected_amount = if let Some(amount) = transfer_amount {
        ...
        hc_to_lamports("1000.0")
    };
    let alice_pubkey = huione_sdk::pubkey::new_rand();
    let allocations_file = NamedTempFile::new().unwrap();
    let inhuione_csv = allocations_file.path().to_str().unwrap().to_string();
    ...
    wtr.write_record([
        ...
        lamports_to_hc(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
    ...
    let outhuione_file = NamedTempFile::new().unwrap();
    let outhuione_path = outhuione_file.path().to_str().unwrap().to_string();
    ...
    let args = DistributeTokensArgs {
        ...
        outhuione_path: Some(outhuione_path.clone()),
        stake_args: None,
        hpl_token_args: None,
        transfer_amount,
    };
    ...
    check_outhuione_file(&outhuione_path, &db::open_db(&transaction_db, true).unwrap());
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = hc_to_lamports("1000.0");
    let alice_pubkey = huione_sdk::pubkey::new_rand();
    let file = NamedTempFile::new().unwrap();
    let inhuione_csv = file.path().to_str().unwrap().to_string();
    ...
    let stake_args = StakeArgs {
        ...
        unlocked_hc: hc_to_lamports("1.0"),
    ...
    assert_eq!(
        ...
        expected_amount - hc_to_lamports("1.0"),
    );
...
mod tests {
    use {
        ...
        huione_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(hc_to_lamports("1.5")));
    }
    ...
    fn test_read_allocations() {
        ...
        assert_eq!(
            read_allocations(&inhuione_csv, None, false, true).unwrap(),
            vec![allocation]
        );

        let allocation_hc = Allocation {
            recipient: alice_pubkey.to_string(),
            amount: hc_to_lamports("42.0"),
            lockup_date: "".to_string(),
        };

        assert_eq!(
            read_allocations(&inhuione_csv, None, true, true).unwrap(),
            vec![allocation_hc.clone()]
        );
        assert_eq!(
            read_allocations(&inhuione_csv, None, false, false).unwrap(),
            vec![allocation_hc.clone()]
        );
        assert_eq!(
            read_allocations(&inhuione_csv, None, true, false).unwrap(),
            vec![allocation_hc]
        );
    }
    ...
    fn test_read_allocations_transfer_amount() {
        ...
        assert_eq!(
            read_allocations(&inhuione_csv, Some(amount), false, false).unwrap(),
            expected_allocations
        );
    }
    ...
    fn test_apply_previous_transactions() {
        let alice = huione_sdk::pubkey::new_rand();
        let bob = huione_sdk::pubkey::new_rand();
        let mut allocations = vec![
            Allocation {
                recipient: alice.to_string(),
                amount: hc_to_lamports("1.0"),
                lockup_date: "".to_string(),
            },
            Allocation {
    ...
    fn test_set_split_stake_lockup() {
        ...
        let stake_account_address = huione_sdk::pubkey::new_rand();
        let new_stake_account_address = huione_sdk::pubkey::new_rand();
    ...
    fn test_check_payer_balances_distribute_tokens_single_payer() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (allocation_amount + fees_in_hc.parse::<f64>().unwrap()).to_string());
        } else {
        ...
        let transaction = transfer(
            &client,
            hc_to_lamports(allocation_amount.to_string().as_ref()),
            &alice,
    ...
    fn test_check_payer_balances_distribute_tokens_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, fees_in_hc.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    }
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let stake_args = initialize_stake_account(
            hc_to_lamports(allocation_amount.to_string().as_ref()),
            hc_to_lamports(unlocked_hc.to_string().as_ref()),
            &alice,
            &client,
        );
        ...
        let expensive_allocations = vec![Allocation {
            recipient: huione_sdk::pubkey::new_rand().to_string(),
            amount: hc_to_lamports(expensive_allocation_amount.to_string().as_ref()),
            lockup_date: "".to_string(),
        }];
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(
                amount,
                (expensive_allocation_amount - unlocked_hc).to_string()
            );
        } else {
            panic!("check_payer_balances should have errored");
        }
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            ...
            assert_eq!(amount, (unlocked_hc + fees_in_hc.parse::<f64>().unwrap()).to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
    ...
    fn test_check_payer_balances_distribute_stakes_separate_payers() {
        ...
        if let Error::InsufficientFunds(sources, amount) = err_result {
            assert_eq!(sources, vec![FundingSource::SystemAccount].into());
            assert_eq!(amount, unlocked_hc.to_string());
        } else {
            panic!("check_payer_balances should have errored");
        }
