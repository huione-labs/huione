use {
    crate::{
        extract_memos::hpl_memo_id_v1,
        parse_address_lookup_table::parse_address_lookup_table,
        parse_associated_token::{parse_associated_token, hpl_associated_token_id},
    ...
    inflector::Inflector,
    serde_json::Value,
    huione_account_decoder::parse_token::hpl_token_ids,
    huione_sdk::{
...
lazy_static! {
    static ref ADDRESS_LOOKUP_PROGRAM_ID: Pubkey = huione_address_lookup_table_program::id();
    static ref ASSOCIATED_TOKEN_PROGRAM_ID: Pubkey = hpl_associated_token_id();
    static ref BPF_LOADER_PROGRAM_ID: Pubkey = huione_sdk::bpf_loader::id();
    static ref BPF_UPGRADEABLE_LOADER_PROGRAM_ID: Pubkey = huione_sdk::bpf_loader_upgradeable::id();
    static ref MEMO_V1_PROGRAM_ID: Pubkey = hpl_memo_id_v1();
    // static ref MEMO_V3_PROGRAM_ID: Pubkey = hpl_memo_id_v3();
    static ref STAKE_PROGRAM_ID: Pubkey = stake::program::id();
    static ref SYSTEM_PROGRAM_ID: Pubkey = system_program::id();
    static ref VOTE_PROGRAM_ID: Pubkey = huione_vote_program::id();
    static ref NFT_PROGRAM_ID: Pubkey = hpl_nft::id();
    static ref NAME_PROGRAM_ID: Pubkey = hpl_name::id();
    static ref PARSABLE_PROGRAM_IDS: HashMap<Pubkey, ParsableProgram> = {
        ...
        m.insert(*MEMO_V1_PROGRAM_ID, ParsableProgram::HplMemo);
        // m.insert(*MEMO_V3_PROGRAM_ID, ParsableProgram::HplMemo);
        for hpl_token_id in hpl_token_ids() {
            m.insert(hpl_token_id, ParsableProgram::HplToken);
        }
        ...
        m.insert(*NFT_PROGRAM_ID, ParsableProgram::HplNft);
        m.insert(*NAME_PROGRAM_ID, ParsableProgram::HplName);
        m
    };
}
...
pub fn parse(
    ...
    let parsed_json = match program_name {
        ...
        ParsableProgram::HplMemo => parse_memo(instruction)?,
        ParsableProgram::HplToken => serde_json::to_value(parse_token(instruction, account_keys)?)?,
        ...
        ParsableProgram::HplNft=> serde_json::to_value(parse_nft(instruction, account_keys)?)?,
        ParsableProgram::HplName=> serde_json::to_value(parse_name(instruction, account_keys)?)?,
    };
...
fn parse_memo(instruction: &CompiledInstruction) -> Result<Value, ParseInstructionError> {
    parse_memo_data(&instruction.data)
        .map(Value::String)
        .map_err(|_| ParseInstructionError::InstructionNotParsable(ParsableProgram::HplMemo))
}
