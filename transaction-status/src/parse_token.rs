use {
    ...
    hpl_token::{
        ...
        huione_program::{instruction::Instruction as HplTokenInstruction, program_option::COption},
    },
    huione_account_decoder::parse_token::{pubkey_from_hpl_token, token_amount_to_ui_amount},
    huione_sdk::{
...
pub fn parse_token(
    ...
    let token_instruction = TokenInstruction::unpack(&instruction.data)
        .map_err(|_| ParseInstructionError::InstructionNotParsable(ParsableProgram::HplToken))?;
    match instruction.accounts.iter().max() {
        Some(index) if (*index as usize) < account_keys.len() => {}
        _ => {
            // Runtime should prevent this from ever happening
            return Err(ParseInstructionError::InstructionKeyMismatch(
                ParsableProgram::HplToken,
            ));
        }
    }
...
fn check_num_token_accounts(accounts: &[u8], num: usize) -> Result<(), ParseInstructionError> {
    check_num_accounts(accounts, num, ParsableProgram::HplToken)
}

pub fn hpl_token_instruction(instruction: HplTokenInstruction) -> Instruction {
    Instruction {
        program_id: pubkey_from_hpl_token(&instruction.program_id),
        accounts: instruction
            .accounts
            .iter()
            .map(|meta| AccountMeta {
                pubkey: pubkey_from_hpl_token(&meta.pubkey),
...
mod test {
    ...
    use {
        ...
        hpl_token::{
            instruction::*,
            huione_program::{
                instruction::CompiledInstruction as HplTokenCompiledInstruction, message::Message,
                pubkey::Pubkey as HplTokenPubkey,
            },
        },
        huione_sdk::{instruction::CompiledInstruction, pubkey::Pubkey},
        std::str::FromStr,
    };

    fn convert_pubkey(pubkey: Pubkey) -> HplTokenPubkey {
        HplTokenPubkey::from_str(&pubkey.to_string()).unwrap()
    }

    fn convert_compiled_instruction(
        instruction: &HplTokenCompiledInstruction,
    ) -> CompiledInstruction {
        CompiledInstruction {
    ...
    fn test_parse_token() {
        ...
        let rent_sysvar = huione_sdk::sysvar::rent::id();
        ...
        // Test SyncNative
        let sync_native_ix =
            sync_native(&hpl_token::id(), &convert_pubkey(account_pubkey)).unwrap();
    ...
    fn test_token_ix_not_enough_keys() {
        ...
        for _ in 0..10 {
            keys.push(huione_sdk::pubkey::new_rand());
        }

        let (_, _) =
            Pubkey::find_program_address(&[b"MintMeta", &keys[0].to_bytes()], &hpl_token::id());
        ...
        // Test SyncNative
        let sync_native_ix = sync_native(&hpl_token::id(), &convert_pubkey(keys[0])).unwrap();
        let message = Message::new(&[sync_native_ix], None);
        let mut compiled_instruction = convert_compiled_instruction(&message.instructions[0]);
