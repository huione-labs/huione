use {
    ...
    huione_account_decoder::parse_token::{
        is_known_hpl_token_id, pubkey_from_hpl_token, hpl_token_native_mint,
...
mod test {
    ...
    fn test_collect_token_balance_from_account() {
        ...
        let mint = Account {
            lamports: 100,
            data: data.to_vec(),
            owner: pubkey_from_hpl_token(&hpl_token::id()),
            executable: false,
            rent_epoch: 0,
        };
        ...
        let token_data = TokenAccount {
            mint: hpl_token_pubkey(&mint_pubkey),
            owner: hpl_token_pubkey(&token_owner),
            amount: 42,
        ...
        let other_mint_data = TokenAccount {
            mint: hpl_token_pubkey(&other_mint_pubkey),
            owner: hpl_token_pubkey(&token_owner),
            amount: 42,
            delegate: COption::None,
            state: hpl_token::state::AccountState::Initialized,
    ...
    fn test_mint_info_data_oa() {
        let mint_data = MintMeta {
            ...
            symbol: hpl_token::native_mint::SYMBOL.to_string(),
            name: hpl_token::native_mint::NAME.to_string(),
            icon: hpl_token::native_mint::ICON.to_string(),
        };
