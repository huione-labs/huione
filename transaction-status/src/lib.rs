...
pub use {crate::extract_memos::extract_and_fmt_memos, huione_sdk::reward_type::RewardType};
use {
    crate::{
...
pub mod serde_u128_str {
    ...
    ///
    pub fn serialize<S>(n: &u128, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
pub mod serde_vec_u128_str {
    use serde::{self, ser::SerializeSeq, Deserialize, Deserializer, Serializer};
    ///
    pub fn serialize<S>(n: &Vec<u128>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut seq = serializer.serialize_seq(Some(n.len()))?;
        for elem in n {
            seq.serialize_element(&elem.to_string())?;
        }
        seq.end()
    }
    ...
    where
        D: Deserializer<'de>,
    {
        let de: Vec<String> = Deserialize::deserialize(deserializer)?;
        let mut re: Vec<u128> = Vec::with_capacity(de.len());
        for i in 0..de.len() {
            if let Ok(d) = de[i].parse() {
                re.push(d);
            } else {
                return Err(serde::de::Error::custom("vec_u128 deserialize err!"));
            }
        }
        Ok(re)
    }
...
/// serde i128 to str
pub mod serde_i128_str {
    use serde::{self, Deserialize, Deserializer, Serializer};
    ///
    pub fn serialize<S>(n: &i128, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        serializer.serialize_str(&n.to_string())
...
pub mod serde_vec_i128_str {
    use serde::{self, ser::SerializeSeq, Deserialize, Deserializer, Serializer};
    ///
    pub fn serialize<S>(n: &Vec<i128>, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut seq = serializer.serialize_seq(Some(n.len()))?;
    ...
    ///
    pub fn deserialize<'de, D>(deserializer: D) -> Result<Vec<i128>, D::Error>
    where
        D: Deserializer<'de>,
    {
        let de: Vec<String> = Deserialize::deserialize(deserializer)?;
        let mut re: Vec<i128> = Vec::with_capacity(de.len());
        for i in 0..de.len() {
            if let Ok(d) = de[i].parse() {
                re.push(d);
            } else {
                return Err(serde::de::Error::custom("vec_i128 deserialize err!"));
            }
        }
        Ok(re)
    }
...
impl<'a> TryInto<CompiledInstruction> for &'a UiCompiledInstruction {
    type Error = bs58::decode::Error;
    fn try_into(self: &'a UiCompiledInstruction) -> Result<CompiledInstruction, Self::Error> {
        Ok(CompiledInstruction {
            program_id_index: self.program_id_index,
            accounts: self.accounts.clone(),
            data: bs58::decode(self.data.clone()).into_vec()?,
        })
    }
}
...
pub struct TransactionStatusMeta {
    ...
    pub fee: u128,
    pub pre_balances: Vec<u128>,
    pub post_balances: Vec<u128>,
...
pub struct UiTransactionStatusMeta {
    ...
    pub status: TransactionResult<()>, // This field is deprecated.  See https://github.com/huione-labs/huione/issues/9302
    #[serde(with = "serde_u128_str")]
    pub fee: u128,
    #[serde(with = "serde_vec_u128_str")]
    pub pre_balances: Vec<u128>,
    #[serde(with = "serde_vec_u128_str")]
    pub post_balances: Vec<u128>,
    #[serde(
...
pub struct Reward {
    pub pubkey: String,
    #[serde(with = "serde_i128_str")]
    pub lamports: i128,
    #[serde(with = "serde_u128_str")]
    pub post_balance: u128, // Account balance in lamports after `lamports` was applied
...
mod test {
    ...
    fn test_serde_empty_fields() {
        fn test_serde<'de, T: serde::Serialize + serde::Deserialize<'de>>(
            ...
            let expected_json_outhuione_value: serde_json::Value =
                serde_json::from_str(expected_json_output).unwrap();
            assert_eq!(reserialized_value, expected_json_outhuione_value);
        }
    ...
    fn test_ui_transaction_status_meta_ctors_serialization() {
        ...
        let expected_json_outhuione_value: serde_json::Value = serde_json::from_str(
            "{\
            \"err\":null,\
            \"status\":{\"Ok\":null},\
            \"fee\":\"1234\",\
