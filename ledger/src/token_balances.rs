use {
    huione_account_decoder::parse_token::{
        is_known_hpl_token_id, hpl_token_native_mint,pubkey_from_hpl_token,
        token_amount_to_ui_amount, UiTokenAmount,
    },
    huione_measure::measure::Measure,
    huione_metrics::datapoint_debug,
    huione_runtime::{bank::Bank, transaction_batch::TransactionBatch},
    huione_sdk::{account::ReadableAccount, pubkey::Pubkey},
...
fn get_mint_decimals(bank: &Bank, mint: &Pubkey) -> Option<u8> {
    if mint == &hpl_token_native_mint() {
        Some(hpl_token::native_mint::DECIMALS)
    } else {
...
pub fn collect_token_balances(
    ...
    for transaction in batch.sanitized_transactions() {
        ...
        let has_token_program = account_keys.iter().any(is_known_hpl_token_id);

        let mut transaction_balances: Vec<TransactionTokenBalance> = vec![];
        if has_token_program {
            for (index, account_id) in account_keys.iter().enumerate() {
                if transaction.message().is_invoked(index) || is_known_hpl_token_id(account_id) {
                    continue;
                }
...
fn collect_token_balance_from_account(
    ...
    let mint = pubkey_from_hpl_token(&token_account.mint);
...
mod test {
    use {
        super::*,
        huione_sdk::{account::Account, genesis_config::create_genesis_config},
        hpl_token::huione_program::program_option::COption,
    ...
    fn test_collect_token_balance_from_account() {
        ...
        let other_mint = Account {
            lamports: 100,
            data: data.to_vec(),
            owner: Pubkey::new_unique(), // !is_known_hpl_token_id
            executable: false,
            rent_epoch: 0,
        };
        ...
        let token_data = TokenAccount {
            ...
            state: hpl_token::state::AccountState::Initialized,
        ...
        let hpl_token_account_pubkey = Pubkey::new_unique();
        accounts.insert(hpl_token_account_pubkey, hpl_token_account);
        ...
        // Account is not owned by hpl_token (nor does it have TokenAccount state)
        assert_eq!(
        ...
        // TokenAccount owned by hpl_token::id() works
        assert_eq!(
            collect_token_balance_from_account(
                &bank,
                &hpl_token_account_pubkey,
                &mut mint_decimals
            ),
            Some(TokenBalanceData {
                ...
                program_id: hpl_token::id().to_string(),
            })
        );

        // TokenAccount is not owned by known hpl-token program_id
        assert_eq!(
            collect_token_balance_from_account(&bank, &other_account_pubkey, &mut mint_decimals),
            None
        );

        // TokenAccount's mint is not owned by known hpl-token program_id
        assert_eq!(
            collect_token_balance_from_account(
                &bank,
    ...
    // #[test]
    // fn test_collect_token_balance_from_hpl_token_2022_account() {
    //     let (mut genesis_config, _mint_keypair) = create_genesis_config(500);
    ...
    //         amount: 42,
    //         delegate: COption::None,
    //         state: hpl_token_2022::state::AccountState::Initialized,
    //         is_native: COption::Some(100),
    //         delegated_amount: 0,
    //         close_authority: COption::None,
    //     };
    ...
    //     // TokenAccount owned by hpl_token_2022::id() works
    //     assert_eq!(
    //         collect_token_balance_from_account(
    //             &bank,
    //             &hpl_token_account_pubkey,
    ...
    //                 ui_amount_string: "0.42".to_string(),
    //             },
    //             program_id: hpl_token_2022::id().to_string(),
    //         })
    //     );
