use {
    ...
    huione_metrics::datapoint_error,
    huione_program_runtime::timings::{ExecuteTimingType, ExecuteTimings, ThreadExecuteTimings},
    huione_rayon_threadlimit::{get_max_thread_count, get_thread_count},
    huione_runtime::{
    ...
    huione_transaction_status::token_balances::TransactionTokenBalancesSet,
    std::{
        borrow::Cow,
...
lazy_static! {
    static ref PAR_THREAD_POOL: ThreadPool = rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("huioneBstoreProc{i:02}"))
        .build()
        .unwrap();
}
...
fn supermajority_root(roots: &[(Slot, u128)], total_epoch_stake: u128) -> Option<Slot> {
    if roots.is_empty() {
        return None;
    }
...
fn supermajority_root_from_vote_accounts(
    bank_slot: Slot,
    total_epoch_stake: u128,
    vote_accounts: &VoteAccountsHashMap,
) -> Option<Slot> {
    let mut roots_stakes: Vec<(Slot, u128)> = vote_accounts
        .iter()
...
pub mod tests {
    use {
        ...
        rand::{thread_rng, Rng},
        huione_entry::entry::{create_ticks, next_entry, next_entry_mut},
        huione_program_runtime::declare_process_instruction,
        huione_runtime::{
            genesis_utils::{
    ...
    fn test_process_entries_2_txes_collision() {
        ...
        // fund: huione 4 in each of 1 and 2
    ...
    fn test_transaction_result_does_not_affect_bankhash() {
        ...
        let mock_program_id = huione_sdk::pubkey::new_rand();
    ...
    fn test_process_entries_2nd_entry_collision_with_self_and_error() {
        ...
        // fund: huione some money in each of 1 and 2
    ...
    fn test_process_entry_tx_random_execution_no_error() {
        ...
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config((num_accounts + 1) as u128 * initial_lamports);
    ...
    fn test_process_blockstore_from_root() {
        ...
        */
        ...
        bank_forks.set_root(
            1,
            &huione_runtime::accounts_background_service::AbsRequestSender::default(),
            None,
        );
    ...
    fn test_supermajority_root_from_vote_accounts() {
        let convert_to_vote_accounts = |roots_stakes: Vec<(Slot, u128)>| -> VoteAccountsHashMap {
            roots_stakes
                .into_iter()
                .map(|(root, stake)| {
                    ...
                    let mut vote_account =
                        AccountSharedData::new(1, VoteState::size_of(), &huione_vote_program::id());
    ...
    fn test_confirm_slot_entries_progress_num_txs_indexes() {
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config(100 * LAMPORTS_PER_HC);
        let genesis_hash = genesis_config.hash();
        let bank = Arc::new(Bank::new_for_tests(&genesis_config));
        ...
        let keypair3 = Keypair::new();
        let keypair4 = Keypair::new();
        bank.transfer(LAMPORTS_PER_HC, &mint_keypair, &keypair1.pubkey())
            .unwrap();
        bank.transfer(LAMPORTS_PER_HC, &mint_keypair, &keypair2.pubkey())
            .unwrap();
