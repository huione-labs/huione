...
impl LeaderSchedule {
    ...
    pub fn new(ids_and_stakes: &[(Pubkey, u128)], seed: [u8; 32], len: u64, repeat: u64) -> Self {
...
mod tests {
    ...
    fn test_leader_schedule_basic() {
        ...
        let stakes: Vec<_> = (0..num_keys)
            .map(|i| (huione_sdk::pubkey::new_rand(), i))
            .collect();

        let seed = huione_sdk::pubkey::new_rand();
        let mut seed_bytes = [0u8; 32];
        seed_bytes.copy_from_slice(seed.as_ref());
        let len = num_keys * 10;
        let leader_schedule = LeaderSchedule::new(&stakes, seed_bytes, len as u64, 1);
        let leader_schedule2 = LeaderSchedule::new(&stakes, seed_bytes, len as u64, 1);
        assert_eq!(leader_schedule.slot_leaders.len() as u128, len);
    ...
    fn test_repeated_leader_schedule() {
        ...
        let len = num_keys * 10;
        let repeat = 8;
        let leader_schedule = LeaderSchedule::new(&stakes, seed_bytes, len as u64, repeat);
        assert_eq!(leader_schedule.slot_leaders.len() as u128, len);
