...
use {
    ...
    huione_runtime::hardened_unpack::UnpackError,
    huione_sdk::{
    ...
    fn open(path: &Path, options: BlockstoreOptions) -> Result<Rocks> {
        ...
        let db = match access_type {
            ...
            AccessType::Secondary => {
                let secondary_path = path.join("huione-secondary");

                info!(
                    "Opening Rocks with secondary (read only) access at: {:?}",
                    secondary_path
                );
                info!("This secondary access could temporarily degrade other accesses, such as by huione-validator");
    ...
    C: ProtobufColumn + ColumnName,
{
    ...
    pub fn huione_protobuf(&self, key: C::Index, value: &C::Type) -> Result<()> {
