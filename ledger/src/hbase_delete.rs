use {log::*, huione_measure::measure::Measure, huione_sdk::clock::Slot, std::result::Result};

// Attempt to delete this many blocks in parallel
const NUM_BLOCKS_TO_DELETE_IN_PARALLEL: usize = 32;

pub fn delete_confirmed_blocks(
    ...
    for blocks in blocks_to_delete.chunks(NUM_BLOCKS_TO_DELETE_IN_PARALLEL) {
        ...
        let deletion_futures = blocks
            .iter()
            .map(|block| hbase.delete_confirmed_block(*block, dry_run));

        for (block, result) in blocks
            .iter()
            // .zip(futures::future::join_all(deletion_futures))
            .zip(deletion_futures)
        {
            if result.is_err() {
                error!(
