...
use {
    ...
    huione_entry::entry::{create_ticks, Entry},
    ...
    huione_rayon_threadlimit::get_max_thread_count,
    huione_runtime::{
        bank::Bank,
    ...
    huione_storage_proto::{StoredExtendedRewards, StoredTransactionStatusMeta},
...
// see: https://github.com/huione-labs/huione/pull/24853
lazy_static! {
    static ref PAR_THREAD_POOL: ThreadPool = rayon::ThreadPoolBuilder::new()
        .num_threads(get_max_thread_count())
        .thread_name(|i| format!("huioneBstore{i:02}"))
        .build()
        .unwrap();
    static ref PAR_THREAD_POOL_ALL_CPUS: ThreadPool = rayon::ThreadPoolBuilder::new()
        .num_threads(num_cpus::get())
        .thread_name(|i| format!("huioneBstoreAll{i:02}"))
        .build()
        .unwrap();
}
...
///
/// `huione_core::completed_data_sets_service::CompletedDataSetsService` is the main receiver of
...
impl Blockstore {
    ...
    pub fn huione_meta_bytes(&self, slot: Slot, bytes: &[u8]) -> Result<()> {
    ...
    /// Can interfere with automatic meta update and potentially break chaining.
    /// Dangerous. Use with care.
    pub fn huione_meta(&self, slot: Slot, meta: &SlotMeta) -> Result<()> {
        self.huione_meta_bytes(slot, &bincode::serialize(meta)?)
    }
    ...
    fn initialize_transaction_status_index(&self) -> Result<()> {
        ...
        self.transaction_status_cf
            .huione_protobuf(cf::TransactionStatus::as_index(2), &default_status)?;
    ...
    pub fn write_transaction_status(
        ...
        self.transaction_status_cf
            .huione_protobuf((primary_index, signature, slot), &status)?;
    ...
    pub fn write_rewards(&self, index: Slot, rewards: Rewards) -> Result<()> {
        ...
        self.rewards_cf.huione_protobuf(index, &rewards)
    }
                ...
                if let Ok(entries) = self.get_slot_entries(slot, 0) {
                    entries.into_par_iter().for_each(|entry| {
                        entry.transactions.into_iter().for_each(|tx| {
                            ...
                            // see: https://github.com/huione-labs/huione/issues/30165
...
/// https://github.com/huione-labs/huione/pull/2253
///
/// Arguments:
...
pub fn create_new_ledger(
    ...
    let version = huione_sdk::shred_version::version_from_hash(&last_hash);
...
fn adjust_ulimit_nofile(enforce_ulimit_nofile: bool) -> Result<()> {
    ...
    // https://docs.huione.com/running-validator/validator-start#increased-memory-mapped-files-limit
...
pub mod tests {
    use {
        ...
        huione_account_decoder::parse_token::UiTokenAmount,
        huione_entry::entry::{next_entry, next_entry_mut},
        huione_runtime::bank::{Bank, RewardType},
        huione_sdk::{
            hash::{self, hash, Hash},
        ...
        huione_storage_proto::convert::generated,
        huione_transaction_status::{
    ...
    pub(crate) fn make_slot_entries_with_transactions(num_entries: u64) -> Vec<Entry> {
        ...
        for x in 0..num_entries {
            let transaction = Transaction::new_with_compiled_instructions(
                ...
                Hash::default(),
                vec![huione_sdk::pubkey::new_rand()],
        ...
        pub fn test_chaining_tree() {
            ...
            for slot in slots {
                ...
                if rng.gen_bool(0.5) {
                    ...
                    blockstore.huione_shared_coding_shreds(&coding_shreds).unwrap();
                } else {
                    blockstore.huione_shared_coding_shreds(&coding_shreds).unwrap();
                    blockstore.write_shreds(slot_shreds).unwrap();
                }
    ...
    */
    ...
    fn test_get_rooted_block() {
        ...
        blockstore
            .huione_meta_bytes(slot - 1, &serialize(&parent_meta).unwrap())
            .unwrap();
        ...
        let expected_transactions: Vec<VersionedTransactionWithStatusMeta> = entries
            ...
            .flat_map(|entry| entry.transactions)
            .map(|transaction| {
                let mut pre_balances: Vec<u128> = vec![];
                let mut post_balances: Vec<u128> = vec![];
                for i in 0..transaction.message.static_account_keys().len() {
                    pre_balances.push(i as u128 * 10);
                    post_balances.push(i as u128 * 11);
                }
                ...
                blockstore
                    .transaction_status_cf
                    .huione_protobuf((0, signature, slot), &status)
                    .unwrap();
                let status = TransactionStatusMeta {
                    status: Ok(()),
                    fee: 42,
                ...
                blockstore
                    .transaction_status_cf
                    .huione_protobuf((0, signature, slot + 1), &status)
                    .unwrap();
                let status = TransactionStatusMeta {
                    status: Ok(()),
                    fee: 42,
                ...
                blockstore
                    .transaction_status_cf
                    .huione_protobuf((0, signature, slot + 2), &status)
                    .unwrap();
                VersionedTransactionWithStatusMeta {
                    transaction,
    ...
    fn test_persist_transaction_status() {
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: huione_sdk::transaction::Result::<()>::Err(TransactionError::AccountNotFound),
            fee: 5u128,
        ...
        .into();
        assert!(transaction_status_cf
            .huione_protobuf((0, Signature::default(), 0), &status,)
            .is_ok());
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: huione_sdk::transaction::Result::<()>::Ok(()),
            fee: 9u128,
            pre_balances: pre_balances_vec.clone(),
            post_balances: post_balances_vec.clone(),
            inner_instructions: Some(inner_instructions_vec.clone()),
        ...
        .into();
        assert!(transaction_status_cf
            .huione_protobuf((0, Signature::from([2u8; 64]), 9), &status,)
            .is_ok());
    ...
    fn test_get_transaction_status() {
        ...
        transaction_status_cf
            .huione_protobuf((0, signature2, 1), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature2, 2), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature4, 1), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature5, 1), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature5, 3), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature6, 1), &status)
            .unwrap();
        ...
        transaction_status_cf
            .huione_protobuf((1, signature4, 2), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((1, signature5, 4), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((1, signature5, 5), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((1, signature6, 3), &status)
            .unwrap();
    ...
    fn do_test_lowest_cleanup_slot_and_special_cfs(simulate_ledger_cleanup_service: bool) {
        ...
        transaction_status_cf
            .huione_protobuf((0, signature1, lowest_cleanup_slot), &status)
            .unwrap();

        transaction_status_cf
            .huione_protobuf((0, signature2, lowest_available_slot), &status)
            .unwrap();

        let address0 = huione_sdk::pubkey::new_rand();
        let address1 = huione_sdk::pubkey::new_rand();
        blockstore
            .write_transaction_status(
                lowest_cleanup_slot,
                signature1,
    ...
    fn test_map_transactions_to_statuses() {
        ...
        for x in 0..4 {
            ...
            let status = TransactionStatusMeta {
                status: huione_sdk::transaction::Result::<()>::Err(
            ...
            .into();
            transaction_status_cf
                .huione_protobuf((0, transaction.signatures[0], slot), &status)
                .unwrap();
        ...
        for (x, m) in map.iter().enumerate() {
            assert_eq!(m.meta.fee, x as u128);
        }
    ...
    fn test_rewards_protobuf_backward_compatability() {
        ...
        let rewards: Rewards = (0..100)
            .map(|i| Reward {
                pubkey: huione_sdk::pubkey::new_rand().to_string(),
                lamports: 42 + i,
                post_balance: std::u128::MAX,
        ...
        for slot in 2..4 {
            blockstore
                .rewards_cf
                .huione_protobuf(slot, &protobuf_rewards)
                .unwrap();
        }
        for slot in 0..4 {
            assert_eq!(
    ...
    fn test_transaction_status_protobuf_backward_compatability() {
        ...
        for slot in 2..4 {
            blockstore
                .transaction_status_cf
                .huione_protobuf((0, Signature::default(), slot), &protobuf_status)
                .unwrap();
        }
        for slot in 0..4 {
            assert_eq!(
    ...
    fn make_large_tx_entry(num_txs: usize) -> Entry {
        let txs: Vec<_> = (0..num_txs)
            .map(|_| {
                ...
                huione_sdk::system_transaction::transfer(&keypair0, &to, 1, Hash::default())
            })
            .collect();
