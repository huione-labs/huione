use {
    ...
    huione_account_decoder::parse_token::{real_number_string_trimmed, UiTokenAmount},
    huione_sdk::{
        hash::Hash,
...
pub mod generated {
    include!(concat!(
        env!("OUT_DIR"),
        "/huione.storage.confirmed_block.rs"
    ));
}
...
pub mod tx_by_addr {
    include!(concat!(
        env!("OUT_DIR"),
        "/huione.storage.transaction_by_addr.rs"
    ));
}
...
impl From<Reward> for generated::Reward {
    fn from(reward: Reward) -> Self {
        Self {
            ...
            lamports: reward.lamports.to_string(),
            post_balance: reward.post_balance.to_string(),
...
impl From<generated::Reward> for Reward {
    fn from(reward: generated::Reward) -> Self {
        Self {
            pubkey: reward.pubkey,
            lamports: i128::from_str(&reward.lamports).unwrap(),
            post_balance: u128::from_str(&reward.post_balance).unwrap(),
            reward_type: match reward.reward_type {
                0 => None,
...
impl From<TransactionStatusMeta> for generated::TransactionStatusMeta {
    fn from(value: TransactionStatusMeta) -> Self {
        ...
        Self {
            err,
            fee:fee.to_string(),
            pre_balances:pre_balances.iter().map(|b|{b.to_string()}).collect(),
            post_balances:post_balances.iter().map(|b|{b.to_string()}).collect(),
            inner_instructions,
...
impl TryFrom<generated::TransactionStatusMeta> for TransactionStatusMeta {
    ...
    fn try_from(value: generated::TransactionStatusMeta) -> std::result::Result<Self, Self::Error> {
        ...
        Ok(Self {
            status,
            fee:u128::from_str(&fee).unwrap(),
            pre_balances:pre_balances.iter().map(|b|{u128::from_str(&b).unwrap()}).collect(),
            post_balances:post_balances.iter().map(|b|{u128::from_str(&b).unwrap()}).collect(),
            inner_instructions,
            log_messages,
            pre_token_balances,
            post_token_balances,
            ...
            ui_token_amount: UiTokenAmount {
                ...
                ui_amount_string: if !ui_token_amount.ui_amount_string.is_empty() {
                    ...
                    real_number_string_trimmed(
                        u128::from_str(&ui_token_amount.amount).unwrap_or_default(),
