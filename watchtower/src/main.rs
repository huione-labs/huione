...
use {
    ...
    huione_clap_utils::{
        ...
        input_parsers::{pubkeys_of,lamports_of_hc},
        input_validators::{is_parsable, is_pubkey_or_keypair, is_url, is_valid_percentage},
    },
    huione_cli_output::display::format_labeled_address,
    huione_metrics::{datapoint_error, datapoint_info},
    huione_notifier::{NotificationType, Notifier},
    huione_rpc_client::rpc_client::RpcClient,
    huione_rpc_client_api::{client_error, response::RpcVoteAccountStatus},
    huione_sdk::{
        hash::Hash,
        native_token::{HC},
        pubkey::Pubkey,
...
struct Config {
    ...
    minimum_validator_identity_balance: u128,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `huione-watchtower`:
        ...
        define environment variable before running `huione-watchtower`:
        ...
        .arg(
            Arg::with_name("minimum_validator_identity_balance")
                ...
                .help("Alert when the validator identity balance is less than this amount of HC")
        )
        .arg(
    ...
    let minimum_validator_identity_balance = lamports_of_hc(&matches,"minimum_validator_identity_balance").unwrap();
    let json_rpc_url =
...
) -> client_error::Result<(u64, Hash, RpcVoteAccountStatus, HashMap<Pubkey, u128>)> {
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    huione_metrics::set_panic_hook("watchtower", /*version:*/ None);
        ...
        let failure = match get_cluster_info(&config, &rpc_client) {
            Ok((transaction_count, recent_blockhash, vote_accounts, validator_balances)) => {
                ...
                info!(
                    ...
                    HC(total_stake),
                    HC(total_current_stake),
                    HC(total_delinquent_stake)
                );
                ...
                for validator_identity in config.validator_identity_pubkeys.iter() {
                    ...
                    if let Some(balance) = validator_balances.get(validator_identity) {
                        if *balance < config.minimum_validator_identity_balance {
                            failures.push((
                                "balance",
                                format!("{} has {}", formatted_validator_identity, HC(*balance)),
                            ));
                        }
                    }
                }
