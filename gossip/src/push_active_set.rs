use {
    ...
    huione_sdk::{native_token::LAMPORTS_PER_HC, pubkey::Pubkey},
...
impl PushActiveSet {
    ...
    fn get_entry(&self, stake: Option<&u128>) -> &PushActiveSetEntry {
...
fn get_stake_bucket(stake: Option<&u128>) -> usize {
    let stake = stake.copied().unwrap_or_default() / LAMPORTS_PER_HC;
    let bucket = u128::BITS - stake.leading_zeros();
...
mod tests {
    ...
    fn test_get_stake_bucket() {
        ...
        for (k, bucket) in buckets.into_iter().enumerate() {
            let stake = (k as u128) * LAMPORTS_PER_HC;
        ...
        for (stake, bucket) in [
            ...
            let stake = stake * LAMPORTS_PER_HC;
            assert_eq!(get_stake_bucket(Some(&stake)), bucket);
        }
        assert_eq!(
            get_stake_bucket(Some(&u128::MAX)),
    ...
    fn test_push_active_set() {
        ...
        const MAX_STAKE: u128 = (1 << 20) * LAMPORTS_PER_HC ;
        ...
        // }
        // println!("=================");
        // let mut b = [1,5,7,6,17].into_iter().map(|k| &nodes[k]);
        // p = b.next();
        // while  p != None {
        //     match p {
        //         Some(k) => {
        ...
        assert!(active_set
            .get_nodes(&pubkey, origin, |_| false, &stakes)
            .eq([1,5,7,6].into_iter().map(|k| &nodes[k])));
