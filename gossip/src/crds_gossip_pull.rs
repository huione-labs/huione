...
use {
    ...
    huione_bloom::bloom::{AtomicBloom, Bloom},
    huione_sdk::{
        hash::{hash, Hash},
        native_token::LAMPORTS_PER_HC,
        pubkey::Pubkey,
...
impl huione_sdk::sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), huione_sdk::sanitize::SanitizeError> {
...
impl CrdsGossipPull {
    ...
    pub(crate) fn new_pull_request(
        ...
        let (weights, nodes): (Vec<u64>, Vec<ContactInfo>) =
            crds_gossip::dedup_gossip_addresses(nodes, stakes)
                .into_values()
                .map(|(stake, node)| {
                    let stake = stake.min(stake_cap) / LAMPORTS_PER_HC;
                    let weight = u128::BITS - stake.leading_zeros();
    ...
    pub(crate) fn generate_pull_responses(
        ...
        outhuione_size_limit: usize, // Limit number of crds values returned.
        now: u64,
        stats: &GossipStats,
    ) -> Vec<Vec<CrdsValue>> {
        Self::filter_crds_values(thread_pool, crds, requests, outhuione_size_limit, now, stats)
    }
    ...
    fn filter_crds_values(
        ...
        let outhuione_size_limit = outhuione_size_limit.try_into().unwrap_or(i64::MAX);
        let outhuione_size_limit = AtomicI64::new(outhuione_size_limit);
        let crds = crds.read().unwrap();
        let apply_filter = |caller: &CrdsValue, filter: &CrdsFilter| {
            if outhuione_size_limit.load(Ordering::Relaxed) <= 0 {
                return Vec::default();
            }
            ...
            let out: Vec<_> = crds
                ...
                .take(outhuione_size_limit.load(Ordering::Relaxed).max(0) as usize)
                .collect();
            outhuione_size_limit.fetch_sub(out.len() as i64, Ordering::Relaxed);
            out
        };
...
pub struct CrdsTimeouts<'a> {
    pubkey: Pubkey,
    stakes: &'a HashMap<Pubkey, /*lamports:*/ u128>,
    default_timeout: u64,
...
impl<'a> CrdsTimeouts<'a> {
    pub fn new(
        ...
        let default_timeout = if stakes.values().all(|&stake| stake == 0u128) {
            extended_timeout
        } else {
            default_timeout
        };
...
impl<'a> Index<&Pubkey> for CrdsTimeouts<'a> {
    ...
    fn index(&self, pubkey: &Pubkey) -> &Self::Output {
        if pubkey == &self.pubkey {
            &u64::MAX
        } else if self.stakes.get(pubkey) > Some(&0u128) {
...
pub(crate) mod tests {
    use {
        ...
        huione_perf::test_tx::new_test_vote_tx,
        huione_sdk::{
    ...
    fn test_hash_as_u64_random() {
        ...
        for _ in 0..100 {
            let hash = huione_sdk::hash::new_rand(&mut rng);
    ...
    fn test_crds_filter_set_add() {
        ...
        let hash_values: Vec<_> = repeat_with(|| huione_sdk::hash::new_rand(&mut rng))
            .take(1024)
            .collect();
    ...
    fn test_new_pull_request() {
        ...
        let new = ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), now);
        ping_cache
            .lock()
            .unwrap()
        ...
        let offline = ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let old = ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let new = ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), 0);
    ...
    fn test_generate_pull_responses() {
        ...
        let rsp = CrdsGossipPull::generate_pull_responses(
            ...
            &dest_crds,
            &filters,
            usize::MAX, // outhuione_size_limit
            now,
            &GossipStats::default(),
        );
    ...
    fn test_process_pull_request_response() {
        ...
        let new = ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), 1);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = huione_sdk::pubkey::new_rand();
    ...
    fn test_gossip_purge() {
        ...
        let mut node_crds = Crds::default();
        let entry = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(
            ContactInfo::new_localhost(&huione_sdk::pubkey::new_rand(), 0),
        ));
