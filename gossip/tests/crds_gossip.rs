...
fn stakes(network: &Network) -> HashMap<Pubkey, u128> {
...
fn connected_staked_network_create(stakes: &[u128]) -> Network {
    let num = stakes.len();
    let mut network: HashMap<_, _> = (0..num)
        .map(|n| {
            let node_keypair = Arc::new(Keypair::new());
...
fn test_connected_staked_network() {
    ...
    let stake_sum: u128 = stakes.iter().sum();
    let avg_stake: u128 = stake_sum / stakes.len() as u128;
    let avg_stake_pruned = network.stake_pruned / network.connections_pruned.len() as u128;
    trace!(
