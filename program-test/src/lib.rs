//! The huione-program-test provides a BanksClient-based test framework BPF programs
...
use {
    ...
    log::*,
    huione_banks_client::start_client,
    huione_banks_server::banks_server::start_local_server,
    huione_bpf_loader_program::serialization::serialize_parameters,
...
// Export types so test clients can limit their huione crate dependencies
pub use {
    huione_banks_client::{BanksClient, BanksClientError},
    huione_banks_interface::BanksTransactionResultWithMetadata,
    huione_program_runtime::invoke_context::InvokeContext,
    huione_sdk::transaction_context::IndexOfAccount,
};
...
pub fn builtin_process_instruction(
    process_instruction: huione_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `huione-program`-style entrypoint into the runtime's entrypoint style, for
...
struct SyscallStubs {}
impl huione_sdk::program_stubs::SyscallStubs for SyscallStubs {
...
impl Default for ProgramTest {
    ...
    ///
    fn default() -> Self {
        huione_logger::setup_with_default(
            "huione_rbpf::vm=debug,\
             huione_runtime::message_processor=debug,\
             huione_runtime::system_instruction_processor=trace,\
             huione_program_test=info",
        );
        let prefer_bpf =
        ...
        let deactivate_feature_set =
            HashSet::from([huione_sdk::feature_set::native_programs_consume_cu::id()]);
...
impl ProgramTest {
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                huione_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
        ...
        let bootstrap_validator_stake_lamports =
            rent.minimum_balance(VoteState::size_of()) + hc_to_lamports("1000000.0");

        let mint_keypair = Keypair::new();
        let voting_keypair = Keypair::new();

        let mut genesis_config = create_genesis_config_with_leader_ex(
            hc_to_lamports("1000000.0"),
            &mint_keypair.pubkey(),
        ...
        // Add commonly-used HPL programs as a convenience to the user
        for (program_id, account) in programs::hpl_programs(&Rent::default()).iter() {
    ...
    pub fn warp_to_slot(&mut self, warp_slot: Slot) -> Result<(), ProgramTestError> {
        ...
        let warp_bank = if pre_warp_slot == working_slot {
            ...
            bank_forks.insert(Bank::warp_from_parent(
                ...
                huione_runtime::accounts_db::CalcAccountsHashDataSource::IndexForTests,
            ))
        };
