...
use {
    ...
    huione_entry::poh::compute_hashes_per_tick,
    huione_genesis::{Base64Account},
    huione_ledger::{blockstore::create_new_ledger, blockstore_options::LedgerColumnOptions},
    ...
    huione_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
pub fn load_genesis_accounts(file: &str, genesis_config: &mut GenesisConfig) -> io::Result<u128> {
    let mut lamports = 0;
...
const MAX_TOKEN_SUPPLY:u128 = 100_000000000_000000000;
#[allow(clippy::cognitive_complexity)]
fn main() -> Result<(), Box<dyn error::Error>> {
    // let default_faucet_pubkey = huione_cli_config::Config::default().keypair_path;
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("distribute_account")
                .long("distribute-account")
                .value_name("DISTRIBUTE PROGRAM PDA ACCOUNT")
                .takes_value(true)
                .validator(is_pubkey)
                .help("Distribute token PDA address"),
        )
        .arg(
            Arg::with_name("distribute_program")
                .long("distribute-program")
                .value_name("DISTRIBUTE PROGRAM ID ")
                .takes_value(true)
                .validator(is_pubkey)
                .requires("distribute_account")
                .help("Distribute token program ID"),
        )
        .get_matches();
    ...
    let rent = Rent {
        lamports_per_byte_year: value_t_or_exit!(matches, "lamports_per_byte_year", u128),
    ...
    fn rent_exempt_check(matches: &ArgMatches<'_>, name: &str, exempt: u128) -> io::Result<u128> {
        let lamports = value_t_or_exit!(matches, name, u128);
    ...
    let bootstrap_validator_lamports =
        value_t_or_exit!(matches, "bootstrap_validator_lamports", u128);
    ...
    let faucet_lamports = value_t!(matches, "faucet_lamports", u128).unwrap_or(0);
    ...
    let mut fee_rate_governor = FeeRateGovernor::new(
        value_t_or_exit!(matches, "target_lamports_per_signature", u128),
        value_t_or_exit!(matches, "target_signatures_per_slot", u128),
    );
    ...
    huione_stake_program::add_genesis_accounts(&mut genesis_config);
    // // oa comment activate_all_features
    // oa 1.1.0 comment activate_all_features
    // if genesis_config.cluster_type == ClusterType::Development {
    //     huione_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    // }else{
    //     huione_runtime::genesis_utils::activate_init_features(&mut genesis_config);
    // }
    ...
    let parse_address = |address: &str, inhuione_type: &str| {
        address.parse::<Pubkey>().unwrap_or_else(|err| {
            eprintln!("Error: invalid {inhuione_type} {address}: {err}");
            process::exit(1);
        })
    };
    ...
    let distribute_account = pubkey_of(&matches, "distribute_account");
    let distribute_program = pubkey_of(&matches, "distribute_program");

    if let Some(distribute_program) = distribute_program {
        genesis_config.add_account(
            distribute_account.unwrap(),
            AccountSharedData::new(MAX_TOKEN_SUPPLY - issued_lamports, 0, &distribute_program),
        );
    }
    //else{
    //     // distribute account address
    //     let dis_acc = "DistAccount11111111111111111111111111111111".parse::<Pubkey>().unwrap_or_else(|err| {
    //         eprintln!("Error: invalid dis_account address {}", err);
    //         process::exit(1);
    //     });
    //     // distribute contract address
    //     let dis_contract = "DistConTract1111111111111111111111111111111".parse::<Pubkey>().unwrap_or_else(|err| {
    //         eprintln!("Error: invalid dis_contract address {}", err);
    //         process::exit(1);
    //     });
    //     genesis_config.add_account(
    //         dis_acc,
    //         AccountSharedData::new(MAX_TOKEN_SUPPLY - issued_lamports , 0, &dis_contract),
    //     );
    // }

    let all_lamports = genesis_config
        .accounts
        .iter()
        .map(|(_key, account)| account.lamports)
        .sum::<u128>();
    eprintln!("issued_lamports {}", all_lamports);
    huione_logger::setup();
    create_new_ledger(
        &ledger_path,
        &genesis_config,
...
mod tests {
    use {
        super::*,
        huione_sdk::genesis_config::GenesisConfig,
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            huione_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: huione_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
