...
use {
    ...
    hpl_token::{
        huione_program::{
            program_option::COption, program_pack::Pack, pubkey::Pubkey as HplTokenPubkey,
        },
...
// A helper function to convert hpl_token::id() as hpl_sdk::pubkey::Pubkey to
// huione_sdk::pubkey::Pubkey
fn hpl_token_id() -> Pubkey {
    Pubkey::new_from_array(hpl_token::id().to_bytes())
}

// Returns all known HPL Token program ids
pub fn hpl_token_ids() -> Vec<Pubkey> {
    vec![hpl_token_id()]
}

// Check if the provided program id as a known HPL Token program id
pub fn is_known_hpl_token_id(program_id: &Pubkey) -> bool {
    *program_id == hpl_token_id()
}

// A helper function to convert hpl_token::native_mint::id() as hpl_sdk::pubkey::Pubkey to
// huione_sdk::pubkey::Pubkey
pub fn hpl_token_native_mint() -> Pubkey {
    Pubkey::new_from_array(hpl_token::native_mint::id().to_bytes())
}


pub fn hpl_token_native_min_info() -> Pubkey {
    Pubkey::new_from_array(hpl_token::native_mint_info::id().to_bytes())
}


// The program id of the `hpl_token_native_mint` account
pub fn hpl_token_native_mint_program_id() -> Pubkey {
    hpl_token_id()
}

// A helper function to convert a huione_sdk::pubkey::Pubkey to hpl_sdk::pubkey::Pubkey
pub fn hpl_token_pubkey(pubkey: &Pubkey) -> HplTokenPubkey {
    HplTokenPubkey::new_from_array(pubkey.to_bytes())
}

// A helper function to convert a hpl_sdk::pubkey::Pubkey to huione_sdk::pubkey::Pubkey
pub fn pubkey_from_hpl_token(pubkey: &HplTokenPubkey) -> Pubkey {
...
pub fn parse_token(
    ...
    if data.len() == Account::get_packed_len() {
        let account = Account::unpack(data)
            .map_err(|_| ParseAccountError::AccountNotParsable(ParsableAccount::HplToken))?;
        let decimals = additional_data.hpl_token_decimals.ok_or_else(|| {
            ParseAccountError::AdditionalDataMissing(
                "no mint_decimals provided to parse hpl-token account".to_string(),
            )
        })?;
        ...
        Ok(TokenAccountType::Mint(UiMint {
            ...
            symbol: match additional_data.hpl_token_symbol {
                Some(s ) => s,
                None => "Not Set".to_string(),
            },
            name: match additional_data.hpl_token_name {
                Some(s ) => s,
                None => "Not Set".to_string(),
            },
            icon: match additional_data.hpl_token_icon {
                Some(s ) => s,
                None => "Not Set".to_string(),
            },
        ...
        Ok(TokenAccountType::Multisig(UiMultisig {
            ...
            signers: multisig
                ...
                .iter()
                .filter_map(|pubkey| {
                    if pubkey != &HplTokenPubkey::default() {
...
mod test {
    ...
    #[test]
    fn test_parse_token() {
        let mint_pubkey = HplTokenPubkey::from([2; 32]);
        let owner_pubkey = HplTokenPubkey::from([3; 32]);
        ...
        let signer1 = HplTokenPubkey::from([1; 32]);
        let signer2 = HplTokenPubkey::from([2; 32]);
        let signer3 = HplTokenPubkey::from([3; 32]);
        let mut multisig_data = vec![0; Multisig::get_packed_len()];
        let mut signers = [HplTokenPubkey::default(); 11];
        signers[0] = signer1;
        signers[1] = signer2;
        signers[2] = signer3;
