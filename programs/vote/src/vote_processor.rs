...
use {
    ...
    log::*,
    huione_program::vote::{instruction::VoteInstruction, program::id, state::VoteAuthorize},
...
declare_process_instruction!(process_instruction, 2100, |invoke_context| {
    ...
    match limited_deserialize(data)? {
        ...
        VoteInstruction::UpdateVoteState(_vote_state_update)
        | VoteInstruction::UpdateVoteStateSwitch(_vote_state_update, _) => {
            // if invoke_context
            //     .feature_set
        ...
        VoteInstruction::CompactUpdateVoteState(_vote_state_update)
        | VoteInstruction::CompactUpdateVoteStateSwitch(_vote_state_update, _) => {
            // if invoke_context
            //     .feature_set
            //     .is_active(&feature_set::allow_votes_to_directly_update_vote_state::id())
            //     && invoke_context
...
mod tests {
    ...
    fn create_test_account() -> (Pubkey, AccountSharedData) {
        ...
        (
            vote_pubkey,
            vote_state::create_account(&vote_pubkey, &huione_sdk::pubkey::new_rand(), 0, balance),
        )
    }

    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        let vote_pubkey = huione_sdk::pubkey::new_rand();
        let authorized_voter = huione_sdk::pubkey::new_rand();
        let authorized_withdrawer = huione_sdk::pubkey::new_rand();
    ...
    fn create_test_account_with_epoch_credits(
        ...
        let mut current_epoch_credits: u128 = 0;
    ...
    fn test_initialize_vote_account() {
        ...
        let node_pubkey = huione_sdk::pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = huione_sdk::pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
    ...
    fn test_authorize_withdrawer() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_withdrawer_pubkey = huione_sdk::pubkey::new_rand();
        let instruction_data = serialize(&VoteInstruction::Authorize(
