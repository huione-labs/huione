...
macro_rules! create_vm {
    ($vm:ident, $program:expr, $regions:expr, $orig_account_lengths:expr, $invoke_context:expr $(,)?) => {
        ...
        let heap_size = invoke_context
            .get_compute_budget()
            .heap_size
            .unwrap_or(huione_sdk::entrypoint::HEAP_LENGTH);
        let round_up_heap_size = invoke_context
            .feature_set
            .is_active(&huione_sdk::feature_set::round_up_heap_size::id());
        ...
        let $vm = heap_cost_result.and_then(|_| {
            let mut stack = huione_rbpf::aligned_memory::AlignedMemory::<
                { huione_rbpf::ebpf::HOST_ALIGN },
            >::zero_filled(stack_size);
            let mut heap = huione_rbpf::aligned_memory::AlignedMemory::<
                { huione_rbpf::ebpf::HOST_ALIGN },
...
macro_rules! mock_create_vm {
    ($vm:ident, $additional_regions:expr, $orig_account_lengths:expr, $invoke_context:expr $(,)?) => {
        let loader = std::sync::Arc::new(BuiltinProgram::new_loader(
            huione_rbpf::vm::Config::default(),
        ));
        let function_registry = huione_rbpf::vm::FunctionRegistry::default();
        let executable = huione_rbpf::elf::Executable::<
            huione_rbpf::verifier::TautologyVerifier,
            InvokeContext,
        >::from_text_bytes(
            &[0x95, 0, 0, 0, 0, 0, 0, 0],
            loader,
        ...
        .unwrap();
        let verified_executable = huione_rbpf::elf::Executable::verified(executable).unwrap();
        $crate::create_vm!(
            $vm,
            &verified_executable,
            $additional_regions,
    ...
    match limited_deserialize(instruction_data)? {
        ...
        UpgradeableLoaderInstruction::DeployWithMaxDataLen { max_data_len } => {
            ...
            let signers = [[new_program_id.as_ref(), &[bump_seed]]]
                ...
                .collect::<Result<Vec<Pubkey>, huione_sdk::pubkey::PubkeyError>>()?;
...
pub mod test_utils {
    use {
        super::*, huione_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
        huione_sdk::account::ReadableAccount,
    };
