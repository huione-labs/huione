...
use {
    ...
    huione_sdk::{
        ...
        feature_set::{
            self, curve25519_syscall_enabled,
...
pub fn create_program_runtime_environment<'a>(
    ...
    // )?;
    result.register_function(b"sol_get_fees_sysvar", SyscallGetRentSysvar::call)?;
...
declare_syscall!(
    ...
    fn inner_call(
        ...
        use huione_zk_token_sdk::curve25519::{curve_syscall_traits::*, edwards, ristretto};
        match curve_id {
            CURVE25519_EDWARDS => {
                let cost = invoke_context
                    .get_compute_budget()
...
declare_syscall!(
    ...
    fn inner_call(
        ...
        curve_id: u64,
        group_op: u64,
        left_inhuione_addr: u64,
        right_inhuione_addr: u64,
        result_point_addr: u64,
        memory_mapping: &mut MemoryMapping,
    ) -> Result<u64, Error> {
        use huione_zk_token_sdk::curve25519::{
        ...
        match curve_id {
            CURVE25519_EDWARDS => match group_op {
                ...
                MUL => {
                    ...
                    let inhuione_point = translate_type::<edwards::PodEdwardsPoint>(
                        memory_mapping,
                        right_inhuione_addr,
                        invoke_context.get_check_aligned(),
                    )?;

                    if let Some(result_point) = edwards::multiply_edwards(scalar, inhuione_point) {
                        *translate_type_mut::<edwards::PodEdwardsPoint>(
                            memory_mapping,
                            result_point_addr,
                            invoke_context.get_check_aligned(),
            ...
            CURVE25519_RISTRETTO => match group_op {
                ...
                MUL => {
                    ...
                    let inhuione_point = translate_type::<ristretto::PodRistrettoPoint>(
                        memory_mapping,
                        right_inhuione_addr,
                        invoke_context.get_check_aligned(),
                    )?;

                    if let Some(result_point) = ristretto::multiply_ristretto(scalar, inhuione_point) {
                        *translate_type_mut::<ristretto::PodRistrettoPoint>(
                            memory_mapping,
                            result_point_addr,
                            invoke_context.get_check_aligned(),
...
declare_syscall!(
    ...
    fn inner_call(
        ...
        use huione_sdk::alt_bn128::prelude::{ALT_BN128_ADD, ALT_BN128_MUL, ALT_BN128_PAIRING};
        let budget = invoke_context.get_compute_budget();
        let (cost, output): (u64, usize) = match group_op {
            ...
            ALT_BN128_PAIRING => {
                let ele_len = inhuione_size.saturating_div(ALT_BN128_PAIRING_ELEMENT_LEN as u64);
                let cost = budget
...
declare_syscall!(
    ...
    fn inner_call(
        ...
        let inhuione_len: u64 = std::cmp::max(params.base_len, params.exponent_len);
        let inhuione_len: u64 = std::cmp::max(inhuione_len, params.modulus_len);
...
mod tests {
    ...
    use {
        ...
        core::slice,
        huione_program_runtime::{invoke_context::InvokeContext, with_mock_invoke_context},
        huione_rbpf::{
            elf::SBPFVersion,
            error::EbpfError,
    ...
    fn test_translate_type() {
        ...
        let instruction = Instruction::new_with_bincode(
            ...
            &"foobar",
            vec![AccountMeta::new(huione_sdk::pubkey::new_rand(), false)],
        );
    ...
    fn test_translate_slice() {
        ...
        *data.first_mut().unwrap() = 10;
        ...
        // Pubkeys
        let mut data = vec![huione_sdk::pubkey::new_rand(); 5];
        let memory_mapping = MemoryMapping::new(
            vec![MemoryRegion::new_readonly(
                unsafe {
        ...
        let translated_data =
            translate_slice::<Pubkey>(&memory_mapping, 0x100000000, data.len() as u64, true, true)
                .unwrap();
        assert_eq!(data, translated_data);
        *data.first_mut().unwrap() = huione_sdk::pubkey::new_rand(); // Both should point to same place
        assert_eq!(data, translated_data);
    }
    ...
    fn test_syscall_sol_alloc_free() {
        // large alloc
        {
            ...
            SyscallAllocFree::call(
                invoke_context,
                huione_sdk::entrypoint::HEAP_LENGTH as u64,
                0,
                0,
                0,
                0,
    ...
    fn test_syscall_edwards_curve_point_validation() {
        use huione_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_EDWARDS;
    ...
    fn test_syscall_ristretto_curve_point_validation() {
        use huione_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_RISTRETTO;
    ...
    fn test_syscall_edwards_curve_group_ops() {
        use huione_zk_token_sdk::curve25519::curve_syscall_traits::{
    ...
    fn test_create_program_address() {
        // These tests duplicate the direct tests in huione_program::pubkey
