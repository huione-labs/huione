...
#[deprecated(
    since = "1.8.0",
    note = "Please use `huione_sdk::stake::program::id` or `huione_program::stake::program::id` instead"
)]
pub use huione_sdk::stake::program::{check_id, id};
use huione_sdk::{
...
pub fn get_minimum_delegation(feature_set: &FeatureSet) -> u128 {
    if feature_set.is_active(&feature_set::stake_raise_minimum_delegation_to_1_sol::id()) {
        ...
        MINIMUM_DELEGATION_SOL * LAMPORTS_PER_HC
    } else {
        #[allow(deprecated)]
        huione_sdk::stake::MINIMUM_STAKE_DELEGATION
    }
}
