...
#[deprecated(
    since = "1.8.0",
    note = "Please use `huione_sdk::stake::state` or `huione_program::stake::state` instead"
)]
pub use huione_sdk::stake::state::*;
use {
    ethnum::U256,
    huione_program_runtime::{ic_msg, invoke_context::InvokeContext},
    huione_sdk::{
    ...
    huione_vote_program::vote_state::{self, VoteState, VoteStateVersions},
...
pub enum InflationPointCalculationEvent {
    CalculatedPoints(u64, u128, U256, U256),
    SplitRewards(u128, u128, u128, PointValue),
    EffectiveStakeAtRewardedEpoch(u128),
    RentExemptReserve(u128),
    Delegation(Delegation, Pubkey),
    Commission(u8),
    CreditsObserved(u128, Option<u128>),
...
pub struct PointValue {
    pub rewards: u128, // lamports to split
    pub points: U256,  // over these points
}
        ...
        new_vote_state.epoch_credits().iter().copied()
    {
        ...
        if let Some(inflation_point_calc_tracer) = inflation_point_calc_tracer.as_ref() {
            inflation_point_calc_tracer(&InflationPointCalculationEvent::CalculatedPoints(
                epoch,
                stake_amount,
                U256::new(earned_credits),
                U256::new(earned_points),
            ));
        }
    }
...
fn calculate_stake_rewards(
    ...
    let rewards = points
        .checked_mul(U256::new(point_value.rewards))
        .unwrap()
        .checked_div(point_value.points)
        .unwrap();

    let rewards = u128::try_from(rewards).unwrap();
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != huione_vote_program::id() {
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != huione_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != huione_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
struct ValidatedSplitInfo {
    source_remaining_balance: u128,
    destination_rent_exempt_reserve: u128,
}
...
fn validate_split_amount(
    ...
    additional_required_lamports: u128,
        ...
        .is_active(&stake_split_uses_rent_sysvar::ID)
    {
        ...
        calculate_split_rent_exempt_reserve(
            ...
            source_data_len as u128,
            destination_data_len as u128,
        )
    };
...
fn merge_delegation_stake_and_credits_observed(
    ...
    stake: &mut Stake,
    absorbed_lamports: u128,
    absorbed_credits_observed: u128,
) -> Result<(), InstructionError> {
    stake.credits_observed =
...
fn stake_weighted_credits_observed(
    ...
    if stake.credits_observed == absorbed_credits_observed {
        ...
        let total_stake = U256::new(stake.delegation.stake.checked_add(absorbed_lamports)?);
        let stake_weighted_credits =
            U256::new(stake.credits_observed).checked_mul(U256::new(stake.delegation.stake))?;
        let absorbed_weighted_credits =
            U256::new(absorbed_credits_observed).checked_mul(U256::new(absorbed_lamports))?;
        ...
        let total_weighted_credits = stake_weighted_credits
            ...
            .checked_sub(U256::new(1))?;
        u128::try_from(total_weighted_credits.checked_div(total_stake)?).ok()
    }
}
...
pub fn redeem_rewards(
    ...
    credits_auto_rewind: bool,
    new_rate_activation_epoch: Option<Epoch>,
) -> Result<(u128, u128), InstructionError> {
...
fn calculate_split_rent_exempt_reserve(
    source_rent_exempt_reserve: u128,
    source_data_len: u128,
    split_data_len: u128,
) -> u128 {
...
mod tests {
    use {
        super::*,
        proptest::prelude::*,
        huione_program_runtime::with_mock_invoke_context,
        huione_sdk::{
    ...
    fn test_authorized_authorize() {
        let staker = huione_sdk::pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        let staker = huione_sdk::pubkey::new_rand();
        let custodian = huione_sdk::pubkey::new_rand();
        let invalid_custodian = huione_sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_stake_activating_and_deactivating() {
        ...
        let increment = (1_000_f64 * warmup_cooldown_rate(0, None))as u128;
        ...
        // huione in my initial deactivating amount, but don't huione in an entry for next
        stake_history.add(
    ...
    fn test_inflation_and_slashing_with_activating_and_deactivating_stake() {
        ...
        let (delegated_stake, mut stake, stake_history) = {
            ...
            let newly_effective_at_epoch1 = (cluster_stake as f64 * 0.25) as u128;
            ...
            let newly_effective_at_epoch2 =
                ((cluster_stake + newly_effective_at_epoch1) as f64 * 0.25) as u128;
            ...
            let newly_not_effective_stake_at_epoch5 =
                ((cluster_stake + delegated_stake) as f64 * 0.25) as u128;
        ...
        let adjust_staking_status = |rate: f64, status: &[StakeActivationStatus]| {
            status
                ...
                .map(|entry| StakeActivationStatus {
                    effective: (entry.effective as f64 * rate) as u128,
                    activating: (entry.activating as f64 * rate) as u128,
                    deactivating: (entry.deactivating as f64 * rate) as u128,
                })
                .collect::<Vec<_>>()
        };
        ...
        let rate = 1.10;
        stake.stake = (delegated_stake as f64 * rate) as u128;
    ...
    fn test_stop_activating_after_deactivation() {
        ...
        for epoch in 0..=stake.deactivation_epoch + 1 {
            let (activating, deactivating) = if epoch < stake.deactivation_epoch {
                ...
                let other_activation_sum: u128 = other_activations.iter().sum();
            ...
            let effective_rate_limited = (effective as f64 * warmup_cooldown_rate(0, None)) as u128;
        ...
        for epoch in 0..=stake.deactivation_epoch + 1 {
            ...
            let other_activations: u128 = other_activations[..=epoch as usize].iter().sum();
    ...
    fn test_stake_warmup_cooldown_sub_integer_moves() {
        ...
        let bootstrap = (warmup_cooldown_rate(0, None)  * 100.0 / 2.0) as u128;
        let stake_history =
    ...
    fn test_stake_warmup_cooldown(new_rate_activation_epoch: Option<Epoch>) {
        ...
        for epoch in 1..epochs {
            ...
            assert!(
                delta
                    <= ((prev_total_effective_stake as f64 * warmup_cooldown_rate(epoch, new_rate_activation_epoch)) as u128)
                        .max(1)
            );
    ...
    fn test_stake_state_redeem_rewards() {
        ...
        // huione 2 credits in at epoch 0
    ...
    fn test_stake_state_calculate_points_with_typical_values() {
        ...
        //  10_000_000 HC is a big but not unreasaonable stake
        let stake = new_stake(
            native_token::hc_to_lamports("10000000"),
            &Pubkey::default(),
            &vote_state,
            std::u64::MAX,
        );
        ...
        // huione 193,536,000 credits in at epoch 0, typical for a 14-day epoch
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            ...
            minimum_balance,
            minimum_balance as f64 / huione_sdk::native_token::LAMPORTS_PER_HC as f64
        );
    }
    ...
    fn test_calculate_lamports_per_byte_year() {
        ...
        assert_eq!(
            calculate_split_rent_exempt_reserve(rent_exempt_reserve, data_len as u128, data_len as u128),
            rent_exempt_reserve
        );
        ...
        assert_eq!(
            calculate_split_rent_exempt_reserve(rent_exempt_reserve, data_len as u128, larger_data as u128),
            larger_rent_exempt_reserve
        );
        assert_eq!(
            calculate_split_rent_exempt_reserve(larger_rent_exempt_reserve, larger_data as u128, data_len as u128),
            rent_exempt_reserve
        );

        let even_larger_data = huione_sdk::system_instruction::MAX_PERMITTED_DATA_LENGTH;
        let even_larger_rent_exempt_reserve = rent.minimum_balance(even_larger_data as usize);
        assert_eq!(
            calculate_split_rent_exempt_reserve(rent_exempt_reserve, data_len as u128, even_larger_data as u128),
            even_larger_rent_exempt_reserve
        );
        assert_eq!(
            calculate_split_rent_exempt_reserve(
                even_larger_rent_exempt_reserve,
                even_larger_data as u128,
                data_len as u128
            ),
            rent_exempt_reserve
        );
    ...
    fn test_things_can_merge() {
        ...
        let _identical = good_stake;
        let _bad_credits_observed = Stake {
    ...
    fn test_merge_kind_get_if_mergeable() {
        ...
        loop {
            clock.epoch += 1;
            let delta =
                activating.min((effective as f64 * warmup_cooldown_rate(clock.epoch, new_rate_activation_epoch)) as u128);
            effective += delta;
            activating -= delta;
            stake_history.add(
                clock.epoch,
        ...
        loop {
            clock.epoch += 1;
            let delta =
                deactivating.min((effective as f64 * warmup_cooldown_rate(clock.epoch, new_rate_activation_epoch)) as u128);
            effective -= delta;
            deactivating -= delta;
            stake_history.add(
                clock.epoch,
    ...
    fn test_active_stake_merge() {
        ...
        with_mock_invoke_context!(invoke_context, transaction_context, transaction_accounts);
        let clock = Clock::default();
        let delegation_a = 4_242_424_242u128;
        let delegation_b = 6_200_000_000u128;
        let credits_a = 124_521_000u128;
        let rent_exempt_reserve = 227_000_000u128;
        let meta = Meta {
            rent_exempt_reserve,
            ..Meta::default()
        };
        ...
        let delegation = 1_000_000u128;
        let credits_a = 200_000_000u128;
        let credits_b = 100_000_000u128;
        let rent_exempt_reserve = 227_000_000u128;
        let meta = Meta {
            rent_exempt_reserve,
            ..Meta::default()
    ...
    prop_compose! {
        pub fn sum_within(max: u128)(total in 1..max)
    ...
    proptest! {
        ...
        fn test_stake_weighted_credits_observed(
            (credits_a, credits_b) in sum_within(u128::MAX),
            (delegation_a, delegation_b) in sum_within(u128::MAX),
        ) {
            let stake = Stake {
                delegation: Delegation {
                    stake: delegation_a,
            ...
            // should be 1 or 0
            let weighted_credits_total =  U256::new(credits_observed) * U256::new(delegation_a + delegation_b);
            let weighted_credits_a = U256::new(credits_a)  * U256::new(delegation_a);
            let weighted_credits_b = U256::new(credits_b)  * U256::new(delegation_b);
            let raw_diff = weighted_credits_total - (weighted_credits_a + weighted_credits_b);
            let credits_observed_diff = raw_diff / U256::new(delegation_a + delegation_b);
