...
use {
    ...
    huione_gossip::gossip_service::discover_cluster,
    huione_ledger::{
...
pub const RUST_LOG_FILTER: &str =
    "error,huione_core::replay_stage=warn,huione_local_cluster=info,local_cluster=info";

pub const DEFAULT_CLUSTER_LAMPORTS: u128 = 10_000_000 * LAMPORTS_PER_HC;
pub const DEFAULT_NODE_STAKE: u128 = 10 * LAMPORTS_PER_HC;
...
pub fn run_cluster_partition<C>(
    ...
    let node_stakes: Vec<_> = partitions
        .iter()
        .map(|stake_weight| 100 * *stake_weight as u128)
        .collect();
    assert_eq!(node_stakes.len(), num_nodes);
    let cluster_lamports = node_stakes.iter().sum::<u128>() * 2;
...
pub fn test_faulty_node(
    ...
    node_stakes: Vec<u128>,
) -> (LocalCluster, Vec<Arc<Keypair>>) {
    huione_logger::setup_with_default("huione_local_cluster=info");
    ...
    let validator_to_slots = vec![(
        ...
        huione_sdk::clock::DEFAULT_DEV_SLOTS_PER_EPOCH as usize,
    )];
