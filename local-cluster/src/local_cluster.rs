use {
    ...
    itertools::izip,
    log::*,
    huione_client::{connection_cache::ConnectionCache, thin_client::ThinClient},
    huione_core::{
    ...
    huione_ledger::create_new_tmp_ledger,
    huione_runtime::{
        genesis_utils::{
...
impl LocalCluster {
    pub fn new_with_equal_stakes(
        num_nodes: usize,
        cluster_lamports: u128,
        lamports_per_node: u128,
    ...
    pub fn new(config: &mut ClusterConfig, socket_addr_space: SocketAddrSpace) -> Self {
        ...
        let (keys_in_genesis, stakes_in_genesis): (Vec<ValidatorVoteKeypairs>, Vec<u128>) =
            validator_keys
                .iter()
    ...
    pub fn transfer(&self, source_keypair: &Keypair, dest_pubkey: &Pubkey, lamports: u128) -> u128 {
        let (rpc, tpu) = LegacyContactInfo::try_from(&self.entry_point_info)
            .map(|node| {
                cluster_tests::get_client_facing_addr(self.connection_cache.protocol(), node)
            })
