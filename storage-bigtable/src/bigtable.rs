...
impl BigTableConnection {
    ...
    pub async fn huione_bincode_cells_with_retry<T>(
        &self,
        table: &str,
        cells: &[(RowKey, T)],
    ) -> Result<usize>
        ...
        T: serde::ser::Serialize,
    {
        retry(ExponentialBackoff::default(), || async {
            ...
            Ok(client.huione_bincode_cells(table, cells).await?)
        })
        .await
    }
    ...
    pub async fn huione_protobuf_cells_with_retry<T>(
        &self,
        table: &str,
        cells: &[(RowKey, T)],
    ) -> Result<usize>
        ...
        T: prost::Message,
    {
        retry(ExponentialBackoff::default(), || async {
            let mut client = self.client();
            Ok(client.huione_protobuf_cells(table, cells).await?)
        })
        .await
    }
}
...
impl<F: FnMut(Request<()>) -> InterceptedRequestResult> BigTable<F> {
    ...
    async fn huione_row_data(
        ...
        while let Some(res) = response.message().await? {
            for entry in res.entries {
                if let Some(status) = entry.status {
                    if status.code != 0 {
                        eprintln!("huione_row_data error {}: {}", status.code, status.message);
                        warn!("huione_row_data error {}: {}", status.code, status.message);
    ...
    pub async fn huione_bincode_cells<T>(
        &mut self,
        table: &str,
        cells: &[(RowKey, T)],
    ) -> Result<usize>
        ...
        T: serde::ser::Serialize,
    {
        ...
        self.huione_row_data(table, "x", &new_row_data).await?;
        Ok(bytes_written)
    }

    pub async fn huione_protobuf_cells<T>(
        &mut self,
        table: &str,
        cells: &[(RowKey, T)],
    ) -> Result<usize>
