...
pub const DEFAULT_INSTANCE_NAME: &str = "huione-ledger";
    ...
    pub async fn upload_confirmed_block(
        ...
        if !tx_cells.is_empty() {
            ...
            tasks.push(tokio::spawn(async move {
                conn.huione_bincode_cells_with_retry::<TransactionInfo>("tx", &tx_cells)
                    .await
            }));
        }
        ...
        if !tx_by_addr_cells.is_empty() {
            let conn = self.connection.clone();
            tasks.push(tokio::spawn(async move {
                conn.huione_protobuf_cells_with_retry::<tx_by_addr::TransactionByAddr>(
                    "tx-by-addr",
                    &tx_by_addr_cells,
                )
                .await
