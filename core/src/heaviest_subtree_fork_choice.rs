...
use {
    ...
    huione_runtime::{bank::Bank, bank_forks::BankForks, epoch_stakes::EpochStakes},
    huione_sdk::{
        clock::{Epoch, Slot},
...
impl UpdateOperation {
    fn update_stake(&mut self, new_stake: u128) {
        match self {
...
impl HeaviestSubtreeForkChoice {
    ...
    pub fn stake_voted_subtree(&self, key: &SlotHashKey) -> Option<u128> {
        self.fork_infos
            .get(key)
    ...
    pub fn all_slots_stake_voted_subtree(&self) -> impl Iterator<Item = (&SlotHashKey, u128)> {
        self.fork_infos
            .iter()
    ...
    pub fn stake_voted_at(&self, slot: &SlotHashKey) -> Option<u128> {
        self.fork_infos
            .get(slot)
    ...
    fn add_slot_stake(&mut self, slot_hash_key: &SlotHashKey, stake: u128) {
    ...
    fn subtract_slot_stake(&mut self, slot_hash_key: &SlotHashKey, stake: u128) {
        if let Some(fork_info) = self.fork_infos.get_mut(slot_hash_key) {
            fork_info.stake_voted_at -= stake;
            fork_info.stake_voted_subtree -= stake;
        }
    ...
    #[cfg(test)]
    fn set_stake_voted_at(&mut self, slot_hash_key: SlotHashKey, stake_voted_at: u128) {
        self.fork_infos
            .get_mut(&slot_hash_key)
            .unwrap()
...
mod test {
    use {
        ...
        itertools::Itertools,
        huione_runtime::{bank::Bank, bank_utils},
        huione_sdk::{hash::Hash, slot_history::SlotHistory},
    ...
    fn test_aggregate_slot() {
        ...
        for slot in &staked_voted_slots {
            heaviest_subtree_fork_choice.set_stake_voted_at((*slot, Hash::default()), *slot as u128);
            total_stake += *slot as u128;
        }
    ...
    fn test_add_votes_duplicate_then_outdated() {
        ...
        let expected_ancestors_stake = num_validators as u128 * stake;
        ...
        F: FnMut(Slot, &HeaviestSubtreeForkChoice) -> Slot,
    {
        ...
        for slot in slots_range {
            let expected_stake_voted_at =
                vote_count.get(&slot).cloned().unwrap_or(0) as u128 * stake;
            let expected_stake_voted_subtree =
                *num_voted_descendants.get(&slot).unwrap() as u128 * stake;
            assert_eq!(
