use {
    ...
    huione_runtime::{bank::Bank, epoch_stakes::NodeIdToVoteAccounts},
    huione_sdk::{
...
pub(crate) type SlotPubkeys = HashMap</*node:*/ Pubkey, /*stake:*/ u128>;
...
impl ClusterSlots {
    ...
    pub(crate) fn compute_weights(&self, slot: Slot, repair_peers: &[ContactInfo]) -> Vec<u128> {
...
#[cfg(test)]
mod tests {
    use {super::*, huione_runtime::epoch_stakes::NodeVoteAccounts};
    ...
    fn test_best_peer_2() {
        ...
        let k1 = huione_sdk::pubkey::new_rand();
        let k2 = huione_sdk::pubkey::new_rand();
        map.insert(k1, std::u128::MAX / 2);
        map.insert(k2, 0);
        cs.cluster_slots
            .write()
            .unwrap()
        ...
        c1.set_pubkey(k1);
        c2.set_pubkey(k2);
        assert_eq!(cs.compute_weights(0, &[c1, c2]), vec![std::u128::MAX / 4, 1]);
    }
    ...
    fn test_best_completed_slot_peer() {
        ...
        for ci in contact_infos.iter_mut() {
            ci.set_pubkey(huione_sdk::pubkey::new_rand());
        }
        let slot = 9;
