...
use {
    ...
    huione_metrics::inc_new_counter_error,
    huione_perf::packet::{Packet, PacketBatch},
...
impl WindowService {
    ...
    fn start_check_duplicate_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("HC-check-duplicate-error", 1, 1);
        };
        Builder::new()
            .name("huioneWinCheckDup".to_string())
            .spawn(move || {
    ...
    fn start_window_insert_thread(
        ...
        let handle_error = || {
            inc_new_counter_error!("HC-window-insert-error", 1, 1);
        };
        let thread_pool = rayon::ThreadPoolBuilder::new()
            .num_threads(get_thread_count().min(8))
            .thread_name(|i| format!("huioneWinInsert{i:02}"))
            .build()
            .unwrap();
        let reed_solomon_cache = ReedSolomonCache::default();
        Builder::new()
            .name("huioneWinInsert".to_string())
            .spawn(move || {
