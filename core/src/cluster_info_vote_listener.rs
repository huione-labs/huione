use {
    ...
    huione_ledger::blockstore::Blockstore,
...
impl ClusterInfoVoteListener {
    ...
    pub fn new(
        ...
        let listen_thread = {
            ...
            Builder::new()
                .name("huioneCiVoteLstnr".to_string())
                .spawn(move || {
        ...
        let bank_send_thread = Builder::new()
            .name("huioneCiBankSend".to_string())
            .spawn(move || {
        ...
        let send_thread = Builder::new()
            .name("huioneCiProcVotes".to_string())
            .spawn(move || {
    ...
    fn sum_stake(sum: &mut u128, epoch_stakes: Option<&EpochStakes>, pubkey: &Pubkey) {
...
mod tests {
    use {
        ...
        huione_perf::packet,
        huione_rpc::optimistically_confirmed_bank_tracker::OptimisticallyConfirmedBank,
        huione_runtime::{
            bank::Bank,
        ...
        huione_vote_program::{vote_state::Vote, vote_transaction},
        std::{
    ...
    fn test_update_new_root() {
        ...
        let new_voter = huione_sdk::pubkey::new_rand();
            ...
            for voting_keypairs in &validator_voting_keypairs {
                    ...
                    || vote_slot == *replay_vote_slots.last().unwrap()
                {
                    ...
                    assert_eq!(
                        ...
                        stake_per_validator * validator_voting_keypairs.len() as u128
                    );
                } else {
    ...
    fn test_process_votes2() {
        ...
        for (i, keyset) in validator_voting_keypairs.chunks(2).enumerate() {
            ...
            for voting_keypairs in keyset {
                ...
                assert_eq!(
                    optimistic_votes_tracker.stake(),
                    num_voters_per_slot as u128 * stake_per_validator
                );
            }
        }
    }
