...
pub struct ClusterNodes<T> {
    ...
    weighted_shuffle: WeightedShuffle</*stake:*/ u128>,
...
impl<T> ClusterNodes<T> {
    pub(crate) fn submit_metrics(&self, name: &'static str, now: u64) {
        ...
        datapoint_info!(
            name,
            ("epoch_stakes", epoch_stakes / LAMPORTS_PER_HC, i64),
            ...
            ("stake_dead", stake_dead / LAMPORTS_PER_HC, i64),
            ("stake_stale", stake_stale / LAMPORTS_PER_HC, i64),
        );
    }
}

impl ClusterNodes<BroadcastStage> {
    pub fn new(cluster_info: &ClusterInfo, stakes: &HashMap<Pubkey, u128>) -> Self {
...
pub fn new_cluster_nodes<T: 'static>(
    ...
    let stakes: Vec<u128> = nodes.iter().map(|node| node.stake).collect();
...
// All staked nodes + other known tvu-peers + the node itself;
// sorted by (stake, pubkey) in descending order.
fn get_nodes(cluster_info: &ClusterInfo, stakes: &HashMap<Pubkey, u128>) -> Vec<Node> {
    let self_pubkey = cluster_info.id();
...
pub fn make_test_cluster<R: Rng>(
    ...
    Vec<ContactInfo>,
    HashMap<Pubkey, u128>, // stakes
    ClusterInfo,
) {
    use huione_gossip::contact_info::ContactInfo;
    ...
    let mut stakes: HashMap<Pubkey, u128> = nodes
        .iter()
        .filter_map(|node| {
