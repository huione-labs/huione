...
#[frozen_abi(digest = "Dgt8RykgfrCf9gaAgCSBhd6pWwRwaKzXZnWggGPUYEEc")]
...
#[frozen_abi(digest = "C3rBVTzfaWucZYGkxNHA5tt4BAZmgZWoZNDqwyka8hJg")]
...
#[derive(Debug, AbiEnumVisitor, AbiExample, Deserialize, Serialize)]
#[frozen_abi(digest = "DVepn12WJVRWHFGKbCXfaWExVnAMtb7XFqHCfHHiCy1U")]
...
pub(crate) struct RepairPeers {
    ...
    weighted_index: WeightedIndex<u128>,
}

impl RepairPeers {
    fn new(asof: Instant, peers: &[ContactInfo], weights: &[u128]) -> Result<Self> {
        if peers.len() != weights.len() {
            return Err(Error::from(WeightedError::InvalidWeight));
        }
        let (peers, weights): (Vec<_>, Vec<u128>) = peers
            .iter()
            .zip(weights)
...
impl ServeRepair {
    ...
    fn decode_request(
        packet: &Packet,
        epoch_staked_nodes: &Option<Arc<HashMap<Pubkey, u128>>>,
    ...
    pub fn listen(
        ...
        Builder::new()
            .name("huioneRepairListen".to_string())
            .spawn(move || {
...
mod tests {
    use {
        ...
        huione_gossip::{contact_info::ContactInfo, socketaddr, socketaddr_any},
        huione_ledger::{
        ...
        huione_perf::packet::{deserialize_from_with_limit, Packet},
    ...
    fn test_serialize_deserialize_signed_request() {
        ...
        let repair_peer_id = huione_sdk::pubkey::new_rand();
    ...
    fn test_repair_with_repair_validators() {
        ...
        for pubkey in &[huione_sdk::pubkey::new_rand(), *me.pubkey()] {
