...
fn main() -> Result<(), Box<dyn error::Error>> {
    let matches = app(huione_version::version!())
        .try_get_matches()
    ...
    match subcommand {
        ("new", matches) => {
            ...
            let outfile = if matches.is_present("outfile") {
                ...
                path.extend([".config", "huione", key_type.default_file_name()]);
            ...
            match key_type {
                KeyType::ElGamal => {
                    ...
                    if let Some(outfile) = outfile {
                        outhuione_encodable_key(&elgamal_keypair, outfile, "new ElGamal keypair")
                ...
                KeyType::Aes128 => {
                    ...
                    if let Some(outfile) = outfile {
                        outhuione_encodable_key(&aes_key, outfile, "new AES128 key")
                            .map_err(|err| format!("Unable to write {outfile}: {err}"))?;
                    }
        ...
        ("recover", matches) => {
            ...
            match key_type {
                KeyType::ElGamal => {
                    ...
                    outhuione_encodable_key(&keypair, outfile, "recovered ElGamal keypair")?;
                }
                KeyType::Aes128 => {
                    ...
                    outhuione_encodable_key(&key, outfile, "recovered AES128 key")?;
                }
            }
        }
        _ => unreachable!(),
...
mod tests {
    ...
    fn process_test_command(args: &[&str]) -> Result<(), Box<dyn error::Error>> {
        ...
        let app_matches = app(huione_version).get_matches_from(args);
        do_main(&app_matches)
    }
    ...
    fn test_arguments() {
        ...
        app(huione_version).debug_assert();
    }
    ...
    fn test_pubkey() {
        ...
        process_test_command(&["huione-keygen", "pubkey", "elgamal", &keypair_path]).unwrap();
    }
}
