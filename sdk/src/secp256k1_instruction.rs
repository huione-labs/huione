...
//!
//! [np]: https://docs.huione.com/developing/runtime-facilities/programs#secp256k1-program
//!
...
//! [`ecrecover`] precompile. For that HC provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: huione_program::secp256k1_program
//! [`secp256k1_recover`]: huione_program::secp256k1_recover
...
//!
//! The `huione_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `huione_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the HC SDK.
//! This data structure is not provided to HC programs,
...
//!
//! **The huione `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. HC accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a HC program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a HC program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the HC program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The HC program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//! containing many signatures, and a HC program that parses them all. This
...
//! transaction. In the _previous_ example, the HC program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the HC program supports signatures, messages, and
...
//!         huione_sdk::secp256k1_program::ID,
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: huione_sdk::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active HC features. It is used to enable or
...
pub fn verify(
    ...
    for i in 0..count {
        ...
        let sig_parse_result = libsecp256k1::Signature::parse_standard_slice(
