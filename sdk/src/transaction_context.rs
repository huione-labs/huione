...
use {
    ...
    huione_program::entrypoint::MAX_PERMITTED_DATA_INCREASE,
    std::mem::MaybeUninit,
};
use {
    crate::{
...
impl<'a> BorrowedAccount<'a> {
    ...
    #[inline]
    pub fn get_lamports(&self) -> u128 {
        self.account.lamports()
    }

    /// Overwrites the number of lamports of this account (transaction wide)
    #[cfg(not(target_os = "solana"))]
    pub fn set_lamports(&mut self, lamports: u128) -> Result<(), InstructionError> {
        if self
            .transaction_context
            .is_early_verification_of_account_modifications_enabled()
        {
    ...
    #[cfg(not(target_os = "solana"))]
    pub fn checked_add_lamports(&mut self, lamports: u128) -> Result<(), InstructionError> {
        self.set_lamports(
            self.get_lamports()
    ...
    #[cfg(not(target_os = "solana"))]
    pub fn checked_sub_lamports(&mut self, lamports: u128) -> Result<(), InstructionError> {
        self.set_lamports(
            self.get_lamports()
