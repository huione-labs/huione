...
impl ClusterType {
    ...
    pub fn get_genesis_hash(&self) -> Option<Hash> {
        match self {
            Self::MainnetBeta => {
                Some(Hash::from_str("5C1kuxtWEJ9Pgd62VWmU4E2tQv5Fz5VPytE5sQaWCrxX").unwrap())
            }
            Self::Testnet => {
                Some(Hash::from_str("A13Z8oxfAKzKhAKoXupvipFcsRFajNLRgbBx38vif2wK").unwrap())
            }
            Self::Devnet => {
                Some(Hash::from_str("ABqAG4N9H4UWdnyPASx14BWAoauKHVY2dmkUvsrL1Q8p").unwrap())
            }
...
#[frozen_abi(digest = "FW7eiR76Ch5LssyFX5MB4yxaXBhjVcnGpNTrUhMyYULa")]
...
pub struct GenesisConfig {
    ...
    /// with the HC v0.23 release line
...
pub fn create_genesis_config(lamports: u128) -> (GenesisConfig, Keypair) {
            ...
            lamports_to_hc(
                self.accounts
                    ...
                    .map(|(pubkey, account)| {
                        ...
                        println!("key:{} hc:{}",pubkey.to_string(),lamports_to_hc(account.lamports));
                        account.lamports
                    })
                    .sum::<u128>()
            ),
...
mod tests {
    ...
    fn test_genesis_config() {
        ...
        config.add_native_instruction_processor("hi".to_string(), huione_sdk::pubkey::new_rand());
    ...
    // #[test]
    // oa test_show_genesis_config
    // fn test_show_genesis_config() {
    //     let path = Path::new("/root");
    //     let loaded_config = GenesisConfig::load(path).expect("load");
    //     println!("GenesisConfig:{}",loaded_config);
    //     println!("============================================");
    //     println!("Accounts:");
    //     let mut cnt = 0;
    //     let load_hash =  loaded_config.hash();
    //     loaded_config.accounts.into_iter().for_each(|(k, acc)| {
    //         //if acc.executable {
    //             cnt += 1;
    //             println!("{}: key:{} huione:{} : owner:{}",cnt,k,lamports_to_hc(acc.lamports),acc.owner.to_string());
    //         //}
    //     });
    //     assert_eq!(Hash::from_str("5eykt4UsFv8P8NJdTREpY1vzqKqZKvdpKuc147dw2N9d").unwrap(), load_hash);
    // }
}
