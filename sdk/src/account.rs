...
use {
    ...
    huione_program::{account_info::AccountInfo, debug_account_data::*, sysvar::Sysvar},
    std::{
        cell::{Ref, RefCell},
        fmt,
        mem::MaybeUninit,
...
#[repr(C)]
#[frozen_abi(digest = "13Mn1JunJmfqsZQwRL9cdZ33XA1VAuxcfmm8w8QtmFbw")]
...
pub trait WritableAccount: ReadableAccount {
    fn set_lamports(&mut self, lamports: u128);
    fn checked_add_lamports(&mut self, lamports: u128) -> Result<(), LamportsError> {
        self.set_lamports(
            self.lamports()
    ...
    fn checked_sub_lamports(&mut self, lamports: u128) -> Result<(), LamportsError> {
        self.set_lamports(
            self.lamports()
    ...
    fn saturating_add_lamports(&mut self, lamports: u128) {
        self.set_lamports(self.lamports().saturating_add(lamports))
    }
    fn saturating_sub_lamports(&mut self, lamports: u128) {
...
fn shared_new<T: WritableAccount>(lamports: u128, space: usize, owner: &Pubkey) -> T {
    T::create(
        lamports,
        vec![0u8; space],
        *owner,
...
impl Account {
    pub fn new(lamports: u128, space: usize, owner: &Pubkey) -> Self {
        shared_new(lamports, space, owner)
    }
    pub fn new_ref(lamports: u128, space: usize, owner: &Pubkey) -> Rc<RefCell<Self>> {
    ...
    pub fn new_rent_epoch(lamports: u128, space: usize, owner: &Pubkey, rent_epoch: Epoch) -> Self {
...
pub type InheritableAccountFields = (u128, Epoch);
...
pub fn create_account<S: Sysvar>(sysvar: &S, lamports: u128) -> Account {
...
pub fn create_account_with_fields<S: Sysvar>(
    ...
    let mut account = Account::new(lamports, data_len, &huione_program::sysvar::id());
...
pub fn create_account_shared_data<S: Sysvar>(sysvar: &S, lamports: u128) -> AccountSharedData {
...
impl huione_program::account_info::Account for Account {
    fn get(&mut self) -> (&mut u128, &mut [u8], &Pubkey, bool, Epoch) {
        (
            &mut self.lamports,
            &mut self.data,
            &self.owner,
...
pub mod tests {
    ...
    fn test_account_checked_add_lamports_overflow() {
        let key = Pubkey::new_unique();
        let (mut account1, _account2) = make_two_accounts(&key);
        account1.checked_add_lamports(u128::MAX).unwrap();
    }
    ...
    fn test_account_checked_sub_lamports_underflow() {
        let key = Pubkey::new_unique();
        let (mut account1, _account2) = make_two_accounts(&key);
        account1.checked_sub_lamports(u128::MAX).unwrap();
    }
    ...
    fn test_account_checked_add_lamports_overflow2() {
        let key = Pubkey::new_unique();
        let (_account1, mut account2) = make_two_accounts(&key);
        account2.checked_add_lamports(u128::MAX).unwrap();
    }
    ...
    fn test_account_checked_sub_lamports_underflow2() {
        let key = Pubkey::new_unique();
        let (_account1, mut account2) = make_two_accounts(&key);
        account2.checked_sub_lamports(u128::MAX).unwrap();
    }
    ...
    fn test_account_saturating_add_lamports() {
        ...
        let remaining = 22;
        account.set_lamports(u128::MAX - remaining);
        account.saturating_add_lamports(remaining * 2);
        assert_eq!(account.lamports(), u128::MAX);
    }
