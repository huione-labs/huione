...
impl FeeCalculator {
    ...
    pub fn calculate_fee(&self, message: &Message) -> u128 {
        let mut num_signatures: u128 = 0;
        for instruction in &message.instructions {
            ...
            if program_index < message.account_keys.len() {
                ...
                {
                    num_signatures += instruction.data[0] as u128;
                }
            }
        }

        self.lamports_per_signature
            * (u128::from(message.header.num_required_signatures) + num_signatures)
    }
}
...
pub struct FeeRateGovernor {
    ...
    // signatures
    pub target_lamports_per_signature: u128,
    ...
    pub target_signatures_per_slot: u128,

    pub min_lamports_per_signature: u128,
    pub max_lamports_per_signature: u128,

    // What portion of collected fees are to be destroyed, as a fraction of std::u8::MAX
    pub burn_percent: u8,
}

pub const DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE: u128 = 10_000;
pub const DEFAULT_TARGET_SIGNATURES_PER_SLOT: u128 = 50 * DEFAULT_MS_PER_SLOT as u128;
...
impl FeeRateGovernor {
    pub fn new(target_lamports_per_signature: u128, target_signatures_per_slot: u128) -> Self {
    ...
    pub fn new_derived(
        ...
        latest_signatures_per_slot: u128,
    ) -> Self {
        ...
        if me.target_signatures_per_slot > 0 {
                ...
                me.max_lamports_per_signature
                    .min(me.min_lamports_per_signature.max(
                        me.target_lamports_per_signature
                            * std::cmp::min(latest_signatures_per_slot, std::u32::MAX as u128)
                                as u128
                            / me.target_signatures_per_slot as u128,
                    ));
            ...
            let gap = desired_lamports_per_signature as i128
                - base_fee_rate_governor.lamports_per_signature as i128;
            ...
            if gap == 0 {
                ...
                let gap_adjust =
                    std::cmp::max(1, me.target_lamports_per_signature / 20) as i128 * gap.signum();
                ...
                me.lamports_per_signature =
                    me.max_lamports_per_signature
                        .min(me.min_lamports_per_signature.max(
                            (base_fee_rate_governor.lamports_per_signature as i128 + gap_adjust)
                                as u128,
                        ));
            }
        } else {
    ...
    pub fn clone_with_lamports_per_signature(&self, lamports_per_signature: u128) -> Self {
        Self {
            lamports_per_signature,
            ..*self
        }
    ...
    pub fn burn(&self, fees: u128) -> (u128, u128) {
        let burned = fees * u128::from(self.burn_percent) / 100;
        (fees - burned, burned)
    }
...
mod tests {
    ...
    fn test_fee_rate_governor_derived_adjust() {
        ...
        loop {
            ...
            f = FeeRateGovernor::new_derived(&f, std::u128::MAX);
