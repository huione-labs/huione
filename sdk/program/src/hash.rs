...
///
/// This struct is used most often in `huione-sdk` and related crates to contain
...
mod tests {
    ...
    fn test_hash_fromstr() {
        ...
        let inhuione_too_big = bs58::encode(&[0xffu8; HASH_BYTES + 1]).into_string();
        assert!(inhuione_too_big.len() > MAX_BASE58_LEN);
        assert_eq!(
            inhuione_too_big.parse::<Hash>(),
            Err(ParseHashError::WrongSize)
        );
