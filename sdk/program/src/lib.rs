//! The base library for all HC on-chain Rust programs.
//!
//! All HC Rust programs that run on-chain will link to this crate, which
//! acts as a standard library for HC programs. HC programs also link to
//! the [Rust standard library][std], though it is [modified][sstd] for the
//! HC runtime environment. While off-chain programs that interact with the
//! HC network _can_ link to this crate, they typically instead use the
//! [`huione-sdk`] crate, which reexports all modules from `huione-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://docs.huione.com/developing/on-chain-programs/developing-rust#restrictions
//! [`huione-sdk`]: https://docs.rs/huione-sdk/latest/huione_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `huione-program` usage can be found in
//! [the HC Program Library][spl].
//!
//! [spl]: https://github.com/huione-labs/huione-program-library
//!
//! # Defining a huione program
//!
//! HC program crates have some unique properties compared to typical Rust
//! programs:
//!
...
//!   library][sstd] in the HC documentation for more.
//!
...
//!
//! The skeleton of a HC program typically looks like:
//!
//! ```
...
//! ```
//!
//! Note that a HC program must specify its crate-type as "cdylib", and
//! "cdylib" crates will automatically be discovered and built by the `cargo
//! build-bpf` command. HC programs also often have crate-type "rlib" so
//! they can be linked to other Rust crates.
//!
//! # On-chain vs. off-chain compilation targets
//!
//! HC programs run on the [rbpf] VM, which implements a variant of the
...
//! example from the `huione-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/huione-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!
//! `huione-program` and `huione-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `huione-program` for two
...
//!
//! For a more complete description of HC's implementation of eBPF and its
//! limitations, see the main HC documentation for [on-chain programs][ocp].
//!
//! [ocp]: https://docs.huione.com/developing/on-chain-programs/overview
//!
//! # Core data types
//!
//! - [`Pubkey`] &mdash; The address of a [HC account][acc]. Some account
...
//!   even been disposed of. As running HC programs can not safely create or
//!   manage secret keys, the full [`Keypair`] is not defined in
//!   `huione-program` but in `huione-sdk`.
//! - [`Hash`] &mdash; A [SHA-256] hash. Used to uniquely identify blocks, and
//!   also for general purpose hashing.
//! - [`AccountInfo`] &mdash; A description of a single HC account. All accounts
...
//! - [`HC`] &mdash; The HC native token type, with conversions to and from
//!   [_lamports_], the smallest fractional unit of HC, in the [`native_token`]
//!   module.
//!
//! [acc]: https://docs.huione.com/developing/programming-model/accounts
...
//! [`Keypair`]: https://docs.rs/huione-sdk/latest/huione_sdk/signer/keypair/struct.Keypair.html
//! [SHA-256]: https://en.wikipedia.org/wiki/SHA-2
//! [`HC`]: native_token::HC
//! [_lamports_]: https://docs.huione.com/introduction#what-are-sols
//!
//! # Serialization
//!
//! Within the HC runtime, programs, and network, at least three different
//! serialization formats are used, and `huione-program` provides access to
//! those needed by programs.
//!
//! In user-written HC program code, serialization is primarily used for
...
//! [`Instruction`]: instruction::Instruction
//!
//! The three serialization formats in use in HC are:
//!
...
//!   re-exported by `huione-program`, though this crate provides several useful
...
//!
//! - __[`Pack`]__, a HC-specific serialization API that is used by many
//!   older programs in the [HC Program Library][spl] to define their
...
//!
//! HC programs may call other programs, termed [_cross-program
...
//! [cpi]: https://docs.huione.com/developing/programming-model/calling-between-programs
//!
...
//! ```
//!
//! HC also includes a mechinasm to let programs control and sign for
...
//! PDA.
//!
//! [pdas]: https://docs.huione.com/developing/programming-model/calling-between-programs#program-derived-addresses
...
//! # Native programs
//!
//! Some huione programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.huione.com/developing/runtime-facilities/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/huione-sdk/latest/huione_sdk/transaction/struct.Transaction.html
//!
//! This crate defines the program IDs for most native programs. Even though
//! some native programs cannot be invoked by other programs, a HC program
...
//! active on any particular network. The `huione feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://docs.huione.com/terminology#slot
//!
//! Native programs important to HC program authors include:
//!
...
//!   - ID: [`huione_program::system_program`]
//!   - Instruction: [`huione_program::system_instruction`]
...
//!   program.
//!   - ID: [`huione_sdk::compute_budget`](https://docs.rs/huione-sdk/latest/huione_sdk/compute_budget/index.html)
//!   - Instruction: [`huione_sdk::compute_budget`](https://docs.rs/huione-sdk/latest/huione_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`huione_program::ed25519_program`]
//!   - Instruction: [`huione_sdk::ed25519_instruction`](https://docs.rs/huione-sdk/latest/huione_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`huione_program::secp256k1_program`]
//!   - Instruction: [`huione_sdk::secp256k1_instruction`](https://docs.rs/huione-sdk/latest/huione_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`huione_program::bpf_loader`]
//!   - Instruction: [`huione_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`huione_program::bpf_loader_upgradeable`]
//!   - Instruction: [`huione_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`huione_program::bpf_loader_deprecated`]
//!   - Instruction: [`huione_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.huione.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]
#![cfg_attr(RUSTC_NEEDS_PROC_MACRO_HYGIENE, feature(proc_macro_hygiene))]

// Allows macro expansion of `use ::huione_program::*` to work within this crate
extern crate self as huione_program;
...
#[cfg(target_os = "solana")]
pub use huione_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.huione.com/developing/runtime-facilities/programs#config-program
pub mod config {
    pub mod program {
...
pub mod sdk_ids {
    use {
        crate::{
            ...
            feature, incinerator, secp256k1_program, huione_program::pubkey::Pubkey, stake,
...
pub use huione_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use huione_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
...
/// assert_eq!(id(), my_id);
/// ```
pub use huione_sdk_macro::program_declare_id as declare_id;
...
/// ```
/// use std::str::FromStr;
/// use huione_program::{pubkey, pubkey::Pubkey};
///
...
/// assert_eq!(ID, my_id);
/// ```
pub use huione_sdk_macro::program_pubkey as pubkey;

#[macro_use]
extern crate serde_derive;

#[macro_use]
extern crate huione_frozen_abi_macro;
...
///
/// ```compile_fail
/// # use huione_program::unchecked_div_by_const;
/// # fn main() {
...
// `huione_program`'s top-level modules, and `huione_sdk`'s glob re-export of
// `huione_program`'s top-level modules, if this module is not lexically last
