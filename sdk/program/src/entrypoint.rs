...
/// errors. To avoid this, it is common for huione programs to define an
...
/// it is common to huione it into its own file.
///
/// ```no_run
...
/// [BPF]: https://docs.huione.com/developing/on-chain-programs/overview#berkeley-packet-filter-bpf
///
/// # Cargo features
///
...
///
/// # Defining a panic handler for huione
///
/// _The mechanism for defining a huione panic handler is different [from most
...
/// ```
///
/// The above is how huione defines the default panic handler.
#[macro_export]
...
pub unsafe fn deserialize<'a>(input: *mut u8) -> (&'a Pubkey, Vec<AccountInfo<'a>>, &'a [u8]) {
    ...
    for _ in 0..num_accounts {
        ...
        if dup_info == NON_DUP_MARKER {
            ...
            #[allow(clippy::cast_ptr_alignment)]
            let lamports = Rc::new(RefCell::new(&mut *(input.add(offset) as *mut u128)));
            offset += size_of::<u128>();
