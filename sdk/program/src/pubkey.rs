...
/// [`Keypair`]: https://docs.rs/solana-sdk/latest/huione_sdk/signer/keypair/struct.Keypair.html
#[wasm_bindgen]
#[repr(transparent)]
#[derive(
    AbiExample,
...
impl Pubkey {
    ...
    ///
    /// [pda]: https://docs.huione.com/developing/programming-model/calling-between-programs#program-derived-addresses
    ///
    ...
    /// as a HC `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain HC program must be
    ...
    /// program, which invokes an on-chain HC program that uses the address
    ...
    /// instruction as a HC transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain HC program:
    ///
    /// ```
    ...
    /// ```
    /// # use borsh::{BorshSerialize, BorshDeserialize};
    /// # use huione_program::example_mocks::{huione_sdk, huione_client};
    /// # use huione_program::{
    /// #     pubkey::Pubkey,
