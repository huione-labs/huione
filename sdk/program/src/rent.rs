...
//!
//! [rent]: https://docs.huione.com/implemented-proposals/rent

#![allow(clippy::integer_arithmetic)]

use {crate::clock::DEFAULT_SLOTS_PER_EPOCH, huione_sdk_macro::CloneZeroed};
...
/// - 10^9 lamports per HC
/// - $1 per HC
/// - $0.01 per megabyte day
/// - $3.65 per megabyte year
pub const DEFAULT_LAMPORTS_PER_BYTE_YEAR: u128 = 6_000_000_000 / 100 * 365 / (1024 * 1024);
...
pub const ACCOUNT_STORAGE_OVERHEAD: u128 = 128;
...
impl Rent {
    ...
    pub fn calculate_burn(&self, rent_collected: u128) -> (u128, u128) {
        let burned_portion = (rent_collected * u128::from(self.burn_percent)) / 100;
    ...
    pub fn minimum_balance(&self, data_len: usize) -> u128 {
        let bytes = data_len as u128;
        (((ACCOUNT_STORAGE_OVERHEAD + bytes) * self.lamports_per_byte_year) as f64
            * self.exemption_threshold) as u128
    }

    /// Whether a given balance and data length would be exempt.
    pub fn is_exempt(&self, balance: u128, data_len: usize) -> bool {
        // println!("{} bytes exempt lamports {}.",data_len,self.minimum_balance(data_len));
        balance >= self.minimum_balance(data_len)
    }

    /// Rent due on account's data length with balance.
    pub fn due(&self, balance: u128, data_len: usize, years_elapsed: f64) -> RentDue {
    ...
    pub fn due_amount(&self, data_len: usize, years_elapsed: f64) -> u128 {
        let actual_data_len = data_len as u128 + ACCOUNT_STORAGE_OVERHEAD;
        let lamports_per_year = self.lamports_per_byte_year * actual_data_len;
        (lamports_per_year as f64 * years_elapsed) as u128
    }
    ...
    pub fn with_slots_per_epoch(slots_per_epoch: u64) -> Self {
        ...
        let lamports_per_byte_year = (DEFAULT_LAMPORTS_PER_BYTE_YEAR as f64 / ratio) as u128;
        Self {
        ...
        assert_eq!(
            default_rent.due(
                (((2 + ACCOUNT_STORAGE_OVERHEAD) * DEFAULT_LAMPORTS_PER_BYTE_YEAR) as f64
                    * DEFAULT_EXEMPTION_THRESHOLD) as u128,
                2,
                1.2
            ),
            RentDue::Exempt,
        ...
        assert_eq!(
            custom_rent.due(
                (((2 + ACCOUNT_STORAGE_OVERHEAD) * custom_rent.lamports_per_byte_year) as f64
                    * custom_rent.exemption_threshold) as u128,
                2,
                1.2
            ),
            RentDue::Exempt
