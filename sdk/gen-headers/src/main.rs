...
fn transform(inc: &PathBuf) {
    ...
    let mut inhuione_content = vec![];
    input.read_to_end(&mut inhuione_content).unwrap();
    let inhuione_content = str::from_utf8(&inhuione_content).unwrap();
    let output = match fs::File::create(&header_path) {
        Ok(x) => x,
        Err(err) => panic!("Failed to create {}: {}", header_path.display(), err),
    };
    let mut outhuione_writer = BufWriter::new(output);
    let decl_re =
        Regex::new(r"@SYSCALL ([0-9A-Za-z_*]+)[[:space:]]+(sol_[0-9A-Za-z_]+)\(([^);]*)\);")
            .unwrap();
    let comm_re = Regex::new(r",").unwrap();
    let outhuione_content = decl_re.replace_all(inhuione_content, |caps: &Captures| {
    ...
    write!(outhuione_writer, "{outhuione_content}").unwrap();
}
