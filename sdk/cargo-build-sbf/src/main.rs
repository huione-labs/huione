use {
    ...
    log::*,
    regex::Regex,
    huione_download_utils::download_file,
    huione_sdk::signature::{write_keypair_file, Keypair},
    std::{
        borrow::Cow,
...
fn find_installed_platform_tools() -> Vec<String> {
    ...
    let huione = home_dir.join(".cache").join("huione");
    let package = "platform-tools";
    std::fs::read_dir(huione)
        .unwrap()
...
fn install_if_missing(
    ...
    // Check whether the package is already in ~/.cache/huione.
...
fn link_huione_toolchain(config: &Config) {
    ...
    for line in rustup_output.lines() {
        if line.starts_with("huione") {
            let mut it = line.split_whitespace();
            let _ = it.next();
            let path = it.next();
            if path.unwrap() != toolchain_path.to_str().unwrap() {
                let rustup_args = vec!["toolchain", "uninstall", "huione"];
                let output = spawn(
                    &rustup,
                    rustup_args,
                    config.generate_child_script_on_failure,
...
fn build_huione_package(
    ...
    let legacy_program_feature_present = package.name == "huione-sdk";
    ...
    info!("Huione SDK: {}", config.sbf_sdk.display());
    ...
    if env::var("RUSTC").is_ok() {
        warn!(
            "Removed RUSTC from cargo environment, because it overrides +huione cargo command line option."
        );
        env::remove_var("RUSTC")
    }
    let cargo_target = "CARGO_TARGET_SBF_HUIONE_HUIONE_RUSTFLAGS";
    ...
    if let Some(program_name) = program_name {
        ...
        info!("  $ huione program deploy {}", program_so.display());
...
fn build_hc(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
        if !config.workspace {
            build_huione_package(&config, metadata.target_directory.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        build_huione_package(&config, metadata.target_directory.as_ref(), package);
    }
}
...
fn main() {
    ...
    let matches = clap::Command::new(crate_name!())
        ...
        .arg(
            Arg::new("sbf_sdk")
                ...
                .help("Path to the Huione SBF SDK"),
        )
        .arg(
            Arg::new("cargo_args")
    ...
    let config = Config {
        ...
        sbf_sdk: fs::canonicalize(&sbf_sdk).unwrap_or_else(|err| {
            error!(
                "Huione SDK path does not exist: {}: {}",
                sbf_sdk.display(),
                err
            );
            exit(1);
