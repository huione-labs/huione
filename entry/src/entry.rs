...
lazy_static! {
    static ref PAR_THREAD_POOL: ThreadPool = rayon::ThreadPoolBuilder::new()
        ...
        .thread_name(|i| format!("huioneEntry{i:02}"))
        .build()
        .unwrap();
}
...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = huione_perf::perf_libs::locate_perf_libs()
            {
                huione_perf::perf_libs::append_to_ld_library_path(
...
///
/// * For TPU: `huione_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `huione_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `huione_runtime::accounts::Accounts::lock_accounts()`
...
impl EntrySlice for [Entry] {
    ...
    fn verify_cpu_x86_simd(&self, start_hash: &Hash, simd_len: usize) -> EntryVerificationState {
        use huione_sdk::hash::HASH_BYTES;
        let now = Instant::now();
        let genesis = [Entry {
            num_hashes: 0,
            hash: *start_hash,
...
mod tests {
    use {
        super::*,
        huione_perf::test_tx::{test_invalid_tx, test_tx},
        huione_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
    ...
    fn test_transaction_signing() {
        use huione_sdk::signature::Signature;
        let zero = Hash::default();
