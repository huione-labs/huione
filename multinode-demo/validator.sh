...
airdrops_enabled=1
node_hc=500 # 500 HC: number of HC to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-huione HC            - Number of HC this node has been funded from the genesis config (default: $node_hc)
...
while [[ -n $1 ]]; do
  if [[ ${1:0:1} = - ]]; then
    ...
    if [[ $1 = --label ]]; then
      ...
      no_restart=1
      shift
    elif [[ $1 = --node-huione ]]; then
      node_hc="$2"
      shift 2
    elif [[ $1 = --no-airdrop ]]; then
      airdrops_enabled=0
      shift
    # huione-validator options
...
if [[ "$HUIONE_GPU_MISSING" -eq 1 ]]; then
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$HUIONE_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $HUIONE_CUDA ]]; then
  program=$huione_validator_cuda
else
  program=$huione_validator
fi
...
wallet() {
  (
    set -x
    $huione_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
setup_validator_accounts() {
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      echo "Adding $node_hc to validator identity account:"
      (
        set -x
        $huione_cli \
          --keypair "$HUIONE_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_hc"
      ) || return $?
    fi
...
rpc_url=$($huione_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $huione_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $huione_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $huione_keygen new --no-passphrase -so "$authorized_withdrawer"

setup_validator_accounts "$node_hc"
