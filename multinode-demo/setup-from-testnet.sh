...
rm -rf "$HUIONE_CONFIG_DIR"/latest-testnet-snapshot
mkdir -p "$HUIONE_CONFIG_DIR"/latest-testnet-snapshot
(
  cd "$HUIONE_CONFIG_DIR"/latest-testnet-snapshot || exit 1
  set -x
  wget http://test_rpc.huione.com/genesis.tar.bz2
  wget --trust-server-names http://testnet.huione.com/snapshot.tar.bz2
)

snapshot=$(ls "$HUIONE_CONFIG_DIR"/latest-testnet-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$huione_ledger_tool create-snapshot \
  --ledger "$HUIONE_CONFIG_DIR"/latest-testnet-snapshot \
