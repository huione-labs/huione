...
use {
    ...
    huione_gossip::gossip_service::{discover_cluster, get_client, get_multi_client},
...
fn find_node_activated_stake(
    ...
    let total_active_stake: u128 = vote_accounts
        .current
        .iter()
...
fn create_connection_cache(
    ...
    let staked_nodes = Arc::new(RwLock::new(StakedNodes::new(
        Arc::new(stakes),
        HashMap::<Pubkey, u128>::default(), // overrides
    )));
...
fn main() {
    ...
    huione_metrics::set_panic_hook("bench-tps", /*version:*/ None);

    let matches = cli::build_args(huione_version::version!()).get_matches();
    ...
    if *write_to_client_file {
        ...
        let num_accounts = keypairs.len() as u128;
        let max_fee = FeeRateGovernor::new(*target_lamports_per_signature, 0)
            .max_lamports_per_signature
            .saturating_add(max_lamports_for_prioritization(
                *use_randomized_compute_unit_price,
            ));
        let num_lamports_per_account =( (num_accounts - 1 + NUM_SIGNATURES_FOR_TXS as u128 * max_fee)
            / num_accounts) as u128
            + num_lamports_per_account;
