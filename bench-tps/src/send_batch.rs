...
pub fn generate_keypairs(seed_keypair: &Keypair, count: u64) -> (Vec<Keypair>, u64) {
    ...
    while total_keys < count {
        ...
        total_keys += delta as u64;
    }
    (rnd.gen_n_keypairs(total_keys), extra as u64)
}
...
pub fn fund_keys<T: 'static + BenchTpsClient + Send + Sync + ?Sized>(
    ...
    while !not_funded.is_empty() {
        ...
        let mut to_fund: Vec<(&Keypair, Vec<(Pubkey, u128)>)> = vec![];
...
const MAX_SPENDS_PER_TX: u128 = 4;
...
trait SendBatchTransactions<'a, T: Sliceable + Send + Sync> {
    ...
    fn send_transactions<C, F>(&mut self, client: &Arc<C>, to_lamports: u128, log_progress: F)
    where
...
type FundingChunk<'a> = [(FundingSigners<'a>, Vec<(Pubkey, u128)>)];
