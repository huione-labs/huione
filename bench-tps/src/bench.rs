use {
    ...
    rayon::prelude::*,
    huione_client::{nonce_utils, rpc_request::MAX_MULTIPLE_ACCOUNTS},
    huione_metrics::{self, datapoint_info},
    huione_sdk::{
        account::Account,
...
const MAX_COMPUTE_UNIT_PRICE: u128 = 50;
...
pub fn max_lamports_for_prioritization(use_randomized_compute_unit_price: bool) -> u128 {
    if use_randomized_compute_unit_price {
        const MICRO_LAMPORTS_PER_LAMPORT: u128 = 1_000_000;
        let micro_lamport_fee: U256 = U256::new(MAX_COMPUTE_UNIT_PRICE)
            .saturating_mul(U256::new(TRANSFER_TRANSACTION_COMPUTE_UNIT as u128));
        let fee = micro_lamport_fee
            .saturating_add(U256::new(MICRO_LAMPORTS_PER_LAMPORT.saturating_sub(1)))
            .saturating_div(U256::new(MICRO_LAMPORTS_PER_LAMPORT as u128));
    ...
    T: 'static + BenchTpsClient + Send + Sync + ?Sized,
{
    ...
    Builder::new()
        .name("huione-client-sample".to_string())
        .spawn(move || {
    ...
    T: 'static + BenchTpsClient + Send + Sync + ?Sized,
{
    (0..threads)
        .map(|_| {
            ...
            let client = client.clone();
            Builder::new()
                .name("huione-client-sender".to_string())
                .spawn(move || {
                    do_tx_transfers(
                        &exit_signal,
                        &shared_txs,
    ...
    T: 'static + BenchTpsClient + Send + Sync + ?Sized,
{
    ...
    let blockhash_thread = if !use_durable_nonce {
        ...
        Some(
            Builder::new()
                .name("huione-blockhash-poller".to_string())
                .spawn(move || {
...
fn metrics_submit_lamport_balance(lamport_balance: u128) {
    info!("Token balance: {}", lamport_balance);
    datapoint_info!(
        "bench-tps-lamport_balance",
        ("balance", lamport_balance, i128)
    );
}
...
fn transfer_with_compute_unit_price_and_padding(
    ...
    _instruction_padding_config: &Option<InstructionPaddingConfig>,
...
pub fn fund_keypairs<T: 'static + BenchTpsClient + Send + Sync + ?Sized>(
    ...
    // Sample the first keypair, to prevent lamport loss on repeated huione-bench-tps executions
    ...
    if first_keypair_balance < enough_lamports || last_keypair_balance < enough_lamports {
        ...
        let extra_fees = extra as u128 * max_fee;
        let total_keypairs = keypairs.len() as u64 + 1; // Add one for funding keypair
        let total = lamports_per_account * total_keypairs as u128 + extra_fees;
...
mod tests {
    use {
        super::*,
        huione_runtime::{bank::Bank, bank_client::BankClient},
        huione_sdk::{
            commitment_config::CommitmentConfig, fee_calculator::FeeRateGovernor,
            genesis_config::create_genesis_config, native_token::hc_to_lamports, nonce::State,
        },
    };

    #[test]
    fn test_bench_tps_bank_client() {
        let (genesis_config, id) = create_genesis_config(hc_to_lamports("10000.0"));
    ...
    fn test_bench_tps_fund_keys_with_fees() {
        let (mut genesis_config, id) = create_genesis_config(hc_to_lamports("10000.0"));
