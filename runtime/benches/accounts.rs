...
fn deposit_many(bank: &Bank, pubkeys: &mut Vec<Pubkey>, num: usize) -> Result<(), LamportsError> {
    for t in 0..num {
        ...
        let account =
            AccountSharedData::new((t + 1) as u128, 0, AccountSharedData::default().owner());
        pubkeys.push(pubkey);
        assert!(bank.get_account(&pubkey).is_none());
        bank.deposit(&pubkey, (t + 1) as u128)?;
...
fn bench_delete_dependencies(bencher: &mut Bencher) {
    ...
    for i in 0..1000 {
        ...
        accounts.store_slow_uncached(i as u64, &pubkey, &account);
        accounts.store_slow_uncached(i as u64, &old_pubkey, &zero_account);
        old_pubkey = pubkey;
    ...
    F: Fn(&Accounts, &[Pubkey]) + Send + Copy,
{
    ...
    bencher.iter(|| {
        for account in &new_accounts {
            ...
            accounts.store_slow_uncached(slot + 1, &huione_sdk::pubkey::new_rand(), account);
        }
    })
}
