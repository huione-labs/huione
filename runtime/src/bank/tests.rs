#![cfg(test)]
#[allow(deprecated)]
use huione_sdk::sysvar::fees::Fees;
use {
    super::{
    ...
    huione_sdk::{
        ...
        native_loader,
        native_token::{hc_to_lamports, LAMPORTS_PER_HC},
    ...
    huione_stake_program::stake_state::{self, StakeState},
    huione_vote_program::{
        vote_instruction,
        vote_state::{
...
fn test_race_register_tick_freeze() {
    ...
    let p = huione_sdk::pubkey::new_rand();
...
fn test_bank_new() {
    let dummy_leader_pubkey = huione_sdk::pubkey::new_rand();
...
fn create_simple_test_bank(lamports: u128) -> Bank {
    let (genesis_config, _mint_keypair) = create_genesis_config(lamports);
    Bank::new_for_tests(&genesis_config)
}

fn create_simple_test_arc_bank(lamports: u128) -> Arc<Bank> {
...
fn bank0_sysvar_delta() -> u128 {
    const SLOT_HISTORY_SYSVAR_MIN_BALANCE: u128 =  5_481_268_250; //913_326_000;
    SLOT_HISTORY_SYSVAR_MIN_BALANCE
}

fn bank1_sysvar_delta() -> u128 {
    const SLOT_HASHES_SYSVAR_MIN_BALANCE: u128 = 861_130_320;//143_487_360;
...
fn test_bank_capitalization() {
    ...
    assert_eq!(
        ...
        42 * 42 + genesis_sysvar_and_builtin_program_lamports() + 3,
    );
    ...
    assert_eq!(
        bank0.capitalization(),
        42 * 42 + genesis_sysvar_and_builtin_program_lamports() + bank0_sysvar_delta() as u128 +3,
    );
    ...
    assert_eq!(
        ...
        42 * 42
            ...
            + bank0_sysvar_delta() as u128
            + bank1_sysvar_delta() as u128 +3,
    );
}
...
fn store_accounts_for_rent_test(
    ...
    generic_rent_due_for_system_account: u128,
) {
...
fn assert_capitalization_diff(
    bank: &Bank,
    updater: impl Fn(),
    asserter: impl Fn(u128, u128) -> bool,
) {
...
fn test_rent_distribution() {
    ...
    let bootstrap_validator_pubkey = huione_sdk::pubkey::new_rand();
    ...
    let validator_1_pubkey = huione_sdk::pubkey::new_rand();
    ...
    let validator_2_pubkey = huione_sdk::pubkey::new_rand();
    ...
    let validator_3_pubkey = huione_sdk::pubkey::new_rand();
    ...
    let burned_portion =
        total_rent_deducted * u128::from(bank.rent_collector.rent.burn_percent) / 100;
    let rent_to_be_distributed = total_rent_deducted - burned_portion;

    let bootstrap_validator_portion =
        ((bootstrap_validator_stake_lamports * rent_to_be_distributed) as f64 / 100.0) as u128 + 1; // Leftover lamport
    assert_eq!(
    ...
    let tweak_1 = u64::from(validator_1_pubkey > validator_2_pubkey) as u128;
    let validator_1_portion =
        ((validator_1_stake_lamports * rent_to_be_distributed) as f64 / 100.0) as u128 + tweak_1;
    assert_eq!(
    ...
    let validator_2_portion =
        ((validator_2_stake_lamports * rent_to_be_distributed) as f64 / 100.0) as u128 + tweak_2 as u128;
    assert_eq!(
        bank.get_balance(&validator_2_pubkey),
        validator_2_portion + 42 - tweak_2 as u128,
    );

    let validator_3_portion =
        ((validator_3_stake_lamports * rent_to_be_distributed) as f64 / 100.0) as u128 + 1;
    assert_eq!(
            ...
            .map(|(address, reward)| {
                if reward.lamports > 0 {
                    ...
                    if *address == validator_2_pubkey {
                        assert_eq!(reward.post_balance, validator_2_portion + 42 - tweak_2 as u128);
                    } else if *address == validator_3_pubkey {
                        assert_eq!(reward.post_balance, validator_3_portion + 42);
                    }
                    reward.lamports as  u128
                } else {
                    0
                }
            })
...
fn test_distribute_rent_to_validators_overflow() {
    ...
    const RENT_TO_BE_DISTRIBUTED: u128 = 120_525;
    const VALIDATOR_STAKE: u128 = 340_282_366_920_938_463_463_374_607_431_768_211_455u128;

    let validator_pubkey = huione_sdk::pubkey::new_rand();
    let genesis_config =
...
fn test_distribute_rent_to_validators_rent_paying() {
    ...
    const RENT_PER_VALIDATOR: u128 = 55;
    const TOTAL_RENT: u128 = RENT_PER_VALIDATOR * 4;
    ...
    for deactivate_feature in [true] {
        ...
        assert_eq!(
            if deactivate_feature {
                old_empty_validator_lamports + RENT_PER_VALIDATOR as u128
            } else {
        ...
        assert_eq!(
            if deactivate_feature {
                old_rent_paying_validator_lamports + RENT_PER_VALIDATOR as u128
            } else {
        ...
        assert_eq!(
            old_becomes_rent_exempt_validator_lamports + RENT_PER_VALIDATOR as u128,
            new_becomes_rent_exempt_validator_lamports
        );

        assert_eq!(
            old_rent_exempt_validator_lamports + RENT_PER_VALIDATOR as u128,
        ...
        assert_eq!(
            if deactivate_feature {
                RentState::RentPaying {
                    lamports: RENT_PER_VALIDATOR as u128,
                    data_size: 0,
                }
            } else {
                RentState::Uninitialized
        ...
        assert_eq!(
            if deactivate_feature {
                RentState::RentPaying {
                    lamports: old_rent_paying_validator_lamports + RENT_PER_VALIDATOR as u128,
                    data_size: 0,
                }
            } else {
                RentState::RentPaying {
...
fn test_rent_exempt_executable_account() {
    ...
    let account_pubkey = huione_sdk::pubkey::new_rand();
    let account_balance = 1;
    let mut account = AccountSharedData::new(account_balance, 0, &huione_sdk::pubkey::new_rand());
...
fn test_rent_eager_across_epoch_without_gap_under_multi_epoch_cycle() {
    let leader_pubkey = huione_sdk::pubkey::new_rand();
...
fn test_rent_eager_collect_rent_in_partition() {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != huione_sdk::feature_set::set_exempt_rent_epoch_max::id() {
            activate_feature(&mut genesis_config, feature_id);
        }
    }

    let zero_lamport_pubkey = huione_sdk::pubkey::new_rand();
    let rent_due_pubkey = huione_sdk::pubkey::new_rand();
    let rent_exempt_pubkey = huione_sdk::pubkey::new_rand();
    let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
    let zero_lamports = 0;
    ...
    let rent_collected = 12; // this is a function of 'some_slot'
    ...
    F: Fn(&Bank) -> LoadVoteAndStakeAccountsResult,
{
    ...
    assert_eq!(
        bank0.capitalization(),
        42 * 1_000_000_000 + genesis_sysvar_and_builtin_program_lamports() + 3,
    );
    ...
    assert_eq!(
        ...
        42 * 1_000_000_000
            ...
            + bank0_sysvar_delta() as u128 + 4_567_942_253,
    );
    assert!(bank0.rewards.read().unwrap().is_empty());

    load_vote_and_stake_accounts(&bank0);

    // huione a child bank in epoch 1, which calls update_rewards()...
    ...
    let paid_rewards = bank1.capitalization() - bank0.capitalization() - bank1_sysvar_delta() as u128;
        ...
        vec![
            ...
            (
                ...
                RewardInfo {
                    ...
                    lamports: validator_rewards as i128,
...
fn do_test_bank_update_rewards_determinism() -> u128 {
    // create a bank that ticks really slowly...
    ...
    let vote_id = huione_sdk::pubkey::new_rand();
    let mut vote_account =
        vote_state::create_account(&vote_id, &huione_sdk::pubkey::new_rand(), 0, 100);
    let stake_id1 = huione_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = huione_sdk::pubkey::new_rand();
...
fn test_purge_empty_accounts() {
    ...
    for pass in 0..3 {
        huione_logger::setup();
        let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("1."));
...
fn test_one_source_two_tx_one_batch() {
    let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("1."));
    let key1 = huione_sdk::pubkey::new_rand();
    let key2 = huione_sdk::pubkey::new_rand();
    let bank = Bank::new_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
    assert_eq!(bank.last_blockhash(), genesis_config.hash());
...
fn test_insufficient_funds() {
    let mint_amount = hc_to_lamports("1.");
...
fn test_transfer_to_sysvar() {
    huione_logger::setup();
    let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports(10_000.to_string().as_ref()));
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    let amount = genesis_config.rent.minimum_balance(0);

    let normal_pubkey = huione_sdk::pubkey::new_rand();
    let sysvar_pubkey = sysvar::clock::id();
    assert_eq!(bank.get_balance(&normal_pubkey), 0);
    assert_eq!(bank.get_balance(&sysvar_pubkey), 7_017_360);
...
fn test_bank_deposit() {
    ...
    // Test new account
    let key = huione_sdk::pubkey::new_rand();
...
fn test_bank_tx_fee() {
    ...
    let leader = huione_sdk::pubkey::new_rand();
    let GenesisConfigInfo {
        mut genesis_config,
        mint_keypair,
        ..
        ...
        *bank.rewards.read().unwrap(),
        vec![(
            ...
            RewardInfo {
                ...
                lamports: expected_fee_collected as i128,
...
fn test_debits_before_credits() {
    let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("2."));
    let bank = Bank::new_for_tests(&genesis_config);
    let keypair = Keypair::new();
...
fn test_readonly_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &huione_sdk::pubkey::new_rand(), 0);
    let bank = Bank::new_for_tests(&genesis_config);

    let vote_pubkey0 = huione_sdk::pubkey::new_rand();
    let vote_pubkey1 = huione_sdk::pubkey::new_rand();
    let vote_pubkey2 = huione_sdk::pubkey::new_rand();
...
fn test_readonly_relaxed_locks() {
    ...
    let key3 = huione_sdk::pubkey::new_rand();
...
fn test_bank_parent_account_spend() {
    let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("1.0"));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let parent = Arc::new(Bank::new_for_tests(&genesis_config));
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_hash_internal_state() {
    ...
    let pubkey2 = huione_sdk::pubkey::new_rand();
...
fn test_bank_hash_internal_state_verify() {
    for pass in 0..3 {
        ...
        let bank2 = Bank::new_from_parent(&bank0, &huione_sdk::pubkey::new_rand(), 1);
        ...
        let bank3 = Bank::new_from_parent(&bank0, &huione_sdk::pubkey::new_rand(), 2);
...
fn test_hash_internal_state_order() {
    ...
    let bank1 = Bank::new_for_tests(&genesis_config);
    assert_eq!(bank0.hash_internal_state(), bank1.hash_internal_state());
    let key0 = huione_sdk::pubkey::new_rand();
    let key1 = huione_sdk::pubkey::new_rand();
...
fn test_hash_internal_state_error() {
    ...
    assert!(bank
        .transfer(hc_to_lamports("1."), &mint_keypair, &key0)
        .is_err());
...
fn test_bank_update_sysvar_account() {
    ...
    for pass in 0..5 {
        ...
        let dummy_clock_id = huione_sdk::pubkey::new_rand();
...
fn test_bank_inherit_tx_count() {
    ...
    // Bank 2
    let bank2 = Bank::new_from_parent(&bank0, &huione_sdk::pubkey::new_rand(), 2);
    ...
    let bank6 = Bank::new_from_parent(&bank1, &huione_sdk::pubkey::new_rand(), 3);
    assert_eq!(bank1.transaction_count(), 1);
    assert_eq!(bank1.non_vote_transaction_count_since_restart(), 1);
...
fn test_bank_vote_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &huione_sdk::pubkey::new_rand(), 1);
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = huione_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &huione_sdk::pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_bank_get_program_accounts() {
    ...
    assert!(
        genesis_accounts
            .iter()
            .any(|(pubkey, _, _)| huione_sdk::sysvar::is_sysvar_id(pubkey)),
        "no sysvars found"
    );

    let bank0 = Arc::new(new_from_parent(&parent));
    let pubkey0 = huione_sdk::pubkey::new_rand();
    ...
    let pubkey1 = huione_sdk::pubkey::new_rand();
...
fn test_add_duplicate_static_program() {
    ...
    let vote_loader_account = bank.get_account(&huione_vote_program::id()).unwrap();
    bank.add_mockup_builtin(huione_vote_program::id(), process_instruction);
    let new_vote_loader_account = bank.get_account(&huione_vote_program::id()).unwrap();
...
fn test_transaction_with_duplicate_accounts_in_instruction() {
    ...
    declare_process_instruction!(process_instruction, 1, |invoke_context| {
        ...
        let instruction_context = transaction_context.get_current_instruction_context()?;
        let instruction_data = instruction_context.get_instruction_data();
        let lamports = u128::from_le_bytes(instruction_data.try_into().unwrap());
        instruction_context
            .try_borrow_instruction_account(transaction_context, 2)?
    ...
    let from_pubkey = huione_sdk::pubkey::new_rand();
    let to_pubkey = huione_sdk::pubkey::new_rand();
    let dup_pubkey = from_pubkey;
    let from_account = AccountSharedData::new(hc_to_lamports("100."), 1, &mock_program_id);
    ...
    let instruction =
        Instruction::new_with_bincode(mock_program_id, &hc_to_lamports("10."), account_metas);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    assert_eq!(bank.get_balance(&from_pubkey), hc_to_lamports("80."));
    assert_eq!(bank.get_balance(&to_pubkey), hc_to_lamports("20."));
}
...
fn test_account_ids_after_program_ids() {
    ...
    let instruction = Instruction::new_with_bincode(huione_vote_program::id(), &10, account_metas);
    let mut tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    tx.message.account_keys.push(huione_sdk::pubkey::new_rand());
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    let account = bank.get_account(&huione_vote_program::id()).unwrap();
...
fn test_program_id_as_payer() {
    ...
    tx.message.account_keys.push(huione_vote_program::id());
...
fn test_bank_hash_consistency() {
    ...
    loop {
        ...
        if bank.slot == 0 {
            assert_eq!(
                bank.hash().to_string(),
                "CFT7EdGUpE6qoxgPoGxG2rPG8hmcqT9PS2wQokYrMdY"
            );
        }
        if bank.slot == 32 {
            assert_eq!(
                bank.hash().to_string(),
                "5HSzZCyyrme2T4S3Z3enzCXDjgmExzwEd2nFTLhjYykL"
            );
        }
        if bank.slot == 64 {
            assert_eq!(
                bank.hash().to_string(),
                "HCiEPG1Z31ZwkH3SiaqHZcUQdc3fYPb6gA36QoB1CjAS"
            );
        }
        if bank.slot == 128 {
            assert_eq!(
                bank.hash().to_string(),
                "HwGNA242sETfZSDFiKGaviGqRKrkHk1cgHZQbyV5Whmr"
            );
            break;
        }
        bank = Arc::new(new_from_parent(&bank));
...
fn test_same_program_id_uses_unqiue_executable_accounts() {
    ...
    // Add a new program
    let program1_pubkey = huione_sdk::pubkey::new_rand();
    bank.add_mockup_builtin(program1_pubkey, process_instruction);

    // Add a new program owned by the first
    let program2_pubkey = huione_sdk::pubkey::new_rand();
...
fn test_shrink_candidate_slots_cached() {
    ...
    assert_eq!(alive_counts, vec![18, 1, 7]);
}

#[test]
fn test_add_builtin_no_overwrite() {
    let slot = 123;
    let program_id = huione_sdk::pubkey::new_rand();
...
fn test_add_builtin_loader_no_overwrite() {
    let slot = 123;
    let loader_id = huione_sdk::pubkey::new_rand();
...
fn test_reconfigure_token2_native_mint() {
    ...
    let mut genesis_config =
        create_genesis_config_with_leader(5, &huione_sdk::pubkey::new_rand(), 0).genesis_config;

    // ClusterType::Development - Native mint exists immediately
    assert_eq!(genesis_config.cluster_type, ClusterType::Development);
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    assert_eq!(
        bank.get_balance(&inline_hpl_token::native_mint::id()),
        1000000000
    );
    ...
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    assert_eq!(bank.get_balance(&inline_hpl_token::native_mint::id()), 1000000000);
    bank.deposit(&inline_hpl_token::native_mint::id(), 4200000000)
        .unwrap();
    ...
    let native_mint_account = bank
        .get_account(&inline_hpl_token::native_mint::id())
        .unwrap();
    assert_eq!(native_mint_account.data().len(), 90);
    assert_eq!(
        bank.get_balance(&inline_hpl_token::native_mint::id()),
        5200000000
    );
    assert_eq!(native_mint_account.owner(), &inline_hpl_token::id());
...
fn test_bank_load_program() {
    ...
    let programdata_key = huione_sdk::pubkey::new_rand();
...
fn test_bpf_loader_upgradeable_deploy_with_max_len() {
    ...
    let mut file = File::open("/app/orca/huione/huione_project/huione/programs/bpf_loader/test_elfs/out/noop_aligned.so")
    .expect("file open failed");
    let mut elf = Vec::new();
    file.read_to_end(&mut elf).unwrap();
    ...
    let payer_base_balance = LAMPORTS_PER_HC;
    let deploy_fees = {
    ...
    mock_process_instruction(
        ...
        Vec::new(),
        Ok(()),
        huione_bpf_loader_program::process_instruction,
        |invoke_context| {
            invoke_context
    ...
    *instructions.get_mut(0).unwrap() = system_instruction::create_account(
    ...
    bank.store_account(
        &mint_keypair.pubkey(),
        &AccountSharedData::new(u128::MAX / 2, 0, &system_program::id()),
    );
    let mut modified_buffer_account = buffer_account.clone();
    modified_buffer_account.set_lamports(u128::MAX / 2);
    ...
    assert_eq!(
        ...
        TransactionError::InstructionError(1, InstructionError::ArithmeticOverflow),
        bank_client
            .send_and_confirm_message(
                &[&mint_keypair, &program_keypair, &upgrade_authority_keypair],
                message
...
fn min_rent_exempt_balance_for_sysvars(bank: &Bank, sysvar_ids: &[Pubkey]) -> u128 {
    sysvar_ids
        .iter()
        .map(|sysvar_id| {
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &huione_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
    ...
    let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(huione_sdk::pubkey::new_rand)
        .take(total_pubkeys)
        .collect();
    ...
    test_store_scan_consistency(
        move |bank0,
            ...
            loop {
                ...
                let num_new_banks = 2;
                let lamports = current_minor_fork_bank.slot() as u128+ starting_lamports + 1;
                ...
                let lamports = current_major_fork_bank.slot() as u128 + starting_lamports + 1;
                let account = AccountSharedData::new(lamports, 0, &program_id);
    ...
    test_store_scan_consistency(
        |bank0,
         ...
         starting_lamports| {
            ...
            loop {
                let lamports_this_round = current_bank.slot() as u128 + starting_lamports + 1;
...
fn setup_banks_on_fork_to_remove(
    ...
    for _ in (0..num_banks_on_fork).step_by(step_size) {
        let mut lamports_this_round = 0u128;
        for i in 0..step_size {
            ...
            if lamports_this_round == 0 {
                lamports_this_round = bank_at_fork_tip.bank_id() as u128 + starting_lamports + 1;
            }
...
//     let GenesisConfigInfo {
//         mut genesis_config, ..
//     } = create_genesis_config_with_leader(42, &huione_sdk::pubkey::new_rand(), 42);
...
fn test_get_largest_accounts() {
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &pubkeys_hashset, AccountAddressFilter::Include)
            .unwrap(),
        vec![(pubkeys[4], hc_to_lamports("5.0"))]
    );
    assert_eq!(
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &exclude4, AccountAddressFilter::Exclude)
            .unwrap(),
        vec![(pubkeys[3], hc_to_lamports("4.0"))]
    );
...
fn test_rent_debits() {
    ...
    rent_debits.insert(&Pubkey::new_unique(), i128::MAX as u128, 0);
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        huione_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        huione_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_calculate_prioritization_fee() {
    ...
    let request_unit_price = 2_000_000_000_u128;
    let prioritization_fee_details = PrioritizationFeeDetails::new(
        PrioritizationFeeType::ComputeUnitPrice(request_unit_price),
        request_units as u128,
    );
...
fn test_an_empty_instruction_without_program() {
    ...
    let destination = huione_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_with_good_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("1000."));
    let bank = Bank::new_for_tests(&genesis_config);
...
fn test_accounts_data_size_with_bad_transaction() {
    const ACCOUNT_SIZE: u64 = MAX_PERMITTED_DATA_LENGTH;
    let bank = create_simple_test_bank(1_000_000_000_000);
    let transaction = system_transaction::create_account(
        &Keypair::new(),
        &Keypair::new(),
        bank.last_blockhash(),
...
fn test_invalid_rent_state_changes_existing_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(hc_to_lamports("100."), &Pubkey::new_unique(), 42);
    genesis_config.rent = Rent::default();
    ...
    let result = bank.transfer(4, &mint_keypair, &rent_paying_account.pubkey());
    assert!(result.is_ok());
    assert!(check_account_is_rent_exempt(&rent_paying_account.pubkey()));
...
fn test_rent_state_changes_sysvars() {
    ...
    let validator_pubkey = huione_sdk::pubkey::new_rand();
    let validator_stake_lamports = hc_to_lamports("1.");
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        huione_sdk::fee_calculator::DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE,
        huione_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &huione_sdk::incinerator::id())
            .unwrap();
    }
}
        ...
        match instruction {
            MockReallocInstruction::Realloc(new_size, new_balance, _) => {
                ...
                let current_balance = instruction_context
                    .try_borrow_instruction_account(transaction_context, 1)?
                    .get_lamports();
                let diff_balance = (new_balance as i128).saturating_sub(current_balance as i128);
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = huione_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_and_resize_transactions() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = genesis_utils::create_genesis_config(100 * LAMPORTS_PER_HC);
    let mut bank = Bank::new_for_tests(&genesis_config);
    let mock_program_id = Pubkey::new_unique();
    bank.add_mockup_builtin(mock_program_id, mock_realloc_process_instruction);
    ...
    bank.store_account(
        &funding_keypair.pubkey(),
        &AccountSharedData::new(10 * LAMPORTS_PER_HC, 0, &mock_program_id),
    );
    ...
    // Test case: Grow account
    {
        ...
        let account_balance = LAMPORTS_PER_HC;
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_HC);
    let mut bank = Bank::new_for_tests(&genesis_config);
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            huione_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_stake_account_consistency_with_rent_epoch_max_feature(
    ...
    huione_logger::setup();
    let (mut genesis_config, _mint_keypair) = create_genesis_config(100 * LAMPORTS_PER_HC);
    genesis_config.rent = Rent::default();
    let mut bank = Bank::new_for_tests(&genesis_config);
    let expected_initial_rent_epoch = if rent_epoch_max_enabled_initially {
        bank.activate_feature(&huione_sdk::feature_set::set_exempt_rent_epoch_max::id());
...
fn test_calculate_fee_with_congestion_multiplier() {
    let lamports_scale: u128 = 5;
    let base_lamports_per_signature: u128 = 5_000;
    let cheap_lamports_per_signature: u128 = base_lamports_per_signature / lamports_scale;
    let expensive_lamports_per_signature: u128 = base_lamports_per_signature * lamports_scale;
    let signature_count: u64 = 2;
    let signature_fee: u128 = 10;
    let fee_structure = FeeStructure {
    ...
    for remove_congestion_multiplier in [true, false] {
        assert_eq!(
            ...
            signature_fee * signature_count as u128
        );
    }
    ...
    for remove_congestion_multiplier in [true, false] {
        let denominator: u128 = if remove_congestion_multiplier {
            1
        } else {
            lamports_scale
        };
        ...
        assert_eq!(
            ...
            signature_fee * signature_count as u128 / denominator
        );
    }
}
...
fn test_calculate_fee_with_request_heap_frame_flag() {
    let key0 = Pubkey::new_unique();
    let key1 = Pubkey::new_unique();
    let lamports_per_signature: u128 = 5_000;
    let signature_fee: u128 = 10;
    let request_cu: u128 = 1;
    let lamports_per_cu: u128 = 5;
    let fee_structure = FeeStructure {
        lamports_per_signature: signature_fee,
        ..FeeStructure::default()
    };
...
fn test_runtime_feature_enable_with_program_cache() {
    ...
    // Bank Setup
    let (mut genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_HC);
    genesis_config
        .accounts
...
fn test_calculate_loaded_accounts_data_size_cost() {
    ...
    assert_eq!(
        compute_budget.heap_cost as u128,
    ...
    assert_eq!(
        compute_budget.heap_cost as u128 * 2,
        Bank::calculate_loaded_accounts_data_size_cost(&compute_budget)
    );
    ...
    F: Fn(&Bank),
{
    ...
    let mint_lamports = hc_to_lamports("1.0");
    let len1 = 123;
    let len2 = 456;
...
fn test_system_instruction_unsigned_transaction() {
    let (genesis_config, alice_keypair) = create_genesis_config(hc_to_lamports("1.0"));
    let alice_pubkey = alice_keypair.pubkey();
