use {
    huione_sdk::{
        ...
        feature_set::{FeatureSet,InitFeatureSet},
...
const VALIDATOR_LAMPORTS: u128 = 42;

// fun fact: rustc is very close to make this const fn.
pub fn bootstrap_validator_stake_lamports() -> u128 {
    Rent::default().minimum_balance(StakeState::size_of())
}

// Number of lamports automatically used for genesis accounts
pub const fn genesis_sysvar_and_builtin_program_lamports() -> u128 {
    const NUM_BUILTIN_PROGRAMS: u128 = 4+5;
    const FEES_SYSVAR_MIN_BALANCE: u128 = 6014880;
    const STAKE_HISTORY_MIN_BALANCE: u128 = 1203310160;
    const CLOCK_SYSVAR_MIN_BALANCE: u128 = 7017360;
    const RENT_SYSVAR_MIN_BALANCE: u128 = 6390810;
    const EPOCH_SCHEDULE_SYSVAR_MIN_BALANCE: u128 = 6724970;
    const RECENT_BLOCKHASHES_SYSVAR_MIN_BALANCE: u128 = 306424720;
    const TOKEN2_NATIVE_MINT_BALANCE:u128 = 1_000_000_000;
    const TOKEN2_NATIVE_MINT_META_BALANCE:u128 = 1_000_000_000;
    FEES_SYSVAR_MIN_BALANCE
        ...
        + TOKEN2_NATIVE_MINT_BALANCE
        + TOKEN2_NATIVE_MINT_META_BALANCE
}
...
pub fn create_genesis_config(mint_lamports: u128) -> GenesisConfigInfo {
    ...
    create_genesis_config_with_leader(
        mint_lamports,
        &huione_sdk::pubkey::new_rand(), // validator_pubkey
...
pub fn create_genesis_config_with_vote_accounts_and_cluster_type(
    ...
    for (validator_voting_keypairs, stake) in voting_keypairs[1..].iter().zip(&stakes[1..]) {
        ...
        // Huione newly created accounts into genesis
...
pub fn activate_init_features(genesis_config: &mut GenesisConfig) {
    // Activate init features at genesis in test mainnet mode
    for feature_id in InitFeatureSet::default().init {
        activate_feature(genesis_config, feature_id);
    }
}
