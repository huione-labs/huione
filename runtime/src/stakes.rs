...
impl StakesCache {
    ...
    pub(crate) fn check_and_store(
        ...
        if account.lamports() == 0 {
            if huione_vote_program::check_id(owner) {
                let mut stakes = self.0.write().unwrap();
                stakes.remove_vote_account(pubkey);
            } else if huione_stake_program::check_id(owner) {
                let mut stakes = self.0.write().unwrap();
                stakes.remove_stake_delegation(pubkey, new_rate_activation_epoch);
            }
            return;
        }
        debug_assert_ne!(account.lamports(), 0u128);
        if huione_vote_program::check_id(owner) {
    ...
    pub(crate) fn activate_epoch(&self, next_epoch: Epoch, thread_pool: &ThreadPool,new_rate_activation_epoch: Option<Epoch>,) {
        let mut stakes = self.0.write().unwrap();
...
impl<T: Clone> Stakes<T> {
    ...
    pub(crate) fn staked_nodes(&self) -> Arc<HashMap<Pubkey, u128>> {
...
impl Stakes<StakeAccount> {
    ...
    fn activate_epoch(&mut self, next_epoch: Epoch, thread_pool: &ThreadPool, new_rate_activation_epoch: Option<Epoch>,) {
        type StakesHashMap = HashMap</*voter:*/ Pubkey, /*stake:*/ u128>;
        ...
        let stake_history_entry = thread_pool.install(|| {
            stake_delegations
                ...
                .fold(StakeActivationStatus::default, |acc, stake_account| {
                    ...
                    acc + delegation
                        .stake_activating_and_deactivating(self.epoch, Some(&self.stake_history), new_rate_activation_epoch)
                })
        ...
        let delegated_stakes = thread_pool.install(|| {
            stake_delegations
                ...
                .fold(HashMap::default, |mut delegated_stakes, stake_account| {
                    ...
                    *entry += delegation.stake(self.epoch, Some(&self.stake_history), new_rate_activation_epoch);
                    delegated_stakes
                })
    ...
    pub(crate) fn vote_balance_and_staked(&self) -> u128 {
        ...
        self.stake_delegations.values().map(get_stake).sum::<u128>()
            + self.vote_accounts.iter().map(get_lamports).sum::<u128>()
    }
    ...
    fn upsert_vote_account(
        ...
        debug_assert_ne!(vote_account.lamports(), 0u128);
    ...
    fn upsert_stake_delegation(
        ...
        debug_assert_ne!(stake_account.lamports(), 0u128);
        let delegation = stake_account.delegation();
...
pub(crate) mod tests {
    use {
        ...
        huione_sdk::{account::WritableAccount, pubkey::Pubkey, rent::Rent, stake},
    ...
    pub(crate) fn create_staked_node_accounts(
        ...
        let vote_pubkey = huione_sdk::pubkey::new_rand();
        let vote_account =
            vote_state::create_account(&vote_pubkey, &huione_sdk::pubkey::new_rand(), 0, 1);
        let stake_pubkey = huione_sdk::pubkey::new_rand();
        (
    ...
    fn test_stakes_basic() {
        for i in 0..4 {
            ...
            let mut stake_account =
                create_stake_account(42, &vote_pubkey, &huione_sdk::pubkey::new_rand());
            stakes_cache.check_and_store(&stake_pubkey, &stake_account, None);
            let stake = stake_state::stake_from(&stake_account).unwrap();
            {
                let stakes = stakes_cache.stakes();
    ...
    fn test_stakes_multiple_stakers() {
        ...
        let stake_pubkey2 = huione_sdk::pubkey::new_rand();
    ...
    fn test_vote_balance_and_staked_normal() {
        let stakes_cache = StakesCache::default();
        impl Stakes<StakeAccount> {
            fn vote_balance_and_warmed_staked(&self) -> u128 {
                let vote_balance: u128 = self
                    .vote_accounts
                    .iter()
                    .map(|(_pubkey, account)| account.lamports())
                    .sum();
                let warmed_stake: u128 = self
                    .vote_accounts
                    .delegated_stakes()
    ...
    fn test_serde_stakes_enum_compat() {
        ...
        for _ in 0..rng.gen_range(5usize, 10) {
            ...
            let vote_account = vote_state::create_account(
                &vote_pubkey,
                &huione_sdk::pubkey::new_rand(), // node_pubkey
