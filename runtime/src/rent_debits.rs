use {
    ...
    huione_sdk::{pubkey::Pubkey, reward_type::RewardType},
...
impl RentDebit {
    fn try_into_reward_info(self) -> Option<RewardInfo> {
        let rent_debit = i128::try_from(self.rent_collected)
            .ok()
...
impl RentDebits {
    pub(crate) fn get_account_rent_debit(&self, address: &Pubkey) -> u128 {
        self.0
            .get(address)
    ...
    pub fn insert(&mut self, address: &Pubkey, rent_collected: u128, post_balance: u128) {
        if rent_collected != 0 {
            self.0.insert(
                *address,
                RentDebit {
