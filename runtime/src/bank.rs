...
use ethnum::U256;
#[allow(deprecated)]
use huione_sdk::recent_blockhashes_account;
pub use huione_sdk::reward_type::RewardType;
use {
    crate::{
        ...
        // inline_hpl_associated_token_account, 
        inline_hpl_token,
    ...
    huione_bpf_loader_program::syscalls::create_program_runtime_environment,
    huione_measure::{measure, measure::Measure, measure_us},
    huione_perf::perf_libs,
    ...
    huione_stake_program::stake_state::{
        self, InflationPointCalculationEvent, PointValue, StakeState,
    },
    huione_system_program::{get_system_account_kind, SystemAccountKind},
    huione_vote_program::vote_state::{VoteState, VoteStateVersions},
    std::{
        borrow::Cow,
        cell::RefCell,
        ...
        fmt, mem,
        ops::{AddAssign, RangeInclusive,Div},
        path::PathBuf,
        rc::Rc,
        sync::{
            atomic::{
...
#[frozen_abi(digest = "BWSepJmZykHUtYxjzxKTNbk9DmmKyEwKyKvqf6Y3RRoZ")]
...
use huione_frozen_abi::abi_example::AbiExample;
...
impl DurableNonceFee {
    pub fn lamports_per_signature(&self) -> Option<u128> {
        match self {
...
pub type TransactionBalances = Vec<Vec<u128>>;
...
pub trait NonceInfo {
    ...
    fn lamports_per_signature(&self) -> Option<u128>;
...
pub struct BankFieldsToDeserialize {
    ...
    pub(crate) collector_fees: u128,
    pub(crate) fee_calculator: FeeCalculator,
    pub(crate) fee_rate_governor: FeeRateGovernor,
    pub(crate) collected_rent: u128,
...
pub struct Bank {
    ...
    collector_fees: AtomicU128,

    /// Track cluster signature throughput and adjust fee rate
    pub(crate) fee_rate_governor: FeeRateGovernor,

    /// Rent that has been collected
    collected_rent: AtomicU128,
...
impl Bank {
    ...
    fn default_with_accounts(accounts: Accounts) -> Self {
        let mut bank = Self {
            ...
            capitalization: AtomicU128::default(),
            ...
            collector_fees: AtomicU128::default(),
            fee_rate_governor: FeeRateGovernor::default(),
            collected_rent: AtomicU128::default(),
    ...
    fn _new_from_parent(
        ...
        let (fee_rate_governor, fee_components_time_us) = measure_us!(
            FeeRateGovernor::new_derived(&parent.fee_rate_governor, parent.signature_count() as u128)
        );
        ...
        let mut new = Self {
            ...
            collector_fees: AtomicU128::new(0),
            ancestors: Ancestors::default(),
    ...
    pub(crate) fn new_from_fields(
        ...
        let mut bank = Self {
            ...
            capitalization: AtomicU128::new(fields.capitalization),
            ...
            collector_fees: AtomicU128::new(fields.collector_fees),
            fee_rate_governor: fields.fee_rate_governor,
            collected_rent: AtomicU128::new(fields.collected_rent),
    ...
    fn inherit_specially_retained_account_fields(
        ...
        const RENT_UNADJUSTED_INITIAL_BALANCE: u128 = 1;
    ...
    fn calculate_previous_epoch_inflation_rewards(
        &self,
        prev_epoch_capitalization: u128,
        prev_epoch: Epoch,
        ...
        let validator_rewards = (validator_rate
            ...
            * prev_epoch_duration_in_years) as u128;
    ...
    fn update_rewards_with_thread_pool(
        ...
        datapoint_warn!(
            ...
            ("validator_rewards", validator_rewards_paid, i128),
            ("active_stake", active_stake, i128),
            ("pre_capitalization", capitalization, i128),
            ("post_capitalization", self.capitalization(), i128),
    ...
    fn filter_stake_delegations<'a>(
            ...
            .is_active(&feature_set::stake_minimum_delegation_for_rewards::id())
        {
            ...
            let min_stake_delegation =
                huione_stake_program::get_minimum_delegation(&self.feature_set)
                    .max(LAMPORTS_PER_HC);
    ...
    fn _load_vote_and_stake_accounts(
        ...
        let huione_vote_program: Pubkey = huione_vote_program::id();
        ...
        let get_vote_account = |vote_pubkey: &Pubkey| -> Option<VoteAccount> {
            ...
            if account.owner() == &huione_vote_program
        ...
        let make_vote_delegations_entry = |vote_pubkey| {
            ...
            if vote_account.owner() != &huione_vote_program {
        ...
        let push_stake_delegation = |(stake_pubkey, stake_account): (&Pubkey, &StakeAccount<_>)| {
            ...
            if let Some(reward_calc_tracer) = reward_calc_tracer.as_ref() {
                let delegation =
                    InflationPointCalculationEvent::Delegation(delegation, huione_vote_program);
                    ...
                    if let Ok((stakers_reward, voters_reward)) = redeemed {
                        ...
                        return Some(StakeReward {
                            ...
                            stake_reward_info: RewardInfo {
                                ...
                                lamports: i128::try_from(stakers_reward).unwrap(),
                                post_balance,
    ...
    pub fn last_blockhash_and_lamports_per_signature(&self) -> (Hash, u128) {
        let blockhash_queue = self.blockhash_queue.read().unwrap();
    ...
    pub fn get_minimum_balance_for_rent_exemption(&self, data_len: usize) -> u128 {
        self.rent_collector.rent.minimum_balance(data_len).max(1)
    }

    pub fn get_lamports_per_signature(&self) -> u128 {
        self.fee_rate_governor.lamports_per_signature
    }

    pub fn get_lamports_per_signature_for_blockhash(&self, hash: &Hash) -> Option<u128> {
        let blockhash_queue = self.blockhash_queue.read().unwrap();
    ...
    pub fn get_fee_for_message(&self, message: &SanitizedMessage) -> Option<u128> {
    ...
    pub fn collect_balances(&self, batch: &TransactionBatch) -> TransactionBalances {
        ...
        for transaction in batch.sanitized_transactions() {
            let mut transaction_balances: Vec<u128> = vec![];
    ...
    fn program_modification_slot(&self, pubkey: &Pubkey) -> Result<Slot> {
        ...
        if bpf_loader_upgradeable::check_id(program.owner()) {
            ...
            let state = huione_loader_v4_program::get_state(program.data())
    ...
    pub fn load_program(&self, pubkey: &Pubkey) -> Arc<LoadedProgram> {
        ...
        let program_runtime_environment_v1 = self
            ...
            .clone();
        huione_bpf_loader_program::load_program_from_account(
            &self.feature_set,
            None, // log_collector
            &program,
    ...
    fn get_num_signatures_in_message(message: &SanitizedMessage) -> u128 {
        let mut num_signatures = u128::from(message.header().num_required_signatures);
        ...
        for (program_id, instruction) in message.program_instructions_iter() {
            if secp256k1_program::check_id(program_id) || ed25519_program::check_id(program_id) {
                if let Some(num_verifies) = instruction.data.first() {
                    num_signatures = num_signatures.saturating_add(u128::from(*num_verifies));
                }
            }
        }
        num_signatures
    ...
    fn get_num_write_locks_in_message(message: &SanitizedMessage) -> u128 {
        message
            .account_keys()
            .len()
            .saturating_sub(message.num_readonly_accounts()) as u128
    }
    ...
    pub fn calculate_fee(
        ...
        if tx_wide_compute_cap {
            ...
            debug!("CALCULATE_FEE prioritization_fee:{}",prioritization_fee);
            let signature_fee = Self::get_num_signatures_in_message(message)
                .saturating_mul(fee_structure.lamports_per_signature);
            debug!("CALCULATE_FEE lamports_per_signature:{}",fee_structure.lamports_per_signature);
            debug!("CALCULATE_FEE param_lamports_per_signature:{}",lamports_per_signature);
            debug!("CALCULATE_FEE signature_fee:{}",signature_fee);
            let write_lock_fee = Self::get_num_write_locks_in_message(message)
                .saturating_mul(fee_structure.lamports_per_write_lock);
            debug!("CALCULATE_FEE lamports_per_write_lock:{}",fee_structure.lamports_per_write_lock);
            debug!("CALCULATE_FEE write_lock_fee:{}",write_lock_fee);
            let loaded_accounts_data_size_cost = 0_u128;
            let total_compute_units =
            loaded_accounts_data_size_cost.saturating_add(compute_budget.compute_unit_limit as u128);
            ...
            debug!("CALCULATE_FEE compute_fee:{}",compute_fee);
            ((prioritization_fee
            ...
            lamports_per_signature
            .saturating_mul(Self::get_num_signatures_in_message(message) as u128)
        }
        ...
        //     .unwrap_or_default();
        // let prioritization_fee = prioritization_fee_details.get_fee();
        // let signature_fee = (Self::get_num_signatures_in_message(message) as u128)
        //     .saturating_mul(fee_structure.lamports_per_signature);
        // let write_lock_fee = (Self::get_num_write_locks_in_message(message) as u128)
        //     .saturating_mul(fee_structure.lamports_per_write_lock);
    ...
    //
    fn calculate_loaded_accounts_data_size_cost(compute_budget: &ComputeBudget) -> u128 {
        const ACCOUNT_DATA_COST_PAGE_SIZE: u128 = 32_u128.saturating_mul(1024);
        (compute_budget.loaded_accounts_data_size_limit as u128)
            .saturating_add(ACCOUNT_DATA_COST_PAGE_SIZE.saturating_sub(1))
            .saturating_div(ACCOUNT_DATA_COST_PAGE_SIZE)
            .saturating_mul(compute_budget.heap_cost as u128)
    }
    ...
    fn filter_program_errors_and_collect_fee(
        ...
        let results = txs
            ...
            .map(|(tx, execution_result)| {
                ...
                debug!("CALCULATE_FEE: tx: {}",tx.signature());
                ...
                // oa fees log
                // info!(
                //     "calculate_fee fee: {} lamports_per_signature:{}, tx:{}",
                //     fee,
                //     lamports_per_signature,
                //     tx.message_hash()
                // );
    ...
    fn distribute_rent_to_validators(
        &self,
        vote_accounts: &HashMap<Pubkey, (/*stake:*/ u128, VoteAccount)>,
        rent_to_be_distributed: u128,
    ) {
        ...
        let validator_rent_shares = validator_stakes
            .into_iter()
            .map(|(pubkey, staked)| {
                let rent_share = U256::new(staked)
                    .mul(U256::new(rent_to_be_distributed))
                    .div(U256::new(total_staked))
                    .try_into()
                    .unwrap();
            ...
            .for_each(|(pubkey, rent_share)| {
                ...
                if rent_to_be_paid > 0 {
                    ...
                    if account.checked_add_lamports(rent_to_be_paid).is_err() {
                        ...
                        self.store_account(&pubkey, &account);
                        info!(
                            "distributed rent fee: {} (post_balance: {}, pubkey: {})",
                            rent_to_be_paid,
                            account.lamports(),
                            pubkey
                        );
                        rewards.push((
                            ...
                            RewardInfo {
                                ...
                                lamports: rent_to_be_paid as i128,
    ...
    fn collect_rent_from_accounts(
        ...
        let set_exempt_rent_epoch_max: bool = self
            .feature_set
            .is_active(&huione_sdk::feature_set::set_exempt_rent_epoch_max::id());
    ...
    fn skip_rewrite(rent_amount: u128, account: &AccountSharedData) -> bool {
        // if rent was != 0
    ...
    fn use_multi_epoch_collection_cycle(&self, epoch: Epoch) -> bool {
        ...
        #[cfg(not(test))]
        if self.slot_count_per_normal_epoch() == huione_sdk::epoch_schedule::MINIMUM_SLOTS_PER_EPOCH {
            return false;
        }
    ...
    pub fn transfer(&self, n: u128, keypair: &Keypair, to: &Pubkey) -> Result<Signature> {
    ...
    pub fn read_balance(account: &AccountSharedData) -> u128 {
        account.lamports()
    }
    /// Each program would need to be able to introspect its own state
    /// this is hard-coded to the Budget language
    pub fn get_balance(&self, pubkey: &Pubkey) -> u128 {
    ...
    fn withdraw(&self, pubkey: &Pubkey, lamports: u128) -> Result<()> {
    ...
    pub fn deposit(
        ...
        pubkey: &Pubkey,
        lamports: u128,
    ) -> std::result::Result<u128, LamportsError> {
    ...
    fn verify_accounts_hash(
        &self,
        base: Option<(Slot, /*capitalization*/ u128)>,
    ...
    fn calculate_capitalization(&self, debug_verify: bool) -> u128 {
        let is_startup = true;
        self.rc
            .accounts
            .accounts_db
    ...
    pub fn set_capitalization(&self) -> u128 {
    ...
    pub fn capitalization(&self) -> u128 {
    ...
    pub fn staked_nodes(&self) -> Arc<HashMap<Pubkey, u128>> {
    ...
    pub fn epoch_staked_nodes(&self, epoch: Epoch) -> Option<Arc<HashMap<Pubkey, u128>>> {
    ...
    pub fn total_epoch_stake(&self) -> u128 {
        self.epoch_stakes
            .get(&self.epoch)
            .expect("Epoch stakes for bank's own epoch must exist")
            .total_stake()
    ...
    pub fn epoch_vote_account_stake(&self, vote_account: &Pubkey) -> u128 {
        *self
    ...
    fn reconfigure_token2_native_mint(&mut self) {
        let reconfigure_token2_native_mint = match self.cluster_type() {
            ...
            ClusterType::Testnet => true,     // oa self.epoch() == 93,
            ClusterType::MainnetBeta => true, // oa self.epoch() == 75,
        };

        if reconfigure_token2_native_mint {
            let mut native_mint_account = huione_sdk::account::AccountSharedData::from(Account {
                owner: inline_hpl_token::id(),
                data: inline_hpl_token::native_mint::ACCOUNT_DATA.to_vec(),
                lamports: hc_to_lamports("1"),
                executable: false,
                rent_epoch: self.epoch() + 1,
            });

            let mut native_mint_info_account = huione_sdk::account::AccountSharedData::from(Account {
                owner: inline_hpl_token::id(),
                data: inline_hpl_token::native_mint_info::ACCOUNT_DATA.to_vec(),
                lamports: hc_to_lamports("1"),
                executable: false,
                rent_epoch: self.epoch() + 1,
            });
            ...
            // As a workaround for
            // https://github.com/huione-labs/huione-program-library/issues/374, ensure that the
            // hpl-token 2 native mint account is owned by the hpl-token 2 program.
            let store = if let Some(existing_native_mint_account) =
                self.get_account_with_fixed_root(&inline_hpl_token::native_mint::id())
            {
                if existing_native_mint_account.owner() == &huione_sdk::system_program::id() {
            ...
            if store {
                self.store_account(&inline_hpl_token::native_mint::id(), &native_mint_account);
            }

            // as mint account data
            let _info_store = if let Some(existing_native_mint_info_account) =
                self.get_account_with_fixed_root(&inline_hpl_token::native_mint_info::id())
            {
                if existing_native_mint_info_account.owner() == &huione_sdk::system_program::id() {
                    native_mint_info_account
                        .set_lamports(existing_native_mint_info_account.lamports());
                    true
                } else {
                    false
                }
                ...
                self.capitalization
                    .fetch_add(native_mint_info_account.lamports(), Relaxed);
                true
            };

            if _info_store {
                self.store_account(
                    &inline_hpl_token::native_mint_info::id(),
                    &native_mint_info_account,
                );
            }
        }
...
pub struct TotalAccountsStats {
    ...
    pub lamports_in_rent_paying_accounts: u128,
}
...
pub mod test_utils {
    use {
        super::Bank,
        huione_sdk::{hash::hashv, pubkey::Pubkey},
        huione_vote_program::vote_state::{self, BlockTimestamp, VoteStateVersions},
    };
