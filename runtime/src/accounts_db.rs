...
use {
    ...
    huione_measure::{measure::Measure, measure_us},
    huione_rayon_threadlimit::get_thread_count,
    huione_sdk::{
...
struct GenerateIndexTimings {
    ...
    pub amount_to_top_off_rent: AtomicU128,
...
pub enum AccountsHashVerificationError {
    ...
    MismatchedTotalLamports(u128, u128),
}
...
pub struct AccountsDb {
    ...
    /// Some(offset) means for slots up to (max_slot - (slots_per_epoch - 'offset')), huione them in ancient append vecs
    pub ancient_append_vec_offset: Option<i64>,
    ...
    accounts_hashes: Mutex<HashMap<Slot, (AccountsHash, /*capitalization*/ u128)>>,
    incremental_accounts_hashes:
        Mutex<HashMap<Slot, (IncrementalAccountsHash, /*capitalization*/ u128)>>,
...
pub fn make_min_priority_thread_pool() -> ThreadPool {
    ...
    rayon::ThreadPoolBuilder::new()
        .thread_name(|i| format!("huioneAccountsLo{i:02}"))
...
impl huione_frozen_abi::abi_example::AbiExample for AccountsDb {
    fn example() -> Self {
...
impl AccountsDb {
    ...
    fn default_with_accounts_index(
        ...
        AccountsDb {
            ...
            thread_pool: rayon::ThreadPoolBuilder::new()
                .num_threads(num_threads)
                .thread_name(|i| format!("huioneAccounts{i:02}"))
    ...
    pub fn new_with_config(
        ...
        let filler_account_suffix = if filler_accounts_config.count > 0 {
            Some(huione_sdk::pubkey::new_rand())
        } else {
            None
        };
    ...
    fn start_background_hasher(&mut self) {
        ...
        Builder::new()
            .name("huioneDbStoreHashr".to_string())
            .spawn(move || {
    ...
    fn get_keys_to_unref_ancient<'a>(
        ...
        // Now it is being huione into an ancient storage again, but it is already there, so maintain max of 1 ref per storage in the accounts index.
    ...
    /// huione entire alive contents of 'old_storage' into the current ancient append vec or a newly created ancient append vec
    fn combine_one_store_into_ancient(
        ...
        if to_store.has_overflow() {
            ...
            // All non-overflow accounts were huione in a prior slot's ancient append vec. All overflow accounts
        ...
        F: FnMut(Option<(&Pubkey, AccountSharedData, Slot)>),
    {
        let key = match &index_key {
            ...
            IndexKey::HplTokenMint(key) => key,
            IndexKey::HplTokenOwner(key) => key,
            IndexKey::HplNftOwner(key) => key,
            IndexKey::HplNameOwner(key) => key,
        };
    ...
    /// This runs prior to the storages being huione in AccountsDb.storage
    ...
    pub fn checked_iterative_sum_for_capitalization(total_cap: u128, new_cap: u128) -> u128 {
        let new_total =  U256::new(total_cap) + U256::new(new_cap);
        AccountsHasher::checked_cast_for_capitalization(new_total)
    }

    pub fn checked_sum_for_capitalization<T: Iterator<Item = u128>>(balances: T) -> u128 {
        AccountsHasher::checked_cast_for_capitalization(balances.map(|b| U256::new(b)).sum::<U256>())
    }
    ...
    pub fn calculate_accounts_hash_from_index(
    ...
    ) -> Result<(AccountsHash, u128), AccountsHashVerificationError> {
        ...
        let total_lamports = Mutex::<u128>::new(0);
        let get_hashes = || {
            keys.par_chunks(chunks)
                .map(|pubkeys| {
                    let mut sum = U256::new(0);
                    ...
                    *total =
                        AccountsHasher::checked_cast_for_capitalization(U256::new(*total as u128) + sum);
                    result
                }).collect()
        };
    ...
    fn calculate_accounts_hash_with_verify(
        ...
        slot: Slot,
        config: CalcAccountsHashConfig<'_>,
        expected_capitalization: Option<u128>,
    ) -> Result<(AccountsHash, u128), AccountsHashVerificationError> {
    ...
    pub fn update_incremental_accounts_hash(
        ...
        slot: Slot,
        stats: HashStats,
    ) -> Result<(IncrementalAccountsHash, /*capitalization*/ u128), AccountsHashVerificationError>
    {
    ...
    pub fn set_accounts_hash(
        &self,
        slot: Slot,
        accounts_hash: (AccountsHash, /*capitalization*/ u128),
    ) -> Option<(AccountsHash, /*capitalization*/ u128)> {
    ...
    pub fn get_accounts_hash(&self, slot: Slot) -> Option<(AccountsHash, /*capitalization*/ u128)> {
    ...
    pub fn set_incremental_accounts_hash(
        &self,
        slot: Slot,
        incremental_accounts_hash: (IncrementalAccountsHash, /*capitalization*/ u128),
    ) -> Option<(IncrementalAccountsHash, /*capitalization*/ u128)> {
    ...
    fn _calculate_accounts_hash_from_storages(
        ...
        accounts_hash_cache_path: PathBuf,
    ) -> Result<(AccountsHashEnum, u128), AccountsHashVerificationError> {
    ...
    fn get_filler_account_pubkeys(&self, count: usize) -> Vec<Pubkey> {
        (0..count)
            .map(|_| {
                let subrange = huione_sdk::pubkey::new_rand();
    ...
    pub fn generate_index(
        ...
        for pass in 0..passes {
            ...
            let amount_to_top_off_rent = AtomicU128::new(0);
...
pub mod tests {
    ...
    impl AccountsDb {
        ...
        pub fn set_accounts_hash_for_tests(&self, slot: Slot, accounts_hash: AccountsHash) {
            self.set_accounts_hash(slot, (accounts_hash, u128::default()));
        }
    ...
    fn test_get_keys_to_unref_ancient() {
        ...
        let pubkey3 = huione_sdk::pubkey::new_rand();
        let pubkey4 = huione_sdk::pubkey::new_rand();
    ...
    fn sample_storages_and_account_in_slot(
        ...
        let expected_hashes = vec![
            Hash::from_str("A5of5Ct7gfaCDhRgeP1rUe2f8vB7MPdMTnauk5HWLwfh").unwrap(),
            Hash::from_str("3Qu8Lwpge4nAVxPL7HLetqs6drR73EPBGfWNwsjKQPob").unwrap(), 
            Hash::from_str("9mkNPHfbjGvs5AJdmd3xn34akb1E6XRhS9KA1uWJ7Mdd").unwrap(),
            Hash::from_str("C9jPtxmqq7Kddi8o2j5pouWKV7uAyGa8fuJgZi9nS1kN").unwrap(),
        ];
    ...
    struct TestScan {
        ...
        accum: BinnedHashData,
        current_slot: Slot,
        value_to_use_for_lamports: u128,
    }
    ...
    fn create_account(
        ...
        for t in 0..num {
            let pubkey = huione_sdk::pubkey::new_rand();
            let account =
                AccountSharedData::new((t + 1) as u128, space, AccountSharedData::default().owner());
            pubkeys.push(pubkey);
            assert!(accounts
        ...
        for t in 0..num_vote {
            let pubkey = huione_sdk::pubkey::new_rand();
            let account =
                AccountSharedData::new((num + t + 1) as u128, space, &huione_vote_program::id());
            pubkeys.push(pubkey);
            let ancestors = vec![(slot, 0)].into_iter().collect();
            assert!(accounts
                .load_without_fixed_root(&ancestors, &pubkey)
    ...
    fn test_clean_old_with_both_normal_and_zero_lamport_accounts() {
        ...
        let mut account_data_with_mint = vec![0; inline_hpl_token::Account::get_packed_len()];
        ...
        normal_account.set_owner(inline_hpl_token::id());
        ...
        zero_account.set_owner(inline_hpl_token::id());
        ...
        let index_key = IndexKey::HplTokenMint(mint_key);
        let bank_id = 0;
        accounts
            .accounts_index
            .index_scan_accounts(
    ...
    fn test_accounts_db_serialize_zero_and_free() {
        ...
        let filler_account_pubkey = huione_sdk::pubkey::new_rand();
        ...
        F: Fn(AccountsDb, Slot) -> AccountsDb,
    {
        ...
        let pubkey = huione_sdk::pubkey::new_rand();
        let purged_pubkey1 = huione_sdk::pubkey::new_rand();
        let purged_pubkey2 = huione_sdk::pubkey::new_rand();
    ...
    fn test_hash_stored_account() {
        ...
        let expected_account_hash = if cfg!(debug_assertions) {
            Hash::from_str("6j8aLo9jh6vQudYyafvSKZCB8qpXdYenhdCFPQqGhT5C").unwrap()
        } else {
    ...
    fn test_calculate_accounts_hash_check_hash_mismatch() {
        ...
        // huione wrong hash value in store so we get a mismatch
    ...
    fn test_verify_bank_capitalization() {
        for pass in 0..2 {
            ...
            let native_account_pubkey = huione_sdk::pubkey::new_rand();
            db.store_for_tests(
                some_slot,
                &[(
                    &native_account_pubkey,
                    &huione_sdk::native_loader::create_loadable_account_for_test("foo"),
                )],
            );
            db.add_root_and_flush_write_cache(some_slot);
            db.update_accounts_hash_for_tests(some_slot, &ancestors, true, true);
    ...
    fn test_storage_finder() {
        ...
        let lamports = 100;
        let data_len = 8190;
        let account = AccountSharedData::new(lamports, data_len, &huione_sdk::pubkey::new_rand());
    ...
    fn test_accounts_purge_long_chained_after_snapshot_restore() {
        ...
        let pubkey = huione_sdk::pubkey::new_rand();
        let dummy_pubkey = huione_sdk::pubkey::new_rand();
        let purged_pubkey1 = huione_sdk::pubkey::new_rand();
        let purged_pubkey2 = huione_sdk::pubkey::new_rand();
    ...
    fn test_shrink_stale_slots_processed() {
        ...
        for startup in &[false, true] {
            ...
            let pubkeys: Vec<_> = (0..pubkey_count)
                .map(|_| huione_sdk::pubkey::new_rand())
                .collect();
    ...
    fn test_select_candidates_by_total_usage_3_way_split_condition() {
        // three candidates, one selected for shrink, one is huione back to the candidate list and one is ignored
        huione_logger::setup();
    ...
    fn test_account_balance_for_capitalization_sysvar() {
        let normal_sysvar = huione_sdk::account::create_account_for_test(
            &huione_sdk::slot_history::SlotHistory::default(),
        );
    ...
    fn test_checked_sum_for_capitalization_overflow() {
        assert_eq!(
            AccountsDb::checked_sum_for_capitalization(vec![1, u128::max_value()].into_iter()),
            3
        );
    }
    ...
    fn test_store_reuse() {
        ...
        for i in 0..num_accounts {
            let account = AccountSharedData::new((i + 1) as u128, size, &Pubkey::default());
            let pubkey = huione_sdk::pubkey::new_rand();
        ...
        for (i, key) in keys[1..].iter().enumerate() {
            let account =
                AccountSharedData::new((1 + i + num_accounts) as u128, size, &Pubkey::default());
    ...
    fn test_flush_accounts_cache() {
        ...
        let root5 = 5;
        let root6 = 6;
        let unrooted_key = huione_sdk::pubkey::new_rand();
        let key5 = huione_sdk::pubkey::new_rand();
        let key6 = huione_sdk::pubkey::new_rand();
    ...
    fn start_load_thread(
        ...
        exit: Arc<AtomicBool>,
        pubkey: Arc<Pubkey>,
        expected_lamports: impl Fn(&(AccountSharedData, Slot)) -> u128 + Send + 'static,
    ) -> JoinHandle<()> {
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    loop {
                        ...
                        account.set_lamports(slot as u128 + 1);
    ...
    fn test_calculate_storage_count_and_alive_bytes() {
        let accounts = AccountsDb::new_single_for_tests();
        let shared_key = huione_sdk::pubkey::new_rand();
        let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
        let slot0 = 0;
    ...
    fn test_calculate_storage_count_and_alive_bytes_2_accounts() {
        let accounts = AccountsDb::new_single_for_tests();
        let keys = [
            huione_sdk::pubkey::Pubkey::from([0; 32]),
            huione_sdk::pubkey::Pubkey::from([255; 32]),
        ];
    ...
    fn test_clean_accounts_with_last_full_snapshot_slot() {
        ...
        let accounts_db = AccountsDb::new_single_for_tests();
        let pubkey = huione_sdk::pubkey::new_rand();
        let owner = huione_sdk::pubkey::new_rand();
        let space = 0;
    ...
    fn test_many_unrefs() {
        ...
        // huione the pubkey into the acct idx in 'n' slots
                            ...
                            for mut account_count in account_counts {
                                ...
                                for pubkey in pubkeys.iter().take(account_count) {
                                    ...
                                    if Some(pubkey) == pubkey_opposite_zero_lamports {
                                        account.set_lamports(u128::from(old_lamports == 0));
                                    }
    ...
    fn test_shrink_ancient() {
        ...
        // include all the slots we huione into the ancient append vec - they should contain nothing
        compare_all_accounts(
            &initial_accounts,
            &get_all_accounts(&db, ancient_slot..(max_slot_inclusive + 1)),
        );
