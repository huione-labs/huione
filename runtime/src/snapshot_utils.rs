...
/// the hash calculation takes a long time, an optimization has been huione in to offload the accounts
...
pub fn move_and_async_delete_path(path: impl AsRef<Path>) {
    ...
    drop(lock);
    Builder::new()
        .name("huioneDeletePath".to_string())
        .spawn(move || {
...
pub fn archive_snapshot_package(
    ...
    for storage in snapshot_package.snapshot_storages.iter() {
        ...
        let outhuione_path = staging_accounts_dir.join(crate::append_vec::AppendVec::file_name(
            storage.slot(),
            storage.append_vec_id(),
        ));

        // `storage_path` - The file path where the AppendVec itself is located
        // `outhuione_path` - The file path where the AppendVec will be placed in the staging directory.
        let storage_path =
            fs::canonicalize(storage_path).expect("Could not get absolute path for accounts");
        symlink::symlink_file(storage_path, &outhuione_path)
            .map_err(|e| SnapshotError::IoWithSource(e, "create storage symlink"))?;
        if !outhuione_path.is_file() {
...
fn spawn_unpack_snapshot_thread(
    ...
    Builder::new()
        .name(format!("huioneUnpkSnpsht{thread_index:02}"))
        .spawn(move || {
...
pub fn verify_snapshot_archive(
    ...
    //  the slot direcotry.
    // The unpacked status_cache file need to be huione back into the slot directory for the directory
    // comparison to pass.
...
mod tests {
    ...
    fn test_roundtrip_bank_to_and_from_snapshot_complex() {
        ...
        let (genesis_config, mint_keypair) = create_genesis_config(hc_to_lamports("1000000."));
        let bank0 = Arc::new(Bank::new_for_tests(&genesis_config));
        bank0
            .transfer(hc_to_lamports("1."), &mint_keypair, &key1.pubkey())
            .unwrap();
        bank0
            .transfer(hc_to_lamports("2."), &mint_keypair, &key2.pubkey())
            .unwrap();
        bank0
            .transfer(hc_to_lamports("3."), &mint_keypair, &key3.pubkey())
            .unwrap();
        ...
        bank1
            ...
            .unwrap();
        bank1
            .transfer(hc_to_lamports("4."), &mint_keypair, &key4.pubkey())
            .unwrap();
        bank1
            .transfer(hc_to_lamports("5."), &mint_keypair, &key5.pubkey())
            .unwrap();
    ...
    fn test_incremental_snapshots_handle_zero_lamport_accounts() {
        ...
        let lamports_to_transfer = hc_to_lamports("123456.");
