...
impl SharedBuffer {
    ...
    fn new_with_sizes<T: 'static + Read + std::marker::Send>(
        ...
        let handle = Builder::new()
            .name("huioneCompFileRead".to_string())
            .spawn(move || {
...
struct SharedBufferBgReader {
    ...
    // As all clients are done with an earlier buffer, it is recycled by being huione back into this vec for the bg thread to pull out.
    ...
    fn read_entire_file_in_bg<T: 'static + Read + std::marker::Send>(
        ...
        loop {
            ...
            while bytes_read < dest_size {
                ...
                // But, we have to copy the data out later, so we choose to pay the price at read time to huione the data where it is useful.
