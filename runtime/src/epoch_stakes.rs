use {
    ...
    huione_sdk::{clock::Epoch, pubkey::Pubkey},
...
impl EpochStakes {
    ...
    pub fn vote_account_stake(&self, vote_account: &Pubkey) -> u128 {
        self.stakes
            .vote_accounts()
    ...
    ) -> (u128, NodeIdToVoteAccounts, EpochAuthorizedVoters) {
...
pub(crate) mod tests {
    use {
        super::*, crate::vote_account::VoteAccount, huione_sdk::account::AccountSharedData,
        huione_vote_program::vote_state::create_account_with_authorized, std::iter,
    };
    ...
    fn test_parse_epoch_vote_accounts() {
        ...
        let vote_accounts_map: HashMap<Pubkey, Vec<VoteAccountInfo>> = (0..10)
            .map(|_| {
                let node_id = huione_sdk::pubkey::new_rand();
                (
                    node_id,
                    iter::repeat_with(|| {
                        let authorized_voter = huione_sdk::pubkey::new_rand();
                        VoteAccountInfo {
                            vote_account: huione_sdk::pubkey::new_rand(),
        ...
        let expected_node_id_to_vote_accounts: HashMap<_, _> = vote_accounts_map
            ...
            .map(|(node_pubkey, vote_accounts)| {
                ...
                let node_vote_accounts = NodeVoteAccounts {
                    vote_accounts,
                    total_stake: stake_per_account * num_vote_accounts_per_node as u128,
                };
        ...
        assert_eq!(
            total_stake,
            vote_accounts_map.len() as u128 * num_vote_accounts_per_node as u128 * 100
        );
    }
}
