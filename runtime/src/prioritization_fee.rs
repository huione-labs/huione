use {
    huione_measure::measure,
    huione_sdk::{clock::Slot, pubkey::Pubkey, saturating_add_assign},
...
impl PrioritizationFeeMetrics {
    fn accumulate_total_prioritization_fee(&mut self, val: u128) {
    ...
    fn update_prioritization_fee(&mut self, fee: u128) {
        if fee == 0 {
    ...
    fn report(&self, slot: Slot) {
        datapoint_info!(
            ...
            (
                ...
                self.total_prioritization_fee as i128,
                i128
            ),
            (
                "min_prioritization_fee",
                self.min_prioritization_fee.unwrap_or(0) as i128,
                i128
            ),
            (
...
impl PrioritizationFee {
    ...
    pub fn get_min_transaction_fee(&self) -> Option<u128> {
        (self.min_transaction_fee != u128::MAX).then_some(self.min_transaction_fee)
    }

    pub fn get_writable_account_fee(&self, key: &Pubkey) -> Option<u128> {
        self.min_writable_account_fees.get(key).copied()
    }

    pub fn get_writable_account_fees(&self) -> impl Iterator<Item = (&Pubkey, &u128)> {
    ...
    pub fn report_metrics(&self, slot: Slot) {
        ...
        datapoint_info!(
            ...
            ("slot", slot as i128, i128),
            ("entity", "block", String),
            ("min_prioritization_fee", min_transaction_fee as i128, i128),
        );
        for (account_key, fee) in accounts_fees.iter().take(10) {
            datapoint_trace!(
                ...
                ("min_prioritization_fee", **fee as i128, i128),
            );
        }
    }
}
...
#[cfg(test)]
mod tests {
    use {super::*, huione_sdk::pubkey::Pubkey};
