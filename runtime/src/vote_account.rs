...
pub type VoteAccountsHashMap = HashMap<Pubkey, (/*stake:*/ u128, VoteAccount)>;
    ...
    staked_nodes: OnceCell<
        Arc<
            HashMap<
                ...
                u128,    // Total stake across all vote-accounts.
            >,
        >,
    >,
}
...
impl VoteAccount {
    ...
    pub(crate) fn lamports(&self) -> u128 {
...
impl VoteAccounts {
    ...
    pub fn staked_nodes(&self) -> Arc<HashMap</*node_pubkey:*/ Pubkey, /*stake:*/ u128>> {
        self.staked_nodes
            .get_or_init(|| {
                Arc::new(
                    self.vote_accounts
                        .values()
                        .filter(|(stake, _)| *stake != 0u128)
    ...
    pub fn get_delegated_stake(&self, pubkey: &Pubkey) -> u128 {
        self.vote_accounts
            .get(pubkey)
    ...
    pub(crate) fn delegated_stakes(&self) -> impl Iterator<Item = (&Pubkey, u128)> {
        self.vote_accounts
            .iter()
    ...
    pub(crate) fn find_max_by_delegated_stake(&self) -> Option<&VoteAccount> {
        let key = |(_pubkey, (stake, _vote_account)): &(_, &(u128, _))| *stake;
        let (_pubkey, (_stake, vote_account)) = self.vote_accounts.iter().max_by_key(key)?;
        Some(vote_account)
    }

    pub(crate) fn insert(&mut self, pubkey: Pubkey, (stake, vote_account): (u128, VoteAccount)) {
    ...
    pub(crate) fn remove(&mut self, pubkey: &Pubkey) -> Option<(u128, VoteAccount)> {
        let vote_accounts = Arc::make_mut(&mut self.vote_accounts);
    ...
    pub(crate) fn add_stake(&mut self, pubkey: &Pubkey, delta: u128) {
        let vote_accounts = Arc::make_mut(&mut self.vote_accounts);
    ...
    pub(crate) fn sub_stake(&mut self, pubkey: &Pubkey, delta: u128) {
        let vote_accounts = Arc::make_mut(&mut self.vote_accounts);
        if let Some((stake, vote_account)) = vote_accounts.get_mut(pubkey) {
            *stake = stake
                .checked_sub(delta)
    ...
    fn add_node_stake(&mut self, stake: u128, vote_account: &VoteAccount) {
        if stake == 0u128 {
            return;
        }
    ...
    fn sub_node_stake(&mut self, stake: u128, vote_account: &VoteAccount) {
        if stake == 0u128 {
            return;
        }
        let staked_nodes = match self.staked_nodes.get_mut() {
...
impl TryFrom<AccountSharedData> for VoteAccountInner {
    type Error = Error;
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !huione_vote_program::check_id(account.owner()) {
...
impl FromIterator<(Pubkey, (/*stake:*/ u128, VoteAccount))> for VoteAccounts {
    fn from_iter<I>(iter: I) -> Self
    where
        I: IntoIterator<Item = (Pubkey, (u128, VoteAccount))>,
    {
...
mod tests {
    use {
        ...
        bincode::Options,
        rand::Rng,
        huione_sdk::{pubkey::Pubkey, sysvar::clock::Clock},
        huione_vote_program::vote_state::{VoteInit, VoteStateVersions},
    ...
    fn new_rand_vote_accounts<R: Rng>(
        rng: &mut R,
        num_nodes: usize,
    ) -> impl Iterator<Item = (Pubkey, (/*stake:*/ u128, VoteAccount))> + '_ {
    ...
    fn staked_nodes<'a, I>(vote_accounts: I) -> HashMap<Pubkey, u128>
    where
        I: IntoIterator<Item = &'a (Pubkey, (u128, VoteAccount))>,
    {
