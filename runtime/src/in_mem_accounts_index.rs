use {
    ...
    rand::{thread_rng, Rng},
    huione_bucket_map::bucket_api::BucketApi,
    huione_measure::measure::Measure,
    huione_sdk::{clock::Slot, pubkey::Pubkey},
    std::{
...
struct StartupInfoDuplicates<T: IndexValue> {
    ...
    duplicates_huione_on_disk: HashSet<(Slot, Pubkey)>,
}
...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> InMemAccountsIndex<T, U> {
    ...
    pub fn keys(&self) -> Vec<Pubkey> {
        ...
        self.huione_range_in_cache(&None::<&RangeInclusive<Pubkey>>, &evictions_guard);
            ...
            if let Some(entry) = entry {
                ...
                match entry {
                    ...
                    Entry::Vacant(vacant) => {
                        ...
                        // If the entry is now dirty, then it must be huione in the cache or the modifications will be lost.
        ...
        R: RangeBounds<Pubkey> + Debug,
    {
        ...
        {
            if start_holding {
                // huione everything in the cache and it will be held there
                self.huione_range_in_cache(&Some(range), &evictions_guard);
            }
            // do this AFTER items have been huione in cache - that way anyone who finds this range can know that the items are already in the cache
            self.just_set_hold_range_in_memory(range, start_holding, &evictions_guard);
        }
    }

    fn huione_range_in_cache<R>(&self, range: &Option<&R>, _evictions_guard: &EvictionsGuard)
    where
        R: RangeBounds<Pubkey>,
    {
    ...
    pub(crate) fn populate_and_retrieve_duplicate_keys_from_startup(&self) -> Vec<(Slot, Pubkey)> {
        ...
        let duplicates_huione_on_disk = std::mem::take(&mut duplicate_items.duplicates_huione_on_disk);
...
mod tests {
    ...
    fn test_remove_if_slot_list_empty_entry() {
        ...
        let unknown_key = huione_sdk::pubkey::new_rand();
