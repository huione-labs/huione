...
pub struct AccountHashesFile {
    ...
    /// The directory where temporary cache files are huione
...
impl CalculateHashIntermediate {
    pub fn new(hash: Hash, lamports: u128, pubkey: Pubkey) -> Self {
        Self {
            hash,
            lamports,
            pubkey,
...
impl AccountsHasher {
    ...
    pub fn checked_cast_for_capitalization(balance: U256) -> u128 {
    ...
    fn de_dup_accounts<'a>(
        ...
        let min_max_sum_entries_hashes = Mutex::new((usize::MAX, usize::MIN, 0u128, 0usize, 0usize));
            ...
            .map(|bin| {
                    ...
                    self.de_dup_accounts_in_parallel(sorted_data_by_pubkey, bin);
                {
                    ...
                    lamports_sum = Self::checked_cast_for_capitalization(
                        U256::new(lamports_sum)  + U256::new(lamports_bin) ,
                    );
            ...
            if item.lamports != 0 {
                ...
                if !(filler_accounts_enabled && self.is_filler_account(&item.pubkey)) {
                    overall_sum = Self::checked_cast_for_capitalization(
                        U256::new(item.lamports)  + U256::new(overall_sum)
                    );
...
pub mod tests {
    ...
    fn test_accountsdb_de_dup_accounts_from_stores() {
        ...
        let accounts: Vec<_> = hashes
            .zip(keys.iter())
            .enumerate()
            .map(|(i, (hash, key))| CalculateHashIntermediate::new(hash, (i + 1) as u128, *key))
            .collect();

        type ExpectedType = (String, bool, u128, String);
