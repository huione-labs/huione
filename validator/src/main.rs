...
use {
    ...
    huione_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of},
    huione_core::{
    ...
    huione_gossip::{cluster_info::Node, legacy_contact_info::LegacyContactInfo as ContactInfo},
    huione_ledger::blockstore_options::{
        BlockstoreCompressionType, BlockstoreRecoveryMode, LedgerColumnOptions, ShredStorageType,
    },
    huione_perf::recycler::enable_recycler_warming,
    huione_poh::poh_service,
    huione_rpc::{
        rpc::{JsonRpcConfig, RpcBigtableConfig,RpcHBaseConfig},
        rpc_pubsub_service::PubSubConfig,
    },
    huione_rpc_client::rpc_client::RpcClient,
    huione_rpc_client_api::config::RpcLeaderScheduleConfig,
    huione_runtime::{
        accounts_db::{
    ...
    huione_send_transaction_service::send_transaction_service,
    huione_streamer::socket::SocketAddrSpace,
    huione_tpu_client::tpu_client::DEFAULT_TPU_ENABLE_UDP,
    huione_validator::{
        admin_rpc_service,
...
fn wait_for_restart_window(
    ...
    loop {
        ...
        let delinquent_stake_percentage = {
            ...
            let current_stake: u128 = vote_accounts
                .current
                .iter()
                .map(|va| va.activated_stake)
                .sum();
            let delinquent_stake: u128 = vote_accounts
                .delinquent
                .iter()
                .map(|va| va.activated_stake)
                .sum();
            ...
            if !healthy {
                ...
                match in_leader_schedule_hole {
                    Ok(_) => {
                        ...
                        {
                            // Restarts using just a full snapshot will huione the node significantly
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr]) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match huione_net_utils::get_cluster_shred_version(entrypoint) {
...
pub fn main() {
    ...
    let huione_version = huione_version::version!();
    let cli_app = app(huione_version, &default_args);
    ...
    let operation = match matches.subcommand() {
        ...
        ("contact-info", Some(subcommand_matches)) => {
            let outhuione_mode = subcommand_matches.value_of("output");
            let admin_client = admin_rpc_service::connect(&ledger_path);
            ...
            if let Some(mode) = outhuione_mode {
                match mode {
        ...
        ("set-public-address", Some(subcommand_matches)) => {
            let parse_arg_addr = |arg_name: &str, arg_long: &str| -> Option<SocketAddr> {
                subcommand_matches.value_of(arg_name).map(|host_port| {
                        huione_net_utils::parse_host_port(host_port).unwrap_or_else(|err| {
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("huione-validator-{}.log", identity_keypair.pubkey()));
    ...
    if cuda {
        huione_perf::perf_libs::init_cuda();
        enable_recycler_warming();
    }

    huione_core::validator::report_target_features();
    ...
    let bind_address = huione_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        huione_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
            .expect("invalid rpc_bind_address")
    } else if private_rpc {
        huione_net_utils::parse_host("127.0.0.1").unwrap()
    } else {
        bind_address
    };
    ...
    let entrypoint_addrs = values_t!(matches, "entrypoint", String)
        ...
        .into_iter()
        .map(|entrypoint| {
            huione_net_utils::parse_host_port(&entrypoint).unwrap_or_else(|e| {
    ...
    let tower_storage: Arc<dyn huione_core::tower_storage::TowerStorage> =
    ...
    let rpc_hbase_config = if matches.is_present("enable_rpc_hbase_ledger_storage")
        || matches.is_present("enable_hbase_ledger_upload")
    {
        Some(RpcHBaseConfig {
            enable_hbase_ledger_upload: matches.is_present("enable_hbase_ledger_upload"),
            enable_rpc_hbase_ledger_storage: matches.is_present("enable_rpc_hbase_ledger_storage"),
            hbase_rpc_url: matches.value_of("hbase_rpc_url").map(|address| {
                huione_net_utils::parse_host_port(address).expect("failed to parse faucet address").to_string()
            }).unwrap(),
        })
    } else {
        None
    ...
    let mut validator_config = ValidatorConfig {
        ...
        rpc_config: JsonRpcConfig {
            ...
            faucet_addr: matches.value_of("rpc_faucet_addr").map(|address| {
                huione_net_utils::parse_host_port(address).expect("failed to parse faucet address")
            }),
            full_api,
    ...
    let dynamic_port_range =
        huione_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches.value_of("public_rpc_addr").map(|addr| {
        huione_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.huione.com/running-validator/validator-start#system-tuning");
            exit(1);
        }
    }
    ...
    let gossip_host: IpAddr = matches
        ...
        .map(|gossip_host| {
            huione_net_utils::parse_host(gossip_host).unwrap_or_else(|err| {
        ...
        .unwrap_or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                let gossip_host = order.into_iter().find_map(|i| {
                    ...
                    huione_net_utils::get_public_ip_addr(entrypoint_addr).map_or_else(
                        |err| {
                            eprintln!(
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            huione_net_utils::find_available_port_in_range(bind_address, (0, 1)).unwrap_or_else(
                |err| {
    ...
    let public_tpu_addr = matches.value_of("public_tpu_addr").map(|public_tpu_addr| {
        huione_net_utils::parse_host_port(public_tpu_addr).unwrap_or_else(|err| {
    ...
    let public_tpu_forwards_addr =
        matches
            ...
            .map(|public_tpu_forwards_addr| {
                huione_net_utils::parse_host_port(public_tpu_forwards_addr).unwrap_or_else(|err| {
    ...
    if !private_rpc {
        ...
        let rpc_url = value_t!(matches, "public_rpc_url", String);
        match rpc_url {
            Ok(url) => node.info.rpc_url = url,
            Err(_) =>{},
        }
    }

    huione_metrics::set_host_id(identity_keypair.pubkey().to_string());
    huione_metrics::set_panic_hook("validator", Some(String::from(huione_version)));
    huione_entry::entry::init_poh();
...
fn process_account_indexes(matches: &ArgMatches) -> AccountSecondaryIndexes {
    let account_indexes: HashSet<AccountIndex> = matches
        ...
        .map(|value| match value {
            ...
            "hpl-token-mint" => AccountIndex::HplTokenMint,
            "hpl-token-owner" => AccountIndex::HplTokenOwner,
            "hpl-nft-owner" => AccountIndex::HplNftOwner,
            "hpl-name-owner" => AccountIndex::HplNameOwner,
            _ => unreachable!(),
        })
        .collect();
