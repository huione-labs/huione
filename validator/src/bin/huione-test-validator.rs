use {
    ...
    log::*,
    huione_clap_utils::{
        input_parsers::{pubkey_of, pubkeys_of, value_of, lamports_of_hc},
        input_validators::normalize_to_url_if_moniker,
        
    },
    huione_core::tower_storage::FileTowerStorage,
    huione_faucet::faucet::run_local_faucet_with_port,
    huione_rpc::{
        rpc::{JsonRpcConfig, RpcBigtableConfig},
        rpc_pubsub_service::PubSubConfig,
    },
    huione_rpc_client::rpc_client::RpcClient,
    huione_runtime::accounts_index::{AccountIndex, AccountSecondaryIndexes},
    huione_sdk::{
...
fn main() {
    ...
    let version = huione_version::version!();
    ...
    huione_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        huione_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        huione_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    let parse_address = |address: &str, inhuione_type: &str| {
        address
            ...
            .unwrap_or_else(|err| {
                println!("Error: invalid {inhuione_type} {address}: {err}");
                exit(1);
            })
    };
    ...
    if let Some(values) = matches.values_of("bpf_program") {
        for (address, program) in values.into_iter().tuples() {
            ...
            upgradeable_programs_to_load.push(UpgradeableProgramInfo {
                program_id: address,
                loader: huione_sdk::bpf_loader_upgradeable::id(),
    ...
    let faucet_lamports_str : String = value_of(&matches, "faucet_hc").unwrap();
    let faucet_lamports = hc_to_lamports(&faucet_lamports_str);
    ...
    // let faucet_per_time_cap = value_t!(matches, "faucet_per_time_huione_cap", String)
    //     .ok()
    //     .map(hc_to_lamports);
    let faucet_per_time_cap = lamports_of_hc(&matches,"faucet_per_time_huione_cap");
    // let faucet_per_request_cap = value_t!(matches, "faucet_per_request_huione_cap", String)
    //     .ok()
    //     .map(hc_to_lamports);
    let faucet_per_request_cap = lamports_of_hc(&matches, "faucet_per_request_huione_cap");
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `huione airdrop` localnet HC after creating one\n",
        );
    }
