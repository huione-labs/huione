use {
    ...
    huione_geyser_plugin_manager::GeyserPluginManagerRequest,
    huione_gossip::contact_info::{ContactInfo, Protocol, SOCKET_ADDR_UNSPECIFIED},
    huione_rpc::rpc::verify_pubkey,
    huione_rpc_client_api::{config::RpcAccountIndex, custom_error::RpcCustomError},
    huione_runtime::accounts_index::AccountIndex,
    huione_sdk::{
        exit::Exit,
        pubkey::Pubkey,
...
impl AdminRpc for AdminRpcImpl {
    ...
    fn set_log_filter(&self, filter: String) -> Result<()> {
        ...
        huione_logger::setup_with(&filter);
        Ok(())
    }
...
fn rpc_account_index_from_account_index(account_index: &AccountIndex) -> RpcAccountIndex {
    match account_index {
        ...
        AccountIndex::HplTokenOwner => RpcAccountIndex::HplTokenOwner,
        AccountIndex::HplTokenMint => RpcAccountIndex::HplTokenMint,
        AccountIndex::HplNameOwner => RpcAccountIndex::HplNameOwner,
        AccountIndex::HplNftOwner => RpcAccountIndex::HplNftOwner,
    }
}
...
fn account_index_from_rpc_account_index(rpc_account_index: &RpcAccountIndex) -> AccountIndex {
    match rpc_account_index {
        ...
        RpcAccountIndex::HplTokenOwner => AccountIndex::HplTokenOwner,
        RpcAccountIndex::HplTokenMint => AccountIndex::HplTokenMint,
        RpcAccountIndex::HplNameOwner => AccountIndex::HplNameOwner,
        RpcAccountIndex::HplNftOwner => AccountIndex::HplNftOwner,
    }
}
...
pub struct StakedNodesOverrides {
    ...
    pub staked_map_id: HashMap<Pubkey, u128>,
}

pub fn deserialize_pubkey_map<'de, D>(des: D) -> std::result::Result<HashMap<Pubkey, u128>, D::Error>
where
    D: Deserializer<'de>,
{
    let container: HashMap<String, u128> = serde::Deserialize::deserialize(des)?;
    let mut container_typed: HashMap<Pubkey, u128> = HashMap::new();
