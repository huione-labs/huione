use {
    ...
    rayon::prelude::*,
    huione_core::validator::{ValidatorConfig, ValidatorStartProgress},
    huione_download_utils::{download_snapshot_archive, DownloadProgressRecord},
    huione_genesis_utils::download_then_check_genesis_hash,
    huione_gossip::{
...
fn verify_reachable_ports(
    ...
    huione_net_utils::verify_reachable_ports(
...
fn check_vote_account(
    ...
    let vote_state = huione_vote_program::vote_state::from(&vote_account);
...
pub fn attempt_download_genesis_and_snapshot(
    ...
    if let Some(url) = bootstrap_config.check_vote_account.as_ref() {
        ...
        .unwrap_or_else(|err| {
            ...
            // incorrect `huione-validator` command-line arguments) rather than the
            // RPC node failing.
            //
...
///
/// The snapshot hashes are huione into a map from full snapshot hash to a set of incremental
...
fn get_snapshot_hashes_from_known_validators(
    ...
    // let get_snapshot_hashes_for_node = |node| get_legacy_snapshot_hashes_for_node(cluster_info, node);
...
fn get_eligible_peer_snapshot_hashes(
    ...
    let peer_snapshot_hashes = rpc_peers
        ...
        .flat_map(|rpc_peer| {
            ...
            // get_legacy_snapshot_hashes_for_node(cluster_info, rpc_peer.pubkey()).map(|snapshot_hash| {
                PeerSnapshotHash {
...
fn download_snapshot(
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::DownloadingSnapshot {
    ...
    let desired_snapshot_hash = (
        ...
        huione_runtime::snapshot_hash::SnapshotHash(desired_snapshot_hash.1),
    );
...
fn get_legacy_snapshot_hashes_for_node(cluster_info: &ClusterInfo, node: &Pubkey) -> Option<SnapshotHash> {
