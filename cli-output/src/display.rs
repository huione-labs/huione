...
fn is_memo_program(k: &Pubkey) -> bool {
    ...
    // (k_str == hpl_memo_v1_id().to_string()) || (k_str == hpl_memo_id().to_string())
    k_str == hpl_memo_id().to_string()
}
...
pub fn build_balance_message(lamports: u128, use_lamports_unit: bool, show_unit: bool) -> String {
...
fn write_instruction<'a, W: io::Write>(
    ...
    if let AccountKeyType::Known(program_pubkey) = program_pubkey {
        if program_pubkey == &huione_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                huione_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == &huione_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                huione_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
        ...
        if !rewards.is_empty() {
            ...
            for reward in rewards {
                ...
                writeln!(
                    ...
                    sign,
                    lamports_to_hc(reward.lamports.abs() as u128),
...
fn write_fees<W: io::Write>(w: &mut W, transaction_fee: u128, prefix: &str) -> io::Result<()> {
    writeln!(w, "{}  Fee: ◎{}", prefix, lamports_to_hc(transaction_fee))
}
...
mod test {
    use {
        ...
        huione_transaction_status::{Reward, RewardType, TransactionStatusMeta},
        std::io::BufWriter,
    };
