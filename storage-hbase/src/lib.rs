...
use {
    ...
    huione_metrics::inc_new_counter_debug,
    huione_sdk::{
    ...
    huione_storage_proto::convert::{generated, tx_by_addr},
...
pub const DEFAULT_LEDGER_NAME: &str = "huione-ledger";
pub const DEFAULT_HBASE_NAMESPACE: &str = "default";

#[derive(Debug, Error)]
pub enum Error {
    #[error("HBase: {0}")]
    HBaseError(hbase::Error),
...
//
// Note: in order to continue to support old bincode-serialized hbase entries, if new fields are
...
impl LedgerStorage {
    pub fn new(
        ...
        let connection =
            hbase::HBaseConnection::new(thrift2_url, timeout)?;
        Ok(Self { connection })
    }

    /// Return the available slot that contains a block
    pub fn get_first_available_block(&self) -> Result<Option<Slot>> {
        ...
        let blocks = hbase.get_row_keys("blocks", None, None, 1)?;
        if blocks.is_empty() {
            return Ok(None);
        }
    ...
    pub fn get_confirmed_blocks(&self, start_slot: Slot, limit: usize) -> Result<Vec<Slot>> {
        debug!(
    ...
    pub fn get_confirmed_blocks_with_data<'a>(
        &self,
        slots: &'a [Slot],
        ...
        let data = hbase
            ...
            .filter_map(
                |(row_key, block_cell_data): (
                    RowKey,
                    hbase::CellData<StoredConfirmedBlock, generated::ConfirmedBlock>,
                )| {
                    let block = match block_cell_data {
                        hbase::CellData::Bincode(block) => block.into(),
                        hbase::CellData::Protobuf(block) => block.try_into().ok()?,
                    };
    ...
    pub fn get_signature_status(&self, signature: &Signature) -> Result<TransactionStatus> {
        debug!(
    ...
    > {
        ...
        let tx_by_addr_data = hbase
            .get_row_data(
                ...
                limit as i32 + starting_slot_tx_len as i32,
            )?;
    ...
    pub async fn upload_confirmed_block(
        ...
        if !tx_cells.is_empty() {
            bytes_written += self
                .connection
                .huione_bincode_cells_with_retry::<TransactionInfo>("tx", &tx_cells)?;
        }

        if !tx_by_addr_cells.is_empty() {
            bytes_written += self
                .connection
                .huione_protobuf_cells_with_retry::<tx_by_addr::TransactionByAddr>(
                    "tx-by-addr",
                    &tx_by_addr_cells,
                )?;
        }
        ...
        bytes_written += self
            .connection
            .huione_protobuf_cells_with_retry::<generated::ConfirmedBlock>("blocks", &blocks_cells)?;
        info!(
    ...
    pub fn delete_confirmed_block(&self, slot: Slot, dry_run: bool) -> Result<()> {
            ...
            for (signature, expected_tx_info) in expected_tx_infos {
                match fetched_tx_infos.get(&signature) {
                    ...
                    Some(Ok(fetched_tx_info)) => {
                        warn!(
                            "skipped tx row {} because the hbase entry ({:?}) did not match to {:?}",
                            signature,
                            fetched_tx_info,
                            &expected_tx_info,
                        );
                    ...
                    Some(Err(err)) => {
                        warn!(
                            "skipped tx row {} because the hbase entry was corrupted: {:?}",
                            signature, err
                        );
                    }
                    None => {
        ...
        if !dry_run {
            if !address_slot_rows.is_empty() {
                self.connection
                    .delete_rows_with_retry("tx-by-addr", &address_slot_rows, "proto")?;
            }

            if !tx_deletion_rows.is_empty() {
                self.connection
                    .delete_rows_with_retry("tx", &tx_deletion_rows, "bin")?;
            }

            self.connection
                .delete_rows_with_retry("blocks", &[slot_to_blocks_key(slot)], "proto")?;
        }
...
mod test {
    ...
    #[test]
    fn test_slot_to_tx_by_addr_key(){
        println!("slot_to_tx_by_addr_key:{}",slot_to_tx_by_addr_key(0));
        println!("slot_to_tx_by_addr_key:{}",slot_to_tx_by_addr_key(1));
        println!("slot_to_tx_by_addr_key:{}",slot_to_tx_by_addr_key(2));
        println!("slot_to_tx_by_addr_key:{}",slot_to_tx_by_addr_key(1000));
        println!("slot_to_tx_by_addr_key:{}",slot_to_tx_by_addr_key(!0));
    }
}
