    ...
    pub fn finalize(self) -> Hash {
        ...
        if let Ok(dir) = std::env::var("HUIONE_ABI_DUMP_DIR") {
            let thread_name = std::thread::current()
                ...
                .replace(':', "_");
            if thread_name == "main" {
                error!("Bad thread name detected for dumping; Maybe, --test-threads=1? Sorry, HUIONE_ABI_DUMP_DIR doesn't work under 1; increase it");
            }
...
mod tests {
    ...
    #[frozen_abi(digest = "CCK66KouRPZouLFp7QvkvbQP4T2HTwCF3mpQxgUfxUkC")]
    #[derive(Serialize, AbiExample)]
    struct TestNest {
    ...
    #[frozen_abi(digest = "BJdU6Ksozb44TBS1bCYxcxHVqfwEuyZ17ma1kQTjXvVd")]
    #[derive(Serialize, AbiExample)]
    struct TestVecEnum {
