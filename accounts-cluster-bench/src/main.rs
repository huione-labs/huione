...
use {
    ...
    rand::{thread_rng, Rng},
    rayon::prelude::*,
    huione_account_decoder::parse_token::{hpl_token_pubkey},
    huione_clap_utils::{
        hidden_unless_forced, input_parsers::pubkey_of, input_validators::is_url_or_moniker,
    },
    huione_cli_config::{ConfigInput, CONFIG_FILE},
    huione_client::transaction_executor::TransactionExecutor,
    huione_gossip::gossip_service::discover,
    huione_rpc_client::rpc_client::RpcClient,
    huione_runtime::inline_hpl_token,
    huione_sdk::{
...
pub fn poll_get_fee_for_message(client: &RpcClient, message: &mut Message) -> (Option<u128>, Hash) {
    let mut num_retries = MAX_RPC_CALL_RETRIES;
    loop {
...
fn airdrop_lamports(client: &RpcClient, id: &Keypair, desired_balance: u128) -> bool {
            ...
            if let Some(mint_address) = mint {
                instructions.push(hpl_token_instruction(
                    hpl_token::instruction::initialize_account(
                        &hpl_token::id(),
                        &hpl_token_pubkey(&to_pubkey),
                        &hpl_token_pubkey(&mint_address),
                        &hpl_token_pubkey(&base_keypair.pubkey()),
                    )
                    .unwrap(),
                ));
            }
    ...
    let instructions: Vec<_> = (0..num_instructions)
        .filter_map(|_| {
            ...
            let address =
                Pubkey::create_with_seed(&base_keypair.pubkey(), &seed, &program_id).unwrap();
            if hpl_token {
                Some(hpl_token_instruction(
                    hpl_token::instruction::close_account(
                        &hpl_token::id(),
                        &hpl_token_pubkey(&address),
                        &hpl_token_pubkey(&keypair.pubkey()),
                        &hpl_token_pubkey(&base_keypair.pubkey()),
                        &[],
                    )
                    .unwrap(),
                ...
                if chunk_size > 0 {
                    for (i, keypair) in payer_keypairs.iter().enumerate() {
                        let txs: Vec<_> = (0..chunk_size)
                            ...
                            .collect();
                        balances[i] = balances[i].saturating_sub(lamports * txs.len() as u128);
                        info!("txs: {}", txs.len());
            ...
            if close_nth_batch > 0 {
                ...
                if max_closed_seed < expected_closed {
                    let txs: Vec<_> = (0..expected_closed - max_closed_seed)
                        ...
                        .collect();
                    balances[0] = balances[0].saturating_sub(fee * txs.len() as u128);
                    ...
                    if chunk_size > 0 {
                        for (i, keypair) in payer_keypairs.iter().enumerate() {
                            let txs: Vec<_> = (0..chunk_size)
                                ...
                                .collect();
                            balances[i] = balances[i].saturating_sub(fee * txs.len() as u128);
                            info!("close txs: {}", txs.len());
                            let new_ids = executor.push_transactions(txs);
                            info!("close ids: {}", new_ids.len());
                            tx_sent_count += new_ids.len();
...
fn main() {
    ...
    let lamports = value_t!(matches, "lamports", u128).ok();
    ...
    let client = if let Some(addr) = matches.value_of("entrypoint") {
        let entrypoint_addr = huione_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
...
pub mod test {
    use {
        super::*,
        huione_core::validator::ValidatorConfig,
        ...
        huione_sdk::{native_token::hc_to_lamports, poh_config::PohConfig},
    ...
    #[test] // todo oa
    fn test_create_then_reclaim_hpl_token_accounts() {
        huione_logger::setup();
        ...
        // Create Mint
        let hpl_mint_keypair = Keypair::new();
        let (mint_meta ,_) = Pubkey::find_program_address(&[b"MintMeta",&hpl_mint_keypair.pubkey().to_bytes()], &hpl_token::id());
        let hpl_mint_len = Mint::get_packed_len();
        let hpl_mint_rent = rpc_client
            .get_minimum_balance_for_rent_exemption(hpl_mint_len)
            .unwrap();
        let transaction = Transaction::new_signed_with_payer(
            &[
                system_instruction::create_account(
                    &funder.pubkey(),
                    &hpl_mint_keypair.pubkey(),
                    hpl_mint_rent,
                    hpl_mint_len as u64,
                    &inline_hpl_token::id(),
                ),
                hpl_token_instruction(
                ...
                hpl_token_instruction(
                    hpl_token::instruction::initialize_mint(
                        &hpl_token::id(),
                        &hpl_token_pubkey(&hpl_mint_keypair.pubkey()),
                        &hpl_token_pubkey(&hpl_mint_keypair.pubkey()),
                        None,
                        2,
                    )
