...
//! ```bash
//! huione-dos $COMMON --num-signatures 8
//! ```
//! 1.2 With valid signatures
//! ```bash
//! huione-dos $COMMON --valid-signatures --num-signatures 8
//! ```
//! 2. With blockhash and payer:
//! 2.1 Single-instruction transaction
//! ```bash
//! huione-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 1
//! ```
//! 2.2 Multi-instruction transaction
//! ```bash
//! huione-dos $COMMON --valid-blockhash --transaction-type transfer --num-instructions 8
//! ```
//! 2.3 Account-creation transaction
//! ```bash
//! huione-dos $COMMON --valid-blockhash --transaction-type account-creation
//! ```
//!
...
use {
    ...
    log::*,
    rand::{thread_rng, Rng},
    huione_bench_tps::{bench::generate_and_fund_keypairs, bench_tps_client::BenchTpsClient},
    huione_client::{connection_cache::ConnectionCache, tpu_connection::TpuConnection},
    huione_core::serve_repair::{RepairProtocol, RepairRequestHeader, ServeRepair},
    huione_dos::cli::*,
    huione_gossip::{
...
impl TransactionGenerator {
    ...
    fn create_multi_transfer_transaction(&self, payer: &Keypair, to: &[&Keypair]) -> Transaction {
        let to_transfer: u128 = 500_000_000; // specify amount which will cause error
        let to: Vec<(Pubkey, u128)> = to.iter().map(|to| (to.pubkey(), to_transfer)).collect();
...
fn get_target(
    ...
    if nodes.is_empty() {
        // skip-gossip case
        target = Some((huione_sdk::pubkey::new_rand(), entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
...
pub mod test {
    use {
        ...
        huione_gossip::contact_info::LegacyContactInfo,
        huione_local_cluster::{
            cluster::Cluster,
            local_cluster::{ClusterConfig, LocalCluster},
            validator_configs::make_identical_validator_configs,
        },
        huione_rpc::rpc::JsonRpcConfig,
