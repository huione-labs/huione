use {
    ...
    rand::{thread_rng, Rng},
    huione_perf::packet::{PacketBatch, PACKETS_PER_BATCH},
    huione_sdk::{
...
) -> Option<(Pubkey, u128, u128, u128, u128)> {
...
fn compute_receive_window_ratio_for_staked_node(max_stake: u128, min_stake: u128, stake: u128) -> u128 {
            ...
            Ok(new_connection) => {
                ...
                if params.stake > 0 {
                    ...
                    if connection_table_l.total_size < max_staked_connections {
                        ...
                        // huione this connection in the unstaked connection table. If needed, prune a
...
impl ConnectionTable {
    ...
    fn prune_random(&mut self, sample_size: usize, threshold_stake: u128) -> usize {
...
pub mod test {
    ...
    fn test_prune_table_random() {
        ...
        let pruned = table.prune_random(
            2,                      // sample_size
            num_entries as u128 + 1, // threshold_stake
        );
        assert_eq!(pruned, 1);
    }
